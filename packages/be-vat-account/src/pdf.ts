/**
 * The only part of the package that touches a PDF. These statements carry
 * native text, so extraction is exact and no OCR is involved.
 */
 ...
 *
 * Statements run to several pages when a period has many movements. Rows are
 * grouped per page and then concatenated, because Y coordinates restart on each
 * page and grouping across the document would fuse unrelated lines.
 */
