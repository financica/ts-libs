// Generated by scripts/generate-taxonomy.ts from NBB-CBSO 26.0.15.
// Do not edit. Re-run the generator against a new taxonomy release instead.

import type { Check, Datapoint, TaxonomyModule } from "../taxonomy.js";

const datapoints0: readonly Datapoint[] = [
	{
		section: "s.01.00.1",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:psn": "psn:m1",
			"dim:qlt": "qlt:m1",
			"dim:part": "part:m2",
		},
		label: "Enterprise number",
		labels: {
			de: "Unternehmensnummer",
			en: "Enterprise number",
			fr: "Numéro d'entreprise",
			nl: "Ondernemingsnummer",
		},
	},
	{
		section: "s.01.00.1",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m27",
			"dim:psn": "psn:m1",
			"dim:qlt": "qlt:m1",
			"dim:part": "part:m2",
			"dim:mmt": "mmt:m1",
		},
		label: "Enterprise number",
		labels: {
			de: "Unternehmensnummer",
			en: "Enterprise number",
			fr: "Numéro d'entreprise",
			nl: "Ondernemingsnummer",
		},
	},
	{
		section: "s.01.00.1",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m27",
			"dim:psn": "psn:m1",
			"dim:qlt": "qlt:m1",
			"dim:part": "part:m2",
			"dim:mmt": "mmt:m2",
		},
		label: "Enterprise number",
		labels: {
			de: "Unternehmensnummer",
			en: "Enterprise number",
			fr: "Numéro d'entreprise",
			nl: "Ondernemingsnummer",
		},
	},
	{
		section: "s.01.00.1",
		metric: "bln1",
		dimensions: {
			"dim:bas": "bas:m28",
			"dim:psn": "psn:m1",
			"dim:qlt": "qlt:m1",
			"dim:part": "part:m2",
		},
		label: "Enterprise number",
		labels: {
			de: "Unternehmensnummer",
			en: "Enterprise number",
			fr: "Numéro d'entreprise",
			nl: "Ondernemingsnummer",
		},
	},
	{
		section: "s.01.00.1",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:psn": "psn:m1",
			"dim:qlt": "qlt:m1",
			"dim:part": "part:m2",
			"dim:dcl": "dcl:m2",
		},
		label: "Enterprise number",
		labels: {
			de: "Unternehmensnummer",
			en: "Enterprise number",
			fr: "Numéro d'entreprise",
			nl: "Ondernemingsnummer",
		},
	},
	{
		section: "s.01.00.1",
		metric: "dte1",
		dimensions: {
			"dim:evt": "evt:m1",
			"dim:bas": "bas:m27",
			"dim:part": "part:m2",
		},
		label: "Accounts approved by the general assembly of",
		labels: {
			de: "Jahresabschluss, der durch die Generalversammlung vom",
			en: "Accounts approved by the general assembly of",
			fr: "Comptes annuels approuvés par l'assemblée générale du",
			nl: "Jaarrekening goedgekeurd door de algemene vergadering van",
		},
	},
	{
		section: "s.01.00.1",
		metric: "dte1",
		dimensions: {
			"dim:evt": "evt:m1",
			"dim:bas": "bas:m27",
			"dim:part": "part:m2",
			"dim:mmt": "mmt:m1",
		},
		label: "Accounts approved by the general assembly of",
		labels: {
			de: "Jahresabschluss, der durch die Generalversammlung vom",
			en: "Accounts approved by the general assembly of",
			fr: "Comptes annuels approuvés par l'assemblée générale du",
			nl: "Jaarrekening goedgekeurd door de algemene vergadering van",
		},
	},
	{
		section: "s.01.00.1",
		metric: "dte1",
		dimensions: {
			"dim:evt": "evt:m1",
			"dim:bas": "bas:m27",
			"dim:part": "part:m2",
			"dim:mmt": "mmt:m2",
		},
		label: "Accounts approved by the general assembly of",
		labels: {
			de: "Jahresabschluss, der durch die Generalversammlung vom",
			en: "Accounts approved by the general assembly of",
			fr: "Comptes annuels approuvés par l'assemblée générale du",
			nl: "Jaarrekening goedgekeurd door de algemene vergadering van",
		},
	},
	{
		section: "s.01.00.1",
		metric: "bln1",
		dimensions: {
			"dim:evt": "evt:m1",
			"dim:bas": "bas:m28",
			"dim:part": "part:m2",
		},
		label: "Accounts approved by the general assembly of",
		labels: {
			de: "Jahresabschluss, der durch die Generalversammlung vom",
			en: "Accounts approved by the general assembly of",
			fr: "Comptes annuels approuvés par l'assemblée générale du",
			nl: "Jaarrekening goedgekeurd door de algemene vergadering van",
		},
	},
	{
		section: "s.01.00.1",
		metric: "dte1",
		dimensions: {
			"dim:evt": "evt:m1",
			"dim:bas": "bas:m27",
			"dim:part": "part:m2",
			"dim:dcl": "dcl:m2",
		},
		label: "Accounts approved by the general assembly of",
		labels: {
			de: "Jahresabschluss, der durch die Generalversammlung vom",
			en: "Accounts approved by the general assembly of",
			fr: "Comptes annuels approuvés par l'assemblée générale du",
			nl: "Jaarrekening goedgekeurd door de algemene vergadering van",
		},
	},
	{
		section: "s.01.00.1",
		metric: "dte1",
		dimensions: {
			"dim:part": "part:m2",
			"dim:mmt": "mmt:m1",
			"dim:bas": "bas:m27",
		},
		label: "Financial period",
		labels: {
			de: "Geschäftsjahres",
			en: "Financial period",
			fr: "Exercice",
			nl: "Boekjaar",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.01.00.1",
		metric: "dte1",
		dimensions: {
			"dim:part": "part:m2",
			"dim:mmt": "mmt:m2",
			"dim:bas": "bas:m27",
		},
		label: "Financial period",
		labels: {
			de: "Geschäftsjahres",
			en: "Financial period",
			fr: "Exercice",
			nl: "Boekjaar",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.01.00.1",
		metric: "bln1",
		dimensions: {
			"dim:bas": "bas:m28",
			"dim:part": "part:m2",
		},
		label: "Financial period",
		labels: {
			de: "Geschäftsjahres",
			en: "Financial period",
			fr: "Exercice",
			nl: "Boekjaar",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.01.00.1",
		metric: "dte1",
		dimensions: {
			"dim:dcl": "dcl:m1",
			"dim:part": "part:m2",
			"dim:mmt": "mmt:m1",
			"dim:bas": "bas:m27",
		},
		label: "Are data related to the preceding period identical to data published before ?",
		labels: {
			de: "Sind die Beträge des vorangegangenen Geschäftsjahres mit den für das Vorjahr veröffentl. identisch ?",
			en: "Are data related to the preceding period identical to data published before ?",
			fr: "Les montants relatifs à l'exercice précédent sont-ils identiques à ceux publiés antérieurement ?",
			nl: "Zijn de bedragen van het vorige boekjaar identiek met die welke eerder openbaar werden gemaakt ?",
		},
	},
	{
		section: "s.01.00.1",
		metric: "dte1",
		dimensions: {
			"dim:dcl": "dcl:m1",
			"dim:part": "part:m2",
			"dim:mmt": "mmt:m2",
			"dim:bas": "bas:m27",
		},
		label: "Are data related to the preceding period identical to data published before ?",
		labels: {
			de: "Sind die Beträge des vorangegangenen Geschäftsjahres mit den für das Vorjahr veröffentl. identisch ?",
			en: "Are data related to the preceding period identical to data published before ?",
			fr: "Les montants relatifs à l'exercice précédent sont-ils identiques à ceux publiés antérieurement ?",
			nl: "Zijn de bedragen van het vorige boekjaar identiek met die welke eerder openbaar werden gemaakt ?",
		},
	},
	{
		section: "s.01.00.1",
		metric: "bln1",
		dimensions: {
			"dim:dcl": "dcl:m1",
			"dim:bas": "bas:m28",
			"dim:part": "part:m2",
		},
		label: "Are data related to the preceding period identical to data published before ?",
		labels: {
			de: "Sind die Beträge des vorangegangenen Geschäftsjahres mit den für das Vorjahr veröffentl. identisch ?",
			en: "Are data related to the preceding period identical to data published before ?",
			fr: "Les montants relatifs à l'exercice précédent sont-ils identiques à ceux publiés antérieurement ?",
			nl: "Zijn de bedragen van het vorige boekjaar identiek met die welke eerder openbaar werden gemaakt ?",
		},
	},
	{
		section: "s.01.00.1",
		metric: "dte1",
		dimensions: {
			"dim:dcl": "dcl:m2",
			"dim:part": "part:m2",
			"dim:mmt": "mmt:m1",
			"dim:bas": "bas:m27",
		},
		label: "Is this a correction for an annual account already accepted by the NBB for the same exercise ?",
		labels: {
			de: "Handelt es sich um eine Berichtigung eines Jahresabschlusses der schon durch die BNB akzeptiert worden ist ?",
			en: "Is this a correction for an annual account already accepted by the NBB for the same exercise ?",
			fr: "S'agit-il d'un dépôt rectificatif à un compte annuel déjà accepté par la BNB pour le même exercice comptable et dans la même langue ?",
			nl: "Betreft het een verbeterde neerlegging van een reeds door de NBB aanvaarde jaarrekening voor hetzelfde boekjaar en in dezelfde taal ?",
		},
	},
	{
		section: "s.01.00.1",
		metric: "dte1",
		dimensions: {
			"dim:dcl": "dcl:m2",
			"dim:part": "part:m2",
			"dim:mmt": "mmt:m2",
			"dim:bas": "bas:m27",
		},
		label: "Is this a correction for an annual account already accepted by the NBB for the same exercise ?",
		labels: {
			de: "Handelt es sich um eine Berichtigung eines Jahresabschlusses der schon durch die BNB akzeptiert worden ist ?",
			en: "Is this a correction for an annual account already accepted by the NBB for the same exercise ?",
			fr: "S'agit-il d'un dépôt rectificatif à un compte annuel déjà accepté par la BNB pour le même exercice comptable et dans la même langue ?",
			nl: "Betreft het een verbeterde neerlegging van een reeds door de NBB aanvaarde jaarrekening voor hetzelfde boekjaar en in dezelfde taal ?",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.01.00.1",
		metric: "bln1",
		dimensions: {
			"dim:dcl": "dcl:m2",
			"dim:bas": "bas:m28",
			"dim:part": "part:m2",
		},
		label: "Is this a correction for an annual account already accepted by the NBB for the same exercise ?",
		labels: {
			de: "Handelt es sich um eine Berichtigung eines Jahresabschlusses der schon durch die BNB akzeptiert worden ist ?",
			en: "Is this a correction for an annual account already accepted by the NBB for the same exercise ?",
			fr: "S'agit-il d'un dépôt rectificatif à un compte annuel déjà accepté par la BNB pour le même exercice comptable et dans la même langue ?",
			nl: "Betreft het een verbeterde neerlegging van een reeds door de NBB aanvaarde jaarrekening voor hetzelfde boekjaar en in dezelfde taal ?",
		},
	},
	{
		section: "s.01.00.1",
		metric: "dte1",
		dimensions: {
			"dim:dcl": "dcl:m39",
			"dim:part": "part:m2",
			"dim:mmt": "mmt:m1",
			"dim:bas": "bas:m27",
		},
		label: "Is it a company in liquidation ?",
		labels: {
			de: "Ist es ein gesellschaft in Liquidation?",
			en: "Is it a company in liquidation ?",
			fr: "Est-ce une société en liquidation ?",
			nl: "Is het een onderneming in vereffening ?",
		},
	},
	{
		section: "s.01.00.1",
		metric: "dte1",
		dimensions: {
			"dim:dcl": "dcl:m39",
			"dim:part": "part:m2",
			"dim:mmt": "mmt:m2",
			"dim:bas": "bas:m27",
		},
		label: "Is it a company in liquidation ?",
		labels: {
			de: "Ist es ein gesellschaft in Liquidation?",
			en: "Is it a company in liquidation ?",
			fr: "Est-ce une société en liquidation ?",
			nl: "Is het een onderneming in vereffening ?",
		},
	},
	{
		section: "s.01.00.1",
		metric: "bln1",
		dimensions: {
			"dim:dcl": "dcl:m39",
			"dim:bas": "bas:m28",
			"dim:part": "part:m2",
		},
		label: "Is it a company in liquidation ?",
		labels: {
			de: "Ist es ein gesellschaft in Liquidation?",
			en: "Is it a company in liquidation ?",
			fr: "Est-ce une société en liquidation ?",
			nl: "Is het een onderneming in vereffening ?",
		},
	},
	{
		section: "s.01.00.1",
		metric: "bln1",
		dimensions: {
			"dim:bas": "bas:m28",
			"dim:mmt": "mmt:m2",
			"dim:part": "part:m2",
		},
		label: "Period end date of annual account to correct",
		labels: {
			de: "Abschlussdatum der zu korrigierenden Bilanz",
			en: "Period end date of annual account to correct",
			fr: "Date de fin d'exercice du compte annuel à rectifier",
			nl: "Afsluitingsdatum van de jaarrekening die verbeterd dient neergelegd te worden",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.01.00.1",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m35",
			"dim:part": "part:m2",
		},
		label: "Schema of annual account to correct",
		labels: {
			de: "Schema der zu korrigierenden Bilanz",
			en: "Schema of annual account to correct",
			fr: "Schéma du compte annuel à rectifier",
			nl: "Schema van de jaarrekening die verbeterd dient neergelegd te worden",
		},
	},
	{
		section: "s.01.00.1",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m35",
			"dim:part": "part:m2",
			"dim:dcl": "dcl:m2",
		},
		label: "Schema of annual account to correct",
		labels: {
			de: "Schema der zu korrigierenden Bilanz",
			en: "Schema of annual account to correct",
			fr: "Schéma du compte annuel à rectifier",
			nl: "Schema van de jaarrekening die verbeterd dient neergelegd te worden",
		},
	},
	{
		section: "s.01.00.1",
		metric: "list1",
		metricPrefix: "lge-enum",
		dimensions: {
			"dim:bas": "bas:m36",
			"dim:part": "part:m2",
		},
		label: "Language code of annual account to correct",
		labels: {
			de: "Sprachkode der zu korrigierenden Bilanz",
			en: "Language code of annual account to correct",
			fr: "Code langue du compte annuel à rectifier",
			nl: "Taalcode van de jaarrekening die verbeterd dient neergelegd te worden",
		},
	},
	{
		section: "s.01.00.1",
		metric: "list1",
		metricPrefix: "lge-enum",
		dimensions: {
			"dim:bas": "bas:m36",
			"dim:part": "part:m2",
			"dim:dcl": "dcl:m2",
		},
		label: "Language code of annual account to correct",
		labels: {
			de: "Sprachkode der zu korrigierenden Bilanz",
			en: "Language code of annual account to correct",
			fr: "Code langue du compte annuel à rectifier",
			nl: "Taalcode van de jaarrekening die verbeterd dient neergelegd te worden",
		},
	},
	{
		section: "s.01.00.2",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m29",
			"dim:psn": "psn:m1",
			"dim:part": "part:m2",
		},
		label: "Name",
		labels: {
			de: "Name",
			en: "Name",
			fr: "Dénomination",
			nl: "Naam",
		},
	},
	{
		section: "s.01.00.2",
		metric: "list2",
		metricPrefix: "lgf-enum",
		dimensions: {
			"dim:bas": "bas:m30",
			"dim:psn": "psn:m1",
			"dim:part": "part:m2",
		},
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.01.00.2",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m30",
			"dim:psn": "psn:m1",
			"dim:part": "part:m2",
		},
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.01.00.2",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:psn": "psn:m1",
			"dim:ctc": "ctc:m1",
			"dim:part": "part:m2",
		},
		label: "Street",
		labels: {
			de: "Straβe",
			en: "Street",
			fr: "Rue",
			nl: "Straat",
		},
	},
	{
		section: "s.01.00.2",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:psn": "psn:m1",
			"dim:ctc": "ctc:m2",
			"dim:part": "part:m2",
		},
		label: "N°",
		labels: {
			de: "Nr",
			en: "N°",
			fr: "N°",
			nl: "Nr",
		},
	},
	{
		section: "s.01.00.2",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:psn": "psn:m1",
			"dim:ctc": "ctc:m3",
			"dim:part": "part:m2",
		},
		label: "Box",
		labels: {
			de: "Briefkasten",
			en: "Box",
			fr: "Boîte",
			nl: "Bus",
		},
	},
	{
		section: "s.01.00.2",
		metric: "list1",
		metricPrefix: "pcd-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:psn": "psn:m1",
			"dim:ctc": "ctc:m4",
			"dim:part": "part:m2",
		},
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.01.00.2",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:psn": "psn:m1",
			"dim:ctc": "ctc:m4",
			"dim:part": "part:m2",
		},
		label: "Postal code - other",
		labels: {
			de: "Postleitzahl - Sonstiges",
			en: "Postal code - other",
			fr: "Code postal - autre",
			nl: "Postcode - andere",
		},
	},
	{
		section: "s.01.00.2",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:psn": "psn:m1",
			"dim:ctc": "ctc:m5",
			"dim:part": "part:m2",
		},
		label: "Town - other",
		labels: {
			de: "Gemeinde - Sonstiges",
			en: "Town - other",
			fr: "Commune - autre",
			nl: "Gemeente - andere",
		},
	},
	{
		section: "s.01.00.2",
		metric: "list1",
		metricPrefix: "cty-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:psn": "psn:m1",
			"dim:ctc": "ctc:m6",
			"dim:part": "part:m2",
		},
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.01.00.2",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:psn": "psn:m1",
			"dim:ctc": "ctc:m6",
			"dim:part": "part:m2",
		},
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.01.00.2",
		metric: "list1",
		metricPrefix: "cct-enum",
		dimensions: {
			"dim:bas": "bas:m32",
			"dim:part": "part:m2",
		},
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.01.00.2",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m32",
			"dim:part": "part:m2",
		},
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.01.00.2",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m27",
			"dim:evt": "evt:m2",
			"dim:part": "part:m2",
		},
		label: "Date of filing the most recent document mentioning the date of publication of the deed of incorporation and of the deed of amendment of the articles of association",
		labels: {
			de: "Datum der Hinterlegung der letzten Urkunde, die das Veröffentlichungsdatum der Errichtungs- bzw. Änderungsurkunde der Satzungen anführt.",
			en: "Date of filing the most recent document mentioning the date of publication of the deed of incorporation and of the deed of amendment of the articles of association",
			fr: "Date de dépôt de l'acte constitutif ou du document le plus récent mentionnant la date de publication des actes constitutif et modificatif(s) des statuts.",
			nl: "DATUM van de neerlegging van het recentste stuk dat de datum van bekendmaking van de oprichtingsakte en van de akte tot statutenwijziging vermeldt.",
		},
	},
	{
		section: "s.01.00.2",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:psn": "psn:m1",
			"dim:ctc": "ctc:m7",
			"dim:part": "part:m2",
		},
		label: "E-mail address",
		labels: {
			de: "E-Mail-Adresse",
			en: "E-mail address",
			fr: "Adresse E-mail",
			nl: "E-mail adres",
		},
	},
	{
		section: "s.01.00.2",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:psn": "psn:m21",
			"dim:ctc": "ctc:m8",
			"dim:part": "part:m2",
		},
		label: "Entity website",
		labels: {
			de: "Website",
			en: "Entity website",
			fr: "Adresse Internet",
			nl: "Website",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "nmt-enum",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:dcl": "dcl:m7",
			"dim:psn": "psn:m10",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:dcl": "dcl:m6",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:dcl": "dcl:m6",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:dcl": "dcl:m7",
			"dim:mmt": "mmt:m1",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:dcl": "dcl:m7",
			"dim:mmt": "mmt:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "nmt-enum",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:dcl": "dcl:m7",
			"dim:part": "part:m2",
			"dim:psn": "psn:m12",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:dcl": "dcl:m7",
			"dim:mmt": "mmt:m1",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:dcl": "dcl:m7",
			"dim:mmt": "mmt:m2",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "nmt-enum",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:dcl": "dcl:m7",
			"dim:part": "part:m2",
			"dim:psn": "psn:m13",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "nmt-enum",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:dcl": "dcl:m7",
			"dim:part": "part:m2",
			"dim:psn": "psn:m11",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "nmt-enum",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:dcl": "dcl:m7",
			"dim:part": "part:m2",
			"dim:psn": "psn:m14",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "nmt-enum",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:dcl": "dcl:m7",
			"dim:part": "part:m2",
			"dim:psn": "psn:m15",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:dcl": "dcl:m7",
			"dim:mmt": "mmt:m1",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:dcl": "dcl:m7",
			"dim:mmt": "mmt:m2",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:dcl": "dcl:m7",
			"dim:psn": "psn:m10",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:dcl": "dcl:m7",
			"dim:part": "part:m2",
			"dim:psn": "psn:m12",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:dcl": "dcl:m7",
			"dim:part": "part:m2",
			"dim:psn": "psn:m13",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:dcl": "dcl:m7",
			"dim:part": "part:m2",
			"dim:psn": "psn:m11",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:dcl": "dcl:m7",
			"dim:part": "part:m2",
			"dim:psn": "psn:m14",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:dcl": "dcl:m7",
			"dim:part": "part:m2",
			"dim:psn": "psn:m15",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:qlt": "qlt:m1",
			"dim:psn": "psn:m10",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Value",
		labels: {
			de: "Angabe",
			en: "Value",
			fr: "Valeur",
			nl: "Opgave nummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:qlt": "qlt:m1",
			"dim:dcl": "dcl:m6",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Value",
		labels: {
			de: "Angabe",
			en: "Value",
			fr: "Valeur",
			nl: "Opgave nummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:qlt": "qlt:m1",
			"dim:dcl": "dcl:m6",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Value",
		labels: {
			de: "Angabe",
			en: "Value",
			fr: "Valeur",
			nl: "Opgave nummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:qlt": "qlt:m1",
			"dim:mmt": "mmt:m1",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Value",
		labels: {
			de: "Angabe",
			en: "Value",
			fr: "Valeur",
			nl: "Opgave nummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:qlt": "qlt:m1",
			"dim:mmt": "mmt:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Value",
		labels: {
			de: "Angabe",
			en: "Value",
			fr: "Valeur",
			nl: "Opgave nummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:qlt": "qlt:m1",
			"dim:part": "part:m2",
			"dim:psn": "psn:m12",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Value",
		labels: {
			de: "Angabe",
			en: "Value",
			fr: "Valeur",
			nl: "Opgave nummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:qlt": "qlt:m1",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Value",
		labels: {
			de: "Angabe",
			en: "Value",
			fr: "Valeur",
			nl: "Opgave nummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:qlt": "qlt:m1",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Value",
		labels: {
			de: "Angabe",
			en: "Value",
			fr: "Valeur",
			nl: "Opgave nummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:qlt": "qlt:m1",
			"dim:mmt": "mmt:m1",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Value",
		labels: {
			de: "Angabe",
			en: "Value",
			fr: "Valeur",
			nl: "Opgave nummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:qlt": "qlt:m1",
			"dim:mmt": "mmt:m2",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Value",
		labels: {
			de: "Angabe",
			en: "Value",
			fr: "Valeur",
			nl: "Opgave nummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:qlt": "qlt:m1",
			"dim:part": "part:m2",
			"dim:psn": "psn:m13",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Value",
		labels: {
			de: "Angabe",
			en: "Value",
			fr: "Valeur",
			nl: "Opgave nummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:qlt": "qlt:m1",
			"dim:part": "part:m2",
			"dim:psn": "psn:m11",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Value",
		labels: {
			de: "Angabe",
			en: "Value",
			fr: "Valeur",
			nl: "Opgave nummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:qlt": "qlt:m1",
			"dim:part": "part:m2",
			"dim:psn": "psn:m14",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Value",
		labels: {
			de: "Angabe",
			en: "Value",
			fr: "Valeur",
			nl: "Opgave nummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m26",
			"dim:qlt": "qlt:m1",
			"dim:part": "part:m2",
			"dim:psn": "psn:m15",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Value",
		labels: {
			de: "Angabe",
			en: "Value",
			fr: "Valeur",
			nl: "Opgave nummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:qlt": "qlt:m1",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Value",
		labels: {
			de: "Angabe",
			en: "Value",
			fr: "Valeur",
			nl: "Opgave nummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:qlt": "qlt:m1",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Value",
		labels: {
			de: "Angabe",
			en: "Value",
			fr: "Valeur",
			nl: "Opgave nummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:qlt": "qlt:m1",
			"dim:mmt": "mmt:m1",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Value",
		labels: {
			de: "Angabe",
			en: "Value",
			fr: "Valeur",
			nl: "Opgave nummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:qlt": "qlt:m1",
			"dim:mmt": "mmt:m2",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Value",
		labels: {
			de: "Angabe",
			en: "Value",
			fr: "Valeur",
			nl: "Opgave nummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "atc-npi-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:dcl": "dcl:m7",
			"dim:psn": "psn:m10",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:dcl": "dcl:m6",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:dcl": "dcl:m6",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:dcl": "dcl:m7",
			"dim:mmt": "mmt:m1",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:dcl": "dcl:m7",
			"dim:mmt": "mmt:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "atc-npi-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:dcl": "dcl:m7",
			"dim:part": "part:m2",
			"dim:psn": "psn:m12",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "atc-npi-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:dcl": "dcl:m7",
			"dim:part": "part:m2",
			"dim:psn": "psn:m13",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "atc-npi-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:dcl": "dcl:m7",
			"dim:part": "part:m2",
			"dim:psn": "psn:m11",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "atc-npi-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:dcl": "dcl:m7",
			"dim:part": "part:m2",
			"dim:psn": "psn:m14",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "atc-npi-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:dcl": "dcl:m7",
			"dim:part": "part:m2",
			"dim:psn": "psn:m15",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:dcl": "dcl:m7",
			"dim:psn": "psn:m10",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:dcl": "dcl:m7",
			"dim:part": "part:m2",
			"dim:psn": "psn:m12",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:dcl": "dcl:m7",
			"dim:part": "part:m2",
			"dim:psn": "psn:m13",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:dcl": "dcl:m7",
			"dim:part": "part:m2",
			"dim:psn": "psn:m11",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:dcl": "dcl:m7",
			"dim:part": "part:m2",
			"dim:psn": "psn:m14",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:dcl": "dcl:m7",
			"dim:part": "part:m2",
			"dim:psn": "psn:m15",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m1",
			"dim:psn": "psn:m10",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Street",
		labels: {
			de: "Straβe",
			en: "Street",
			fr: "Rue",
			nl: "Straat",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m1",
			"dim:dcl": "dcl:m6",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Street",
		labels: {
			de: "Straβe",
			en: "Street",
			fr: "Rue",
			nl: "Straat",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m1",
			"dim:dcl": "dcl:m6",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Street",
		labels: {
			de: "Straβe",
			en: "Street",
			fr: "Rue",
			nl: "Straat",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m1",
			"dim:mmt": "mmt:m1",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Street",
		labels: {
			de: "Straβe",
			en: "Street",
			fr: "Rue",
			nl: "Straat",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m1",
			"dim:mmt": "mmt:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Street",
		labels: {
			de: "Straβe",
			en: "Street",
			fr: "Rue",
			nl: "Straat",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m1",
			"dim:part": "part:m2",
			"dim:psn": "psn:m12",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Street",
		labels: {
			de: "Straβe",
			en: "Street",
			fr: "Rue",
			nl: "Straat",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m1",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Street",
		labels: {
			de: "Straβe",
			en: "Street",
			fr: "Rue",
			nl: "Straat",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m1",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Street",
		labels: {
			de: "Straβe",
			en: "Street",
			fr: "Rue",
			nl: "Straat",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m1",
			"dim:mmt": "mmt:m1",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Street",
		labels: {
			de: "Straβe",
			en: "Street",
			fr: "Rue",
			nl: "Straat",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m1",
			"dim:mmt": "mmt:m2",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Street",
		labels: {
			de: "Straβe",
			en: "Street",
			fr: "Rue",
			nl: "Straat",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m1",
			"dim:part": "part:m2",
			"dim:psn": "psn:m13",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Street",
		labels: {
			de: "Straβe",
			en: "Street",
			fr: "Rue",
			nl: "Straat",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m1",
			"dim:part": "part:m2",
			"dim:psn": "psn:m11",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Street",
		labels: {
			de: "Straβe",
			en: "Street",
			fr: "Rue",
			nl: "Straat",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m1",
			"dim:part": "part:m2",
			"dim:psn": "psn:m14",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Street",
		labels: {
			de: "Straβe",
			en: "Street",
			fr: "Rue",
			nl: "Straat",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m1",
			"dim:part": "part:m2",
			"dim:psn": "psn:m15",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Street",
		labels: {
			de: "Straβe",
			en: "Street",
			fr: "Rue",
			nl: "Straat",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m1",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Street",
		labels: {
			de: "Straβe",
			en: "Street",
			fr: "Rue",
			nl: "Straat",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m1",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Street",
		labels: {
			de: "Straβe",
			en: "Street",
			fr: "Rue",
			nl: "Straat",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m1",
			"dim:mmt": "mmt:m1",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Street",
		labels: {
			de: "Straβe",
			en: "Street",
			fr: "Rue",
			nl: "Straat",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m1",
			"dim:mmt": "mmt:m2",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Street",
		labels: {
			de: "Straβe",
			en: "Street",
			fr: "Rue",
			nl: "Straat",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m2",
			"dim:psn": "psn:m10",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Nr",
		labels: {
			de: "Nr",
			en: "Nr",
			fr: "N°",
			nl: "Nr",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m2",
			"dim:dcl": "dcl:m6",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Nr",
		labels: {
			de: "Nr",
			en: "Nr",
			fr: "N°",
			nl: "Nr",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m2",
			"dim:dcl": "dcl:m6",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Nr",
		labels: {
			de: "Nr",
			en: "Nr",
			fr: "N°",
			nl: "Nr",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m2",
			"dim:mmt": "mmt:m1",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Nr",
		labels: {
			de: "Nr",
			en: "Nr",
			fr: "N°",
			nl: "Nr",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m2",
			"dim:mmt": "mmt:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Nr",
		labels: {
			de: "Nr",
			en: "Nr",
			fr: "N°",
			nl: "Nr",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m2",
			"dim:part": "part:m2",
			"dim:psn": "psn:m12",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Nr",
		labels: {
			de: "Nr",
			en: "Nr",
			fr: "N°",
			nl: "Nr",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m2",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Nr",
		labels: {
			de: "Nr",
			en: "Nr",
			fr: "N°",
			nl: "Nr",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m2",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Nr",
		labels: {
			de: "Nr",
			en: "Nr",
			fr: "N°",
			nl: "Nr",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m2",
			"dim:mmt": "mmt:m1",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Nr",
		labels: {
			de: "Nr",
			en: "Nr",
			fr: "N°",
			nl: "Nr",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m2",
			"dim:mmt": "mmt:m2",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Nr",
		labels: {
			de: "Nr",
			en: "Nr",
			fr: "N°",
			nl: "Nr",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m2",
			"dim:part": "part:m2",
			"dim:psn": "psn:m13",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Nr",
		labels: {
			de: "Nr",
			en: "Nr",
			fr: "N°",
			nl: "Nr",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m2",
			"dim:part": "part:m2",
			"dim:psn": "psn:m11",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Nr",
		labels: {
			de: "Nr",
			en: "Nr",
			fr: "N°",
			nl: "Nr",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m2",
			"dim:part": "part:m2",
			"dim:psn": "psn:m14",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Nr",
		labels: {
			de: "Nr",
			en: "Nr",
			fr: "N°",
			nl: "Nr",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m2",
			"dim:part": "part:m2",
			"dim:psn": "psn:m15",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Nr",
		labels: {
			de: "Nr",
			en: "Nr",
			fr: "N°",
			nl: "Nr",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m2",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Nr",
		labels: {
			de: "Nr",
			en: "Nr",
			fr: "N°",
			nl: "Nr",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m2",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Nr",
		labels: {
			de: "Nr",
			en: "Nr",
			fr: "N°",
			nl: "Nr",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m2",
			"dim:mmt": "mmt:m1",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Nr",
		labels: {
			de: "Nr",
			en: "Nr",
			fr: "N°",
			nl: "Nr",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m2",
			"dim:mmt": "mmt:m2",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Nr",
		labels: {
			de: "Nr",
			en: "Nr",
			fr: "N°",
			nl: "Nr",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m3",
			"dim:psn": "psn:m10",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Box",
		labels: {
			de: "Briefkasten",
			en: "Box",
			fr: "Boîte",
			nl: "Bus",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m3",
			"dim:dcl": "dcl:m6",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Box",
		labels: {
			de: "Briefkasten",
			en: "Box",
			fr: "Boîte",
			nl: "Bus",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m3",
			"dim:dcl": "dcl:m6",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Box",
		labels: {
			de: "Briefkasten",
			en: "Box",
			fr: "Boîte",
			nl: "Bus",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m3",
			"dim:mmt": "mmt:m1",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Box",
		labels: {
			de: "Briefkasten",
			en: "Box",
			fr: "Boîte",
			nl: "Bus",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m3",
			"dim:mmt": "mmt:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Box",
		labels: {
			de: "Briefkasten",
			en: "Box",
			fr: "Boîte",
			nl: "Bus",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m3",
			"dim:part": "part:m2",
			"dim:psn": "psn:m12",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Box",
		labels: {
			de: "Briefkasten",
			en: "Box",
			fr: "Boîte",
			nl: "Bus",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m3",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Box",
		labels: {
			de: "Briefkasten",
			en: "Box",
			fr: "Boîte",
			nl: "Bus",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m3",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Box",
		labels: {
			de: "Briefkasten",
			en: "Box",
			fr: "Boîte",
			nl: "Bus",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m3",
			"dim:mmt": "mmt:m1",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Box",
		labels: {
			de: "Briefkasten",
			en: "Box",
			fr: "Boîte",
			nl: "Bus",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m3",
			"dim:mmt": "mmt:m2",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Box",
		labels: {
			de: "Briefkasten",
			en: "Box",
			fr: "Boîte",
			nl: "Bus",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m3",
			"dim:part": "part:m2",
			"dim:psn": "psn:m13",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Box",
		labels: {
			de: "Briefkasten",
			en: "Box",
			fr: "Boîte",
			nl: "Bus",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m3",
			"dim:part": "part:m2",
			"dim:psn": "psn:m11",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Box",
		labels: {
			de: "Briefkasten",
			en: "Box",
			fr: "Boîte",
			nl: "Bus",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m3",
			"dim:part": "part:m2",
			"dim:psn": "psn:m14",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Box",
		labels: {
			de: "Briefkasten",
			en: "Box",
			fr: "Boîte",
			nl: "Bus",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m3",
			"dim:part": "part:m2",
			"dim:psn": "psn:m15",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Box",
		labels: {
			de: "Briefkasten",
			en: "Box",
			fr: "Boîte",
			nl: "Bus",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m3",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Box",
		labels: {
			de: "Briefkasten",
			en: "Box",
			fr: "Boîte",
			nl: "Bus",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m3",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Box",
		labels: {
			de: "Briefkasten",
			en: "Box",
			fr: "Boîte",
			nl: "Bus",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m3",
			"dim:mmt": "mmt:m1",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Box",
		labels: {
			de: "Briefkasten",
			en: "Box",
			fr: "Boîte",
			nl: "Bus",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m3",
			"dim:mmt": "mmt:m2",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Box",
		labels: {
			de: "Briefkasten",
			en: "Box",
			fr: "Boîte",
			nl: "Bus",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "pcd-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m4",
			"dim:psn": "psn:m10",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m4",
			"dim:dcl": "dcl:m6",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m4",
			"dim:dcl": "dcl:m6",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m4",
			"dim:mmt": "mmt:m1",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m4",
			"dim:mmt": "mmt:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "pcd-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m4",
			"dim:part": "part:m2",
			"dim:psn": "psn:m12",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m4",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m4",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m4",
			"dim:mmt": "mmt:m1",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m4",
			"dim:mmt": "mmt:m2",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "pcd-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m4",
			"dim:part": "part:m2",
			"dim:psn": "psn:m13",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "pcd-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m4",
			"dim:part": "part:m2",
			"dim:psn": "psn:m11",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "pcd-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m4",
			"dim:part": "part:m2",
			"dim:psn": "psn:m14",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "pcd-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m4",
			"dim:part": "part:m2",
			"dim:psn": "psn:m15",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m4",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m4",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m4",
			"dim:mmt": "mmt:m1",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m4",
			"dim:mmt": "mmt:m2",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m4",
			"dim:psn": "psn:m10",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Postal code - other",
		labels: {
			de: "Postleitzahl - Sonstiges",
			en: "Postal code - other",
			fr: "Code postal - autre",
			nl: "Postcode - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m4",
			"dim:part": "part:m2",
			"dim:psn": "psn:m12",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Postal code - other",
		labels: {
			de: "Postleitzahl - Sonstiges",
			en: "Postal code - other",
			fr: "Code postal - autre",
			nl: "Postcode - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m4",
			"dim:part": "part:m2",
			"dim:psn": "psn:m13",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Postal code - other",
		labels: {
			de: "Postleitzahl - Sonstiges",
			en: "Postal code - other",
			fr: "Code postal - autre",
			nl: "Postcode - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m4",
			"dim:part": "part:m2",
			"dim:psn": "psn:m11",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Postal code - other",
		labels: {
			de: "Postleitzahl - Sonstiges",
			en: "Postal code - other",
			fr: "Code postal - autre",
			nl: "Postcode - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m4",
			"dim:part": "part:m2",
			"dim:psn": "psn:m14",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Postal code - other",
		labels: {
			de: "Postleitzahl - Sonstiges",
			en: "Postal code - other",
			fr: "Code postal - autre",
			nl: "Postcode - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m4",
			"dim:part": "part:m2",
			"dim:psn": "psn:m15",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Postal code - other",
		labels: {
			de: "Postleitzahl - Sonstiges",
			en: "Postal code - other",
			fr: "Code postal - autre",
			nl: "Postcode - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m5",
			"dim:psn": "psn:m10",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "City - other",
		labels: {
			de: "Gemeinde - Sonstiges",
			en: "City - other",
			fr: "Commune - autre",
			nl: "Gemeente - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m5",
			"dim:dcl": "dcl:m6",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "City - other",
		labels: {
			de: "Gemeinde - Sonstiges",
			en: "City - other",
			fr: "Commune - autre",
			nl: "Gemeente - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m5",
			"dim:dcl": "dcl:m6",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "City - other",
		labels: {
			de: "Gemeinde - Sonstiges",
			en: "City - other",
			fr: "Commune - autre",
			nl: "Gemeente - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m5",
			"dim:mmt": "mmt:m1",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "City - other",
		labels: {
			de: "Gemeinde - Sonstiges",
			en: "City - other",
			fr: "Commune - autre",
			nl: "Gemeente - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m5",
			"dim:mmt": "mmt:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "City - other",
		labels: {
			de: "Gemeinde - Sonstiges",
			en: "City - other",
			fr: "Commune - autre",
			nl: "Gemeente - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m5",
			"dim:part": "part:m2",
			"dim:psn": "psn:m12",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "City - other",
		labels: {
			de: "Gemeinde - Sonstiges",
			en: "City - other",
			fr: "Commune - autre",
			nl: "Gemeente - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m5",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "City - other",
		labels: {
			de: "Gemeinde - Sonstiges",
			en: "City - other",
			fr: "Commune - autre",
			nl: "Gemeente - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m5",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "City - other",
		labels: {
			de: "Gemeinde - Sonstiges",
			en: "City - other",
			fr: "Commune - autre",
			nl: "Gemeente - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m5",
			"dim:mmt": "mmt:m1",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "City - other",
		labels: {
			de: "Gemeinde - Sonstiges",
			en: "City - other",
			fr: "Commune - autre",
			nl: "Gemeente - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m5",
			"dim:mmt": "mmt:m2",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "City - other",
		labels: {
			de: "Gemeinde - Sonstiges",
			en: "City - other",
			fr: "Commune - autre",
			nl: "Gemeente - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m5",
			"dim:part": "part:m2",
			"dim:psn": "psn:m13",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "City - other",
		labels: {
			de: "Gemeinde - Sonstiges",
			en: "City - other",
			fr: "Commune - autre",
			nl: "Gemeente - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m5",
			"dim:part": "part:m2",
			"dim:psn": "psn:m11",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "City - other",
		labels: {
			de: "Gemeinde - Sonstiges",
			en: "City - other",
			fr: "Commune - autre",
			nl: "Gemeente - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m5",
			"dim:part": "part:m2",
			"dim:psn": "psn:m14",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "City - other",
		labels: {
			de: "Gemeinde - Sonstiges",
			en: "City - other",
			fr: "Commune - autre",
			nl: "Gemeente - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m5",
			"dim:part": "part:m2",
			"dim:psn": "psn:m15",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "City - other",
		labels: {
			de: "Gemeinde - Sonstiges",
			en: "City - other",
			fr: "Commune - autre",
			nl: "Gemeente - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m5",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "City - other",
		labels: {
			de: "Gemeinde - Sonstiges",
			en: "City - other",
			fr: "Commune - autre",
			nl: "Gemeente - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m5",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "City - other",
		labels: {
			de: "Gemeinde - Sonstiges",
			en: "City - other",
			fr: "Commune - autre",
			nl: "Gemeente - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m5",
			"dim:mmt": "mmt:m1",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "City - other",
		labels: {
			de: "Gemeinde - Sonstiges",
			en: "City - other",
			fr: "Commune - autre",
			nl: "Gemeente - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m5",
			"dim:mmt": "mmt:m2",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "City - other",
		labels: {
			de: "Gemeinde - Sonstiges",
			en: "City - other",
			fr: "Commune - autre",
			nl: "Gemeente - andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "cty-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m6",
			"dim:psn": "psn:m10",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m6",
			"dim:dcl": "dcl:m6",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m6",
			"dim:dcl": "dcl:m6",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m6",
			"dim:mmt": "mmt:m1",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m6",
			"dim:mmt": "mmt:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "cty-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m6",
			"dim:part": "part:m2",
			"dim:psn": "psn:m12",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m6",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m6",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m6",
			"dim:mmt": "mmt:m1",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m6",
			"dim:mmt": "mmt:m2",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "cty-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m6",
			"dim:part": "part:m2",
			"dim:psn": "psn:m13",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "cty-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m6",
			"dim:part": "part:m2",
			"dim:psn": "psn:m11",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "cty-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m6",
			"dim:part": "part:m2",
			"dim:psn": "psn:m14",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list1",
		metricPrefix: "cty-enum",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m6",
			"dim:part": "part:m2",
			"dim:psn": "psn:m15",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m6",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m6",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m6",
			"dim:mmt": "mmt:m1",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:ctc": "ctc:m6",
			"dim:mmt": "mmt:m2",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "List",
		labels: {
			de: "Liste",
			en: "List",
			fr: "Liste",
			nl: "Lijst",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m6",
			"dim:psn": "psn:m10",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m6",
			"dim:part": "part:m2",
			"dim:psn": "psn:m12",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m6",
			"dim:part": "part:m2",
			"dim:psn": "psn:m13",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m6",
			"dim:part": "part:m2",
			"dim:psn": "psn:m11",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m6",
			"dim:part": "part:m2",
			"dim:psn": "psn:m14",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m31",
			"dim:ctc": "ctc:m6",
			"dim:part": "part:m2",
			"dim:psn": "psn:m15",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Other",
		labels: {
			de: "Sonstiges",
			en: "Other",
			fr: "Autre",
			nl: "Andere",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:dcl": "dcl:m27",
			"dim:bas": "bas:m28",
			"dim:psn": "psn:m10",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Profession",
		labels: {
			de: "Beruf",
			en: "Profession",
			fr: "Profession",
			nl: "Beroep",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:dcl": "dcl:m6",
			"dim:bas": "bas:m28",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Profession",
		labels: {
			de: "Beruf",
			en: "Profession",
			fr: "Profession",
			nl: "Beroep",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:dcl": "dcl:m6",
			"dim:bas": "bas:m28",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Profession",
		labels: {
			de: "Beruf",
			en: "Profession",
			fr: "Profession",
			nl: "Beroep",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:dcl": "dcl:m27",
			"dim:bas": "bas:m28",
			"dim:mmt": "mmt:m1",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Profession",
		labels: {
			de: "Beruf",
			en: "Profession",
			fr: "Profession",
			nl: "Beroep",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:dcl": "dcl:m27",
			"dim:bas": "bas:m28",
			"dim:mmt": "mmt:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Profession",
		labels: {
			de: "Beruf",
			en: "Profession",
			fr: "Profession",
			nl: "Beroep",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:dcl": "dcl:m27",
			"dim:bas": "bas:m28",
			"dim:part": "part:m2",
			"dim:psn": "psn:m12",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Profession",
		labels: {
			de: "Beruf",
			en: "Profession",
			fr: "Profession",
			nl: "Beroep",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:dcl": "dcl:m27",
			"dim:bas": "bas:m115",
			"dim:mmt": "mmt:m1",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Profession",
		labels: {
			de: "Beruf",
			en: "Profession",
			fr: "Profession",
			nl: "Beroep",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:dcl": "dcl:m27",
			"dim:bas": "bas:m115",
			"dim:mmt": "mmt:m2",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Profession",
		labels: {
			de: "Beruf",
			en: "Profession",
			fr: "Profession",
			nl: "Beroep",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:dcl": "dcl:m27",
			"dim:bas": "bas:m28",
			"dim:part": "part:m2",
			"dim:psn": "psn:m13",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Profession",
		labels: {
			de: "Beruf",
			en: "Profession",
			fr: "Profession",
			nl: "Beroep",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:dcl": "dcl:m27",
			"dim:bas": "bas:m28",
			"dim:part": "part:m2",
			"dim:psn": "psn:m11",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Profession",
		labels: {
			de: "Beruf",
			en: "Profession",
			fr: "Profession",
			nl: "Beroep",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:dcl": "dcl:m27",
			"dim:bas": "bas:m28",
			"dim:part": "part:m2",
			"dim:psn": "psn:m14",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Profession",
		labels: {
			de: "Beruf",
			en: "Profession",
			fr: "Profession",
			nl: "Beroep",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:dcl": "dcl:m27",
			"dim:bas": "bas:m28",
			"dim:part": "part:m2",
			"dim:psn": "psn:m15",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Profession",
		labels: {
			de: "Beruf",
			en: "Profession",
			fr: "Profession",
			nl: "Beroep",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:dcl": "dcl:m27",
			"dim:bas": "bas:m115",
			"dim:mmt": "mmt:m1",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Profession",
		labels: {
			de: "Beruf",
			en: "Profession",
			fr: "Profession",
			nl: "Beroep",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:dcl": "dcl:m27",
			"dim:bas": "bas:m115",
			"dim:mmt": "mmt:m2",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Profession",
		labels: {
			de: "Beruf",
			en: "Profession",
			fr: "Profession",
			nl: "Beroep",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m34",
			"dim:qlt": "qlt:m5",
			"dim:psn": "psn:m10",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Number",
		labels: {
			de: "Mitgliedsnummer",
			en: "Number",
			fr: "Numéro de membre",
			nl: "Lidmaatschapsnummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m34",
			"dim:qlt": "qlt:m5",
			"dim:dcl": "dcl:m6",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Number",
		labels: {
			de: "Mitgliedsnummer",
			en: "Number",
			fr: "Numéro de membre",
			nl: "Lidmaatschapsnummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m34",
			"dim:qlt": "qlt:m5",
			"dim:dcl": "dcl:m6",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Number",
		labels: {
			de: "Mitgliedsnummer",
			en: "Number",
			fr: "Numéro de membre",
			nl: "Lidmaatschapsnummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m34",
			"dim:qlt": "qlt:m5",
			"dim:mmt": "mmt:m1",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Number",
		labels: {
			de: "Mitgliedsnummer",
			en: "Number",
			fr: "Numéro de membre",
			nl: "Lidmaatschapsnummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m34",
			"dim:qlt": "qlt:m5",
			"dim:mmt": "mmt:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Number",
		labels: {
			de: "Mitgliedsnummer",
			en: "Number",
			fr: "Numéro de membre",
			nl: "Lidmaatschapsnummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m34",
			"dim:qlt": "qlt:m5",
			"dim:part": "part:m2",
			"dim:psn": "psn:m12",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Number",
		labels: {
			de: "Mitgliedsnummer",
			en: "Number",
			fr: "Numéro de membre",
			nl: "Lidmaatschapsnummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:qlt": "qlt:m5",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Number",
		labels: {
			de: "Mitgliedsnummer",
			en: "Number",
			fr: "Numéro de membre",
			nl: "Lidmaatschapsnummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:qlt": "qlt:m5",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Number",
		labels: {
			de: "Mitgliedsnummer",
			en: "Number",
			fr: "Numéro de membre",
			nl: "Lidmaatschapsnummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:qlt": "qlt:m5",
			"dim:mmt": "mmt:m1",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Number",
		labels: {
			de: "Mitgliedsnummer",
			en: "Number",
			fr: "Numéro de membre",
			nl: "Lidmaatschapsnummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:qlt": "qlt:m5",
			"dim:mmt": "mmt:m2",
			"dim:psn": "psn:m12",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Number",
		labels: {
			de: "Mitgliedsnummer",
			en: "Number",
			fr: "Numéro de membre",
			nl: "Lidmaatschapsnummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m34",
			"dim:qlt": "qlt:m5",
			"dim:part": "part:m2",
			"dim:psn": "psn:m13",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Number",
		labels: {
			de: "Mitgliedsnummer",
			en: "Number",
			fr: "Numéro de membre",
			nl: "Lidmaatschapsnummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m34",
			"dim:qlt": "qlt:m5",
			"dim:part": "part:m2",
			"dim:psn": "psn:m11",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Number",
		labels: {
			de: "Mitgliedsnummer",
			en: "Number",
			fr: "Numéro de membre",
			nl: "Lidmaatschapsnummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m34",
			"dim:qlt": "qlt:m5",
			"dim:part": "part:m2",
			"dim:psn": "psn:m14",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Number",
		labels: {
			de: "Mitgliedsnummer",
			en: "Number",
			fr: "Numéro de membre",
			nl: "Lidmaatschapsnummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m34",
			"dim:qlt": "qlt:m5",
			"dim:part": "part:m2",
			"dim:psn": "psn:m15",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Number",
		labels: {
			de: "Mitgliedsnummer",
			en: "Number",
			fr: "Numéro de membre",
			nl: "Lidmaatschapsnummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "list2",
		metricPrefix: "fct-enum",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:qlt": "qlt:m5",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Number",
		labels: {
			de: "Mitgliedsnummer",
			en: "Number",
			fr: "Numéro de membre",
			nl: "Lidmaatschapsnummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:qlt": "qlt:m5",
			"dim:dcl": "dcl:m6",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Number",
		labels: {
			de: "Mitgliedsnummer",
			en: "Number",
			fr: "Numéro de membre",
			nl: "Lidmaatschapsnummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:qlt": "qlt:m5",
			"dim:mmt": "mmt:m1",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Number",
		labels: {
			de: "Mitgliedsnummer",
			en: "Number",
			fr: "Numéro de membre",
			nl: "Lidmaatschapsnummer",
		},
	},
	{
		section: "s.02.01.0.ghi",
		metric: "dte1",
		dimensions: {
			"dim:bas": "bas:m115",
			"dim:qlt": "qlt:m5",
			"dim:mmt": "mmt:m2",
			"dim:psn": "psn:m15",
			"dim:part": "part:m2",
		},
		openDimensions: [
			"dim:acfn",
			"dim:aclp",
			"dim:acnp",
			"dim:afnp",
			"dim:anlp",
			"dim:annp",
			"dim:aprf",
			"dim:aprn",
			"dim:cprf",
			"dim:cprn",
			"dim:open",
		],
		label: "Number",
		labels: {
			de: "Mitgliedsnummer",
			en: "Number",
			fr: "Numéro de membre",
			nl: "Lidmaatschapsnummer",
		},
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m1",
			"dim:spu": "spu:m1",
			"dim:part": "part:m1",
		},
		code: "20",
		label: "FORMATION EXPENSES",
		labels: {
			de: "ERRICHTUNGS- UND ERWEITERUNGSAUFWENDUNGEN",
			en: "FORMATION EXPENSES",
			fr: "FRAIS D'ÉTABLISSEMENT",
			nl: "OPRICHTINGSKOSTEN",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m3",
			"dim:part": "part:m1",
		},
		code: "28",
		label: "Financial fixed assets",
		labels: {
			de: "Finanzanlagen",
			en: "Financial fixed assets",
			fr: "Immobilisations financières",
			nl: "Financiële vaste activa",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m7",
			"dim:ntr": "ntr:m2",
			"dim:part": "part:m1",
		},
		code: "27",
		label: "Assets under construction and advance payments",
		labels: {
			de: "Anlagen im Bau und geleistete Anzahlungen",
			en: "Assets under construction and advance payments",
			fr: "Immobilisations en cours et acomptes versés",
			nl: "Activa in aanbouw en vooruitbetalingen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:typ": "typ:m1",
			"dim:part": "part:m1",
		},
		code: "26",
		label: "Other tangible fixed assets",
		labels: {
			de: "Sonstige Sachanlagen",
			en: "Other tangible fixed assets",
			fr: "Autres immobilisations corporelles",
			nl: "Overige materiële vaste activa",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m6",
			"dim:ntr": "ntr:m2",
			"dim:part": "part:m1",
		},
		code: "25",
		label: "Leasing and other similar rights",
		labels: {
			de: "Leasing und ähnliche Rechte",
			en: "Leasing and other similar rights",
			fr: "Location-financement et droits similaires",
			nl: "Leasing en soortgelijke rechten",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m5",
			"dim:ntr": "ntr:m2",
			"dim:part": "part:m1",
		},
		code: "24",
		label: "Furniture and vehicles",
		labels: {
			de: "Geschäftsausstattung und Fuhrpark",
			en: "Furniture and vehicles",
			fr: "Mobilier et matériel roulant",
			nl: "Meubilair en rollend materieel",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m4",
			"dim:ntr": "ntr:m2",
			"dim:part": "part:m1",
		},
		code: "23",
		label: "Plant, machinery and equipment",
		labels: {
			de: "Anlagen, Maschinen und Betriebsausstattung",
			en: "Plant, machinery and equipment",
			fr: "Installations, machines et outillage",
			nl: "Installaties, machines en uitrusting",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m3",
			"dim:ntr": "ntr:m2",
			"dim:part": "part:m1",
		},
		code: "22",
		label: "Land and buildings",
		labels: {
			de: "Grundstücke und Bauten",
			en: "Land and buildings",
			fr: "Terrains et constructions",
			nl: "Terreinen en gebouwen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:part": "part:m1",
		},
		code: "22/27",
		label: "Tangible fixed assets",
		labels: {
			de: "Sachanlagen",
			en: "Tangible fixed assets",
			fr: "Immobilisations corporelles",
			nl: "Materiële vaste activa",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
			"dim:part": "part:m1",
		},
		code: "21",
		label: "Intangible fixed assets",
		labels: {
			de: "Immaterielle Anlagewerte",
			en: "Intangible fixed assets",
			fr: "Immobilisations incorporelles",
			nl: "Immateriële vaste activa",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m2",
			"dim:part": "part:m1",
		},
		code: "21/28",
		label: "FIXED ASSETS",
		labels: {
			de: "ANLAGEVERMÖGEN",
			en: "FIXED ASSETS",
			fr: "ACTIFS IMMOBILISÉS",
			nl: "VASTE ACTIVA",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m9",
			"dim:rst": "rst:m1",
			"dim:typ": "typ:m3",
			"dim:part": "part:m1",
		},
		code: "290",
		label: "Trade debtors",
		labels: {
			de: "Forderungen aus Lieferungen und Leistungen",
			en: "Trade debtors",
			fr: "Créances commerciales",
			nl: "Handelsvorderingen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m9",
			"dim:rst": "rst:m1",
			"dim:typ": "typ:m1",
			"dim:part": "part:m1",
		},
		code: "291",
		label: "Other amounts receivable",
		labels: {
			de: "Sonstige Forderungen",
			en: "Other amounts receivable",
			fr: "Autres créances",
			nl: "Overige vorderingen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m9",
			"dim:rst": "rst:m1",
			"dim:part": "part:m1",
		},
		code: "29",
		label: "Amounts receivable after more than one year",
		labels: {
			de: "Forderungen mit einer Restlaufzeit von mehr als einem Jahr",
			en: "Amounts receivable after more than one year",
			fr: "Créances à plus d'un an",
			nl: "Vorderingen op meer dan één jaar",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m14",
			"dim:sts": "sts:m2",
			"dim:part": "part:m1",
		},
		code: "30/36",
		label: "Stocks",
		labels: {
			de: "Vorräte",
			en: "Stocks",
			fr: "Stocks",
			nl: "Voorraden",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m21",
			"dim:sts": "sts:m2",
			"dim:part": "part:m1",
		},
		code: "37",
		label: "Contracts in progress",
		labels: {
			de: "In Ausführung befindliche Bestellungen",
			en: "Contracts in progress",
			fr: "Commandes en cours d'exécution",
			nl: "Bestellingen in uitvoering",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m13",
			"dim:sts": "sts:m2",
			"dim:part": "part:m1",
		},
		code: "3",
		label: "Stocks and contracts in progress",
		labels: {
			de: "Vorräte und in Ausführung befindliche Bestellungen",
			en: "Stocks and contracts in progress",
			fr: "Stocks et commandes en cours d'exécution",
			nl: "Voorraden en bestellingen in uitvoering",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m9",
			"dim:rst": "rst:m2",
			"dim:typ": "typ:m3",
			"dim:part": "part:m1",
		},
		code: "40",
		label: "Trade debtors",
		labels: {
			de: "Forderungen aus Lieferungen und Leistungen",
			en: "Trade debtors",
			fr: "Créances commerciales",
			nl: "Handelsvorderingen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m9",
			"dim:rst": "rst:m2",
			"dim:typ": "typ:m1",
			"dim:part": "part:m1",
		},
		code: "41",
		label: "Other amounts receivable",
		labels: {
			de: "Sonstige Forderungen",
			en: "Other amounts receivable",
			fr: "Autres créances",
			nl: "Overige vorderingen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m9",
			"dim:rst": "rst:m2",
			"dim:part": "part:m1",
		},
		code: "40/41",
		label: "Amounts receivable within one year",
		labels: {
			de: "Forderungen mit einer Restlaufzeit bis zu einem Jahr",
			en: "Amounts receivable within one year",
			fr: "Créances à un an au plus",
			nl: "Vorderingen op ten hoogste één jaar",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m22",
			"dim:part": "part:m1",
		},
		code: "50/53",
		label: "Current investments",
		labels: {
			de: "Geldanlagen",
			en: "Current investments",
			fr: "Placements de trésorerie",
			nl: "Geldbeleggingen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m23",
			"dim:part": "part:m1",
		},
		code: "54/58",
		label: "Cash at bank and in hand",
		labels: {
			de: "Flüssige Mittel",
			en: "Cash at bank and in hand",
			fr: "Valeurs disponibles",
			nl: "Liquide middelen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m24",
			"dim:part": "part:m1",
		},
		code: "490/1",
		label: "Accruals and deferred income",
		labels: {
			de: "Rechnungsabgrenzungsposten",
			en: "Accruals and deferred income",
			fr: "Comptes de régularisation",
			nl: "Overlopende rekeningen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m12",
			"dim:part": "part:m1",
		},
		code: "29/58",
		label: "CURRENT ASSETS",
		labels: {
			de: "UMLAUFVERMÖGEN",
			en: "CURRENT ASSETS",
			fr: "ACTIFS CIRCULANTS",
			nl: "VLOTTENDE ACTIVA",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.01.0.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m25",
			"dim:part": "part:m1",
		},
		code: "20/58",
		label: "TOTAL ASSETS",
		labels: {
			de: "SUMME DER AKTIVA",
			en: "TOTAL ASSETS",
			fr: "TOTAL DE L'ACTIF",
			nl: "TOTAAL VAN DE ACTIVA",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m123",
			"dim:ntr": "ntr:m4",
			"dim:part": "part:m3",
		},
		code: "10",
		label: "Association or foundation Funds",
		labels: {
			de: "Vermögen der Vereinigung oder Stiftung",
			en: "Association or foundation Funds",
			fr: "Fonds de l'association ou de la fondation",
			nl: "Fondsen van de vereniging of stichting",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m42",
			"dim:ntr": "ntr:m4",
			"dim:part": "part:m3",
		},
		code: "12",
		label: "Revaluation surpluses",
		labels: {
			de: "Neubewertungsrücklagen",
			en: "Revaluation surpluses",
			fr: "Plus-values de réévaluation",
			nl: "Herwaarderingsmeerwaarden",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m124",
			"dim:ntr": "ntr:m4",
			"dim:part": "part:m3",
		},
		code: "13",
		label: "Allocated funds",
		labels: {
			de: "Zweckgebundenes Vermögen und sonstige Rûcklagen",
			en: "Allocated funds",
			fr: "Fonds affectés et autres réserves",
			nl: "Bestemde fondsen en andere reserves",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m44",
			"dim:ntr": "ntr:m4",
			"dim:sts": "sts:m8",
			"dim:part": "part:m3",
		},
		code: "14",
		label: "Accumulated profits (losses)",
		labels: {
			de: "Gewinnvortrag (Verlustvortrag)",
			en: "Accumulated profits (losses)",
			fr: "Bénéfice (Perte) reporté(e)",
			nl: "Overgedragen winst (verlies)",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m45",
			"dim:ntr": "ntr:m4",
			"dim:part": "part:m3",
		},
		code: "15",
		label: "Investment grants",
		labels: {
			de: "Kapitalsubventionen",
			en: "Investment grants",
			fr: "Subsides en capital",
			nl: "Kapitaalsubsidies",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m37",
			"dim:ntr": "ntr:m4",
			"dim:part": "part:m3",
		},
		code: "10/15",
		label: "EQUITY",
		labels: {
			de: "EIGENKAPITAL",
			en: "EQUITY",
			fr: "FONDS SOCIAL",
			nl: "EIGEN VERMOGEN",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m48",
			"dim:typ": "typ:m4",
			"dim:spu": "spu:m3",
			"dim:part": "part:m3",
		},
		code: "160",
		label: "Pensions and similar obligations",
		labels: {
			de: "Pensionen und ähnliche Verpflichtungen",
			en: "Pensions and similar obligations",
			fr: "Pensions et obligations similaires",
			nl: "Pensioenen en soortgelijke verplichtingen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m48",
			"dim:typ": "typ:m4",
			"dim:spu": "spu:m4",
			"dim:part": "part:m3",
		},
		code: "161",
		label: "Taxation",
		labels: {
			de: "Steuern",
			en: "Taxation",
			fr: "Charges fiscales",
			nl: "Belastingen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m48",
			"dim:typ": "typ:m4",
			"dim:spu": "spu:m5",
			"dim:part": "part:m3",
		},
		code: "162",
		label: "Major repairs and maintenance",
		labels: {
			de: "Große Reparaturen und Instandhaltungsarbeiten",
			en: "Major repairs and maintenance",
			fr: "Grosses réparations et gros entretien",
			nl: "Grote herstellings- en onderhoudswerken",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m48",
			"dim:typ": "typ:m4",
			"dim:spu": "spu:m6",
			"dim:part": "part:m3",
		},
		code: "163",
		label: "Environmental obligations",
		labels: {
			de: "Umweltschutzverpflichtungen",
			en: "Environmental obligations",
			fr: "Obligations environnementales",
			nl: "Milieuverplichtingen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m48",
			"dim:typ": "typ:m4",
			"dim:spu": "spu:m7",
			"dim:part": "part:m3",
		},
		code: "164/5",
		label: "Other liabilities and charges",
		labels: {
			de: "Sonstige Risiken und Aufwendungen",
			en: "Other liabilities and charges",
			fr: "Autres risques et charges",
			nl: "Overige risico's en kosten",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m48",
			"dim:typ": "typ:m4",
			"dim:part": "part:m3",
		},
		code: "160/5",
		label: "Provisions for liabilities and charges",
		labels: {
			de: "Rückstellungen für Risiken und Aufwendungen",
			en: "Provisions for liabilities and charges",
			fr: "Provisions pour risques et charges",
			nl: "Voorzieningen voor risico's en kosten",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m49",
			"dim:sts": "sts:m9",
			"dim:part": "part:m3",
		},
		code: "168",
		label: "Deferred taxes",
		labels: {
			de: "Aufgeschobene Steuern",
			en: "Deferred taxes",
			fr: "Impôts différés",
			nl: "Uitgestelde belastingen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m48",
			"dim:spu": "spu:m21",
			"dim:part": "part:m3",
		},
		code: "167",
		label: "Provisions for grants and legacies to reimburse and gifts with a recovery right",
		labels: {
			de: "Rückstellungen für zurückzuzahlende Subventionen und Legate und für Schenkungen mit Rücknahmerecht",
			en: "Provisions for grants and legacies to reimburse and gifts with a recovery right",
			fr: "Provisions pour subsides et legs à rembourser et pour dons avec droit de reprise",
			nl: "Voorzieningen voor terug te betalen subsidies en legaten en voor schenkingen met terugnemingsrecht",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m47",
			"dim:part": "part:m3",
		},
		code: "16",
		label: "PROVISIONS AND DEFERRED TAXES",
		labels: {
			de: "RÜCKSTELLUNGEN UND AUFGESCHOBENE STEUERN",
			en: "PROVISIONS AND DEFERRED TAXES",
			fr: "PROVISIONS ET IMPÔTS DIFFÉRÉS",
			nl: "VOORZIENINGEN EN UITGESTELDE BELASTINGEN",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m50",
			"dim:rst": "rst:m1",
			"dim:ntr": "ntr:m3",
			"dim:spec": "spec:m94",
			"dim:part": "part:m3",
		},
		code: "172/3",
		label: "Credit institutions, leasing and other similar obligations",
		labels: {
			de: "Kreditinstitute, Verbindlichkeiten aufgrund von Leasing- und ähnlichen Verträgen",
			en: "Credit institutions, leasing and other similar obligations",
			fr: "Etablissements de crédit, dettes de location-financement et assimilées",
			nl: "Kredietinstellingen, leasingschulden en soortgelijke schulden",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m51",
			"dim:rst": "rst:m1",
			"dim:ntr": "ntr:m3",
			"dim:typ": "typ:m1",
			"dim:part": "part:m3",
		},
		code: "174/0",
		label: "Other loans",
		labels: {
			de: "Sonstige Anleihen",
			en: "Other loans",
			fr: "Autres emprunts",
			nl: "Overige leningen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m50",
			"dim:rst": "rst:m1",
			"dim:ntr": "ntr:m3",
			"dim:part": "part:m3",
		},
		code: "170/4",
		label: "Financial debts",
		labels: {
			de: "Finanzverbindlichkeiten",
			en: "Financial debts",
			fr: "Dettes financières",
			nl: "Financiële schulden",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m50",
			"dim:rst": "rst:m1",
			"dim:typ": "typ:m3",
			"dim:part": "part:m3",
		},
		code: "175",
		label: "Trade debts",
		labels: {
			de: "Verbindlichkeiten aus Lieferungen und Leistungen",
			en: "Trade debts",
			fr: "Dettes commerciales",
			nl: "Handelsschulden",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m20",
			"dim:rst": "rst:m1",
			"dim:spec": "spec:m5",
			"dim:part": "part:m3",
		},
		code: "176",
		label: "Advances received on contracts in progress",
		labels: {
			de: "Anzahlungen auf Bestellungen",
			en: "Advances received on contracts in progress",
			fr: "Acomptes sur commandes",
			nl: "Vooruitbetalingen op bestellingen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m50",
			"dim:rst": "rst:m1",
			"dim:typ": "typ:m1",
			"dim:part": "part:m3",
		},
		code: "178/9",
		label: "Other amounts payable",
		labels: {
			de: "Sonstige Verbindlichkeiten",
			en: "Other amounts payable",
			fr: "Autres dettes",
			nl: "Overige schulden",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m50",
			"dim:rst": "rst:m1",
			"dim:part": "part:m3",
		},
		code: "17",
		label: "Amounts payable after more than one year",
		labels: {
			de: "Verbindlichkeiten mit einer Restlaufzeit von mehr als einem Jahr",
			en: "Amounts payable after more than one year",
			fr: "Dettes à plus d'un an",
			nl: "Schulden op meer dan één jaar",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m50",
			"dim:rst": "rst:m3",
			"dim:part": "part:m3",
		},
		code: "42",
		label: "Current portion of amounts payable after more than one year falling due within one year",
		labels: {
			de: "Innerhalb eines Jahres fällig werdende Verbindlichkeiten mit einer ursprünglichen Laufzeit von mehr als einem Jahr",
			en: "Current portion of amounts payable after more than one year falling due within one year",
			fr: "Dettes à plus d'un an échéant dans l'année",
			nl: "Schulden op meer dan één jaar die binnen het jaar vervallen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m50",
			"dim:rst": "rst:m2",
			"dim:ntr": "ntr:m3",
			"dim:spec": "spec:m2",
			"dim:part": "part:m3",
		},
		code: "430/8",
		label: "Credit institutions",
		labels: {
			de: "Kreditinstitute",
			en: "Credit institutions",
			fr: "Etablissements de crédit",
			nl: "Kredietinstellingen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m51",
			"dim:rst": "rst:m2",
			"dim:ntr": "ntr:m3",
			"dim:typ": "typ:m1",
			"dim:part": "part:m3",
		},
		code: "439",
		label: "Other loans",
		labels: {
			de: "Sonstige Anleihen",
			en: "Other loans",
			fr: "Autres emprunts",
			nl: "Overige leningen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m50",
			"dim:rst": "rst:m2",
			"dim:ntr": "ntr:m3",
			"dim:part": "part:m3",
		},
		code: "43",
		label: "Financial debts",
		labels: {
			de: "Finanzverbindlichkeiten",
			en: "Financial debts",
			fr: "Dettes financières",
			nl: "Financiële schulden",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m50",
			"dim:rst": "rst:m2",
			"dim:typ": "typ:m3",
			"dim:spec": "spec:m3",
			"dim:part": "part:m3",
		},
		code: "440/4",
		label: "Suppliers",
		labels: {
			de: "Lieferanten",
			en: "Suppliers",
			fr: "Fournisseurs",
			nl: "Leveranciers",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m50",
			"dim:rst": "rst:m2",
			"dim:typ": "typ:m3",
			"dim:spec": "spec:m4",
			"dim:part": "part:m3",
		},
		code: "441",
		label: "Bills of exchange payable",
		labels: {
			de: "Verbindlichkeiten aus Wechseln",
			en: "Bills of exchange payable",
			fr: "Effets à payer",
			nl: "Te betalen wissels",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m50",
			"dim:rst": "rst:m2",
			"dim:typ": "typ:m3",
			"dim:part": "part:m3",
		},
		code: "44",
		label: "Trade debts",
		labels: {
			de: "Verbindlichkeiten aus Lieferungen und Leistungen",
			en: "Trade debts",
			fr: "Dettes commerciales",
			nl: "Handelsschulden",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m20",
			"dim:rst": "rst:m2",
			"dim:spec": "spec:m5",
			"dim:part": "part:m3",
		},
		code: "46",
		label: "Advances received on contracts in progress",
		labels: {
			de: "Anzahlungen auf Bestellungen",
			en: "Advances received on contracts in progress",
			fr: "Acomptes sur commandes",
			nl: "Vooruitbetalingen op bestellingen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m50",
			"dim:rst": "rst:m2",
			"dim:spec": "spec:m7",
			"dim:part": "part:m3",
		},
		code: "450/3",
		label: "Taxes",
		labels: {
			de: "Steuern",
			en: "Taxes",
			fr: "Impôts",
			nl: "Belastingen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m50",
			"dim:rst": "rst:m2",
			"dim:spec": "spec:m8",
			"dim:part": "part:m3",
		},
		code: "454/9",
		label: "Remuneration and social security",
		labels: {
			de: "Arbeitsentgelte und Soziallasten",
			en: "Remuneration and social security",
			fr: "Rémunérations et charges sociales",
			nl: "Bezoldigingen en sociale lasten",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m50",
			"dim:rst": "rst:m2",
			"dim:spec": "spec:m6",
			"dim:part": "part:m3",
		},
		code: "45",
		label: "Taxes, remuneration and social security",
		labels: {
			de: "Verbindlichkeiten aufgrund von Steuern, Arbeitsentgelten und Soziallasten",
			en: "Taxes, remuneration and social security",
			fr: "Dettes fiscales, salariales et sociales",
			nl: "Schulden met betrekking tot belastingen, bezoldigingen en sociale lasten",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m50",
			"dim:rst": "rst:m2",
			"dim:typ": "typ:m1",
			"dim:part": "part:m3",
		},
		code: "48",
		label: "Other amounts payable",
		labels: {
			de: "Sonstige Verbindlichkeiten",
			en: "Other amounts payable",
			fr: "Autres dettes",
			nl: "Overige schulden",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m50",
			"dim:rst": "rst:m2",
			"dim:part": "part:m3",
		},
		code: "42/48",
		label: "Amounts payable within one year",
		labels: {
			de: "Verbindlichkeiten mit einer Restlaufzeit bis zu einem Jahr",
			en: "Amounts payable within one year",
			fr: "Dettes à un an au plus",
			nl: "Schulden op ten hoogste één jaar",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m24",
			"dim:part": "part:m3",
		},
		code: "492/3",
		label: "Accruals and deferred income",
		labels: {
			de: "Rechnungsabgrenzungsposten",
			en: "Accruals and deferred income",
			fr: "Comptes de régularisation",
			nl: "Overlopende rekeningen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m50",
			"dim:part": "part:m3",
		},
		code: "17/49",
		label: "AMOUNTS PAYABLE",
		labels: {
			de: "VERBINDLICHKEITEN",
			en: "AMOUNTS PAYABLE",
			fr: "DETTES",
			nl: "SCHULDEN",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.03.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m25",
			"dim:part": "part:m3",
		},
		code: "10/49",
		label: "TOTAL LIABILITIES",
		labels: {
			de: "SUMME DER PASSIVA",
			en: "TOTAL LIABILITIES",
			fr: "TOTAL DU PASSIF",
			nl: "TOTAAL VAN DE PASSIVA",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m52",
			"dim:ntr": "ntr:m6",
			"dim:typ": "typ:m7",
			"dim:part": "part:m4",
		},
		code: "76A",
		label: "Of which non-recurring operating income",
		labels: {
			de: "Wovon: nicht wiederkehrende betriebliche Erträge",
			en: "Of which non-recurring operating income",
			fr: "Dont: produits d'exploitation non récurrents",
			nl: "Waarvan: niet-recurrente bedrijfsopbrengsten",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m53",
			"dim:ntr": "ntr:m17",
			"dim:part": "part:m4",
		},
		code: "70",
		label: "Turnover",
		labels: {
			de: "Umsatzerlöse",
			en: "Turnover",
			fr: "Chiffre d'affaires",
			nl: "Omzet",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m125",
			"dim:ntr": "ntr:m17",
			"dim:part": "part:m4",
		},
		code: "73",
		label: "Contributions, gifts, legacies and grants",
		labels: {
			de: "Beiträge, Schenkungen, Legate und Subventionen",
			en: "Contributions, gifts, legacies and grants",
			fr: "Cotisations, dons, legs et subsides",
			nl: "Lidgeld, schenkingen, legaten en subsidies",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m119",
			"dim:ntr": "ntr:m17",
			"dim:part": "part:m4",
		},
		code: "60/61",
		label: "Raw materials, consumables, services and other goods",
		labels: {
			de: "Waren, Roh-, Hilfs- und Betriebsstoffe, übrige Lieferungen und Leistungen",
			en: "Raw materials, consumables, services and other goods",
			fr: "Approvisionnements, marchandises, services et biens divers",
			nl: "Handelsgoederen, grond- en hulpstoffen, diensten en diverse goederen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m118",
			"dim:ntr": "ntr:m17",
			"dim:part": "part:m4",
		},
		code: "9900",
		label: "Gross operating margin",
		labels: {
			de: "Brutto-Betriebsmarge",
			en: "Gross operating margin",
			fr: "Marge brute d’exploitation",
			nl: "Brutomarge",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m1",
			"dim:spec": "spec:m9",
			"dim:bkd": "bkd:m1",
			"dim:ntr": "ntr:m6",
			"dim:part": "part:m4",
		},
		code: "630",
		label: "Depreciation of and other amounts written off formation expenses, intangible and tangible fixed assets",
		labels: {
			de: "Abschreibungen und Wertminderungen auf Errichtungs- und Erweiterungsaufwendungen, auf immaterielle Anlagewerte und Sachanlagen",
			en: "Depreciation of and other amounts written off formation expenses, intangible and tangible fixed assets",
			fr: "Amortissements et réductions de valeur sur frais d'établissement, sur immobilisations incorporelles et corporelles",
			nl: "Afschrijvingen en waardeverminderingen op oprichtingskosten, op immateriële en materiële vaste activa",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m48",
			"dim:typ": "typ:m4",
			"dim:part": "part:m4",
		},
		code: "635/9",
		label: "Provisions for liabilities and charges: Appropriations (uses and write-backs)",
		labels: {
			de: "Rückstellungen für Risiken und Aufwendungen: Zuführungen (Verbrauch und Auflösungen)",
			en: "Provisions for liabilities and charges: Appropriations (uses and write-backs)",
			fr: "Provisions pour risques et charges: dotations (utilisations et reprises)",
			nl: "Voorzieningen voor risico's en kosten: toevoegingen (bestedingen en terugnemingen)",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m1",
			"dim:spec": "spec:m10",
			"dim:bkd": "bkd:m2",
			"dim:mdp": "mdp:m2",
			"dim:ntr": "ntr:m6",
			"dim:part": "part:m4",
		},
		code: "631/4",
		label: "Amounts written off stocks, contracts in progress and trade debtors: Appropriations (write-backs)",
		labels: {
			de: "Wertminderungen von Vorräten, in Ausführung befindlichen Bestellungen und Forderungen aus Lieferungen und Leistungen: Zuführungen (Rücknahmen)",
			en: "Amounts written off stocks, contracts in progress and trade debtors: Appropriations (write-backs)",
			fr: "Réductions de valeur sur stocks, sur commandes en cours d'exécution et sur créances commerciales: dotations (reprises)",
			nl: "Waardeverminderingen op voorraden, op bestellingen in uitvoering en op handelsvorderingen: toevoegingen (terugnemingen)",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m7",
			"dim:part": "part:m4",
		},
		code: "62",
		label: "Remuneration, social security costs and pensions",
		labels: {
			de: "Arbeitsentgelte, Soziallasten und Pensionen",
			en: "Remuneration, social security costs and pensions",
			fr: "Rémunérations, charges sociales et pensions",
			nl: "Bezoldigingen, sociale lasten en pensioenen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m1",
			"dim:typ": "typ:m2",
			"dim:ntr": "ntr:m6",
			"dim:part": "part:m4",
		},
		code: "640/8",
		label: "Other operating charges",
		labels: {
			de: "Sonstige betriebliche Aufwendungen",
			en: "Other operating charges",
			fr: "Autres charges d'exploitation",
			nl: "Andere bedrijfskosten",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am3",
		dimensions: {
			"dim:bas": "bas:m1",
			"dim:spec": "spec:m11",
			"dim:mdp": "mdp:m3",
			"dim:ntr": "ntr:m6",
			"dim:part": "part:m4",
		},
		code: "649",
		label: "Operating charges carried to assets as restructuring costs",
		labels: {
			de: "Auf der Aktivseite als Restrukturierungskosten ausgewiesene betriebliche Aufwendungen",
			en: "Operating charges carried to assets as restructuring costs",
			fr: "Charges d'exploitation portées à l'actif au titre de frais de restructuration",
			nl: "Als herstructureringskosten geactiveerde bedrijfskosten",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m1",
			"dim:typ": "typ:m7",
			"dim:ntr": "ntr:m6",
			"dim:part": "part:m4",
		},
		code: "66A",
		label: "Non-recurring operating charges",
		labels: {
			de: "Nicht wiederkehrende betriebliche Aufwendungen",
			en: "Non-recurring operating charges",
			fr: "Charges d'exploitation non récurrentes",
			nl: "Niet-recurrente bedrijfskosten",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m44",
			"dim:ntr": "ntr:m6",
			"dim:part": "part:m4",
		},
		code: "9901",
		label: "Operating profit (loss)",
		labels: {
			de: "Betriebsgewinn (Betriebsverlust)",
			en: "Operating profit (loss)",
			fr: "Bénéfice (Perte) d'exploitation",
			nl: "Bedrijfswinst (Bedrijfsverlies)",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m52",
			"dim:ntr": "ntr:m3",
			"dim:typ": "typ:m8",
			"dim:part": "part:m4",
		},
		code: "75",
		label: "Recurring financial income",
		labels: {
			de: "Wiederkehrende Finanzerträge",
			en: "Recurring financial income",
			fr: "Produits financiers récurrents",
			nl: "Recurrente financiële opbrengsten",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m52",
			"dim:ntr": "ntr:m3",
			"dim:typ": "typ:m7",
			"dim:part": "part:m4",
		},
		code: "76B",
		label: "Non-recurring financial income",
		labels: {
			de: "Nicht wiederkehrende Finanzerträge",
			en: "Non-recurring financial income",
			fr: "Produits financiers non récurrents",
			nl: "Niet-recurrente financiële opbrengsten",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m52",
			"dim:ntr": "ntr:m3",
			"dim:part": "part:m4",
		},
		code: "75/76B",
		label: "Financial income",
		labels: {
			de: "Finanzerträge",
			en: "Financial income",
			fr: "Produits financiers",
			nl: "Financiële opbrengsten",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m3",
			"dim:typ": "typ:m8",
			"dim:part": "part:m4",
		},
		code: "65",
		label: "Recurring financial charges",
		labels: {
			de: "Wiederkehrende Finanzaufwendungen",
			en: "Recurring financial charges",
			fr: "Charges financières récurrentes",
			nl: "Recurrente financiële kosten",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m3",
			"dim:typ": "typ:m7",
			"dim:part": "part:m4",
		},
		code: "66B",
		label: "Non-recurring financial charges",
		labels: {
			de: "Nicht wiederkehrende Finanzaufwendungen",
			en: "Non-recurring financial charges",
			fr: "Charges financières non récurrentes",
			nl: "Niet-recurrente financiële kosten",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m3",
			"dim:part": "part:m4",
		},
		code: "65/66B",
		label: "Financial charges",
		labels: {
			de: "Finanzaufwendungen",
			en: "Financial charges",
			fr: "Charges financières",
			nl: "Financiële kosten",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m59",
			"dim:spec": "spec:m16",
			"dim:part": "part:m4",
		},
		code: "9903",
		label: "Gain (loss) for the period before taxes",
		labels: {
			de: "Gewinn (Verlust) des Geschäftsjahres vor Steuern",
			en: "Gain (loss) for the period before taxes",
			fr: "Bénéfice (Perte) de l'exercice avant impôts",
			nl: "Winst (Verlies) van het boekjaar vóór belasting",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m49",
			"dim:sts": "sts:m9",
			"dim:mdp": "mdp:m4",
			"dim:part": "part:m4",
		},
		code: "780",
		label: "Transfer from deferred taxes",
		labels: {
			de: "Auflösung von aufgeschobenen Steuern",
			en: "Transfer from deferred taxes",
			fr: "Prélèvement sur les impôts différés",
			nl: "Onttrekking aan de uitgestelde belastingen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m49",
			"dim:sts": "sts:m9",
			"dim:mdp": "mdp:m5",
			"dim:part": "part:m4",
		},
		code: "680",
		label: "Transfer to deferred taxes",
		labels: {
			de: "Zuführung zu aufgeschobenen Steuern",
			en: "Transfer to deferred taxes",
			fr: "Transfert aux impôts différés",
			nl: "Overboeking naar de uitgestelde belastingen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m49",
			"dim:spec": "spec:m17",
			"dim:part": "part:m4",
		},
		code: "67/77",
		label: "Income taxes",
		labels: {
			de: "Steuern auf das Ergebnis",
			en: "Income taxes",
			fr: "Impôts sur le résultat",
			nl: "Belastingen op het resultaat",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m59",
			"dim:part": "part:m4",
		},
		code: "9904",
		label: "Gain (loss) of the period",
		labels: {
			de: "Gewinn (Verlust) des Geschäftsjahres",
			en: "Gain (loss) of the period",
			fr: "Bénéfice (Perte) de l'exercice",
			nl: "Winst (Verlies) van het boekjaar",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m43",
			"dim:sts": "sts:m6",
			"dim:mdp": "mdp:m4",
			"dim:part": "part:m4",
		},
		code: "789",
		label: "Transfer from untaxed reserves",
		labels: {
			de: "Entnahmen aus den steuerfreien Rücklagen",
			en: "Transfer from untaxed reserves",
			fr: "Prélèvement sur les réserves immunisées",
			nl: "Onttrekking aan de belastingvrije reserves",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m43",
			"dim:sts": "sts:m6",
			"dim:mdp": "mdp:m5",
			"dim:part": "part:m4",
		},
		code: "689",
		label: "Transfer to untaxed reserves",
		labels: {
			de: "Einstellung in die steuerfreien Rücklagen",
			en: "Transfer to untaxed reserves",
			fr: "Transfert aux réserves immunisées",
			nl: "Overboeking naar de belastingvrije reserves",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.04.00.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m59",
			"dim:sts": "sts:m11",
			"dim:part": "part:m4",
		},
		code: "9905",
		label: "Gain (loss) of the period available for appropriation",
		labels: {
			de: "Zu verwendender Gewinn (anzurechnender Verlust) des Geschäftsjahres",
			en: "Gain (loss) of the period available for appropriation",
			fr: "Bénéfice (Perte) de l'exercice à affecter",
			nl: "Te bestemmen winst (verlies) van het boekjaar",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.05.00.0.ghi",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m59",
			"dim:sts": "sts:m11",
			"dim:part": "part:m4",
		},
		code: "(9905)",
		label: "Gain (loss) of the period available for appropriation",
		labels: {
			de: "Zu verwendender Gewinn (anzurechnender Verlust) des Geschäftsjahres",
			en: "Gain (loss) of the period available for appropriation",
			fr: "Bénéfice (Perte) de l'exercice à affecter",
			nl: "Te bestemmen winst (verlies) van het boekjaar",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.05.00.0.ghi",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m61",
			"dim:sts": "sts:m8",
			"dim:part": "part:m5",
		},
		code: "14P",
		label: "Profit (loss) brought forward",
		labels: {
			de: "Gewinnvortrag (Verlustvortrag) aus dem Vorjahr",
			en: "Profit (loss) brought forward",
			fr: "Bénéfice (Perte) reporté(e) de l'exercice précédent",
			nl: "Overgedragen winst (verlies) van het vorige boekjaar",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.05.00.0.ghi",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m44",
			"dim:sts": "sts:m11",
			"dim:part": "part:m5",
		},
		code: "9906",
		label: "Profit (loss) to be appropriated",
		labels: {
			de: "Zu verwendender Gewinnsaldo (anzurechnender Verlustsaldo)",
			en: "Profit (loss) to be appropriated",
			fr: "Bénéfice (Perte) à affecter",
			nl: "Te bestemmen winst (verlies)",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.05.00.0.ghi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m37",
			"dim:ntr": "ntr:m4",
			"dim:mdp": "mdp:m4",
			"dim:part": "part:m5",
		},
		code: "791",
		label: "Withdrawals from capital and reserves",
		labels: {
			de: "Entnahmen aus dem Eigenkapital: Vermögen, zweckgebundene Vermögen und sonstige Rücklagen",
			en: "Withdrawals from capital and reserves",
			fr: "Prélèvement sur les capitaux propres: fonds, fonds affectés et autres réserves",
			nl: "Onttrekking aan het eigen vermogen: fondsen, bestemde fondsen en andere reserves",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.05.00.0.ghi",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m124",
			"dim:mdp": "mdp:m5",
			"dim:part": "part:m5",
		},
		code: "691",
		label: "Appropriations to allocated funds",
		labels: {
			de: "Zuweisungen an die zweckgebundenen Vermögen und sonstige Rücklagen",
			en: "Appropriations to allocated funds",
			fr: "Affectation aux fonds affectés et autres réserves",
			nl: "Toevoeging aan de bestemde fondsen en andere reserves",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.05.00.0.ghi",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m44",
			"dim:sts": "sts:m8",
			"dim:part": "part:m3",
			"dim:ntr": "ntr:m4",
		},
		code: "(14)",
		label: "Profit (loss) to be carried forward",
		labels: {
			de: "Vorzutragender Gewinn (Verlust)",
			en: "Profit (loss) to be carried forward",
			fr: "Bénéfice (Perte) à reporter",
			nl: "Over te dragen winst (verlies)",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bkd": "bkd:m4",
			"dim:aln": "aln:m1",
			"dim:part": "part:m6",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
			"dim:rut": "rut:m1",
		},
		code: "8059P",
		label: "Acquisition value at the end of the period",
		labels: {
			de: "Anschaffungswert am Ende des Geschäftsjahres",
			en: "Acquisition value at the end of the period",
			fr: "Valeur d'acquisition au terme de l'exercice",
			nl: "Aanschaffingswaarde per einde van het boekjaar",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m4",
			"dim:mdp": "mdp:m6",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
		},
		code: "8029",
		label: "Acquisitions, including produced fixed assets",
		labels: {
			de: "Anschaffungen einschließlich aktivierter Eigenleistungen",
			en: "Acquisitions, including produced fixed assets",
			fr: "Acquisitions, y compris la production immobilisée",
			nl: "Aanschaffingen, met inbegrip van de geproduceerde vaste activa",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m4",
			"dim:mdp": "mdp:m6",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
			"dim:rut": "rut:m1",
		},
		code: "8029",
		label: "Acquisitions, including produced fixed assets",
		labels: {
			de: "Anschaffungen einschließlich aktivierter Eigenleistungen",
			en: "Acquisitions, including produced fixed assets",
			fr: "Acquisitions, y compris la production immobilisée",
			nl: "Aanschaffingen, met inbegrip van de geproduceerde vaste activa",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:mdp": "mdp:m8",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
		},
		code: "8039",
		label: "Sales and disposals",
		labels: {
			de: "Veräußerungen und Außerdienststellungen",
			en: "Sales and disposals",
			fr: "Cessions et désaffectations",
			nl: "Overdrachten en buitengebruikstellingen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:mdp": "mdp:m8",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
			"dim:rut": "rut:m1",
		},
		code: "8039",
		label: "Sales and disposals",
		labels: {
			de: "Veräußerungen und Außerdienststellungen",
			en: "Sales and disposals",
			fr: "Cessions et désaffectations",
			nl: "Overdrachten en buitengebruikstellingen",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:mdp": "mdp:m9",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
		},
		code: "8049",
		label: "Transfers from one heading to another",
		labels: {
			de: "Umbuchungen von einem Posten in einen anderen",
			en: "Transfers from one heading to another",
			fr: "Transferts d'une rubrique à une autre",
			nl: "Overboekingen van een post naar een andere",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:mdp": "mdp:m9",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
			"dim:rut": "rut:m1",
		},
		code: "8049",
		label: "Transfers from one heading to another",
		labels: {
			de: "Umbuchungen von einem Posten in einen anderen",
			en: "Transfers from one heading to another",
			fr: "Transferts d'une rubrique à une autre",
			nl: "Overboekingen van een post naar een andere",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bkd": "bkd:m4",
			"dim:part": "part:m6",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
		},
		code: "8059",
		label: "Acquisition value at the end of the period",
		labels: {
			de: "Anschaffungswert am Ende des Geschäftsjahres",
			en: "Acquisition value at the end of the period",
			fr: "Valeur d'acquisition au terme de l'exercice",
			nl: "Aanschaffingswaarde per einde van het boekjaar",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bkd": "bkd:m4",
			"dim:part": "part:m6",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
			"dim:rut": "rut:m1",
		},
		code: "8059",
		label: "Acquisition value at the end of the period",
		labels: {
			de: "Anschaffungswert am Ende des Geschäftsjahres",
			en: "Acquisition value at the end of the period",
			fr: "Valeur d'acquisition au terme de l'exercice",
			nl: "Aanschaffingswaarde per einde van het boekjaar",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bkd": "bkd:m1",
			"dim:aln": "aln:m1",
			"dim:part": "part:m6",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
			"dim:rut": "rut:m1",
		},
		code: "8129P",
		label: "Amortisations and amounts written down at the end of the period",
		labels: {
			de: "Abschreibungen und Wertminderungen am Ende des Geschäftsjahres",
			en: "Amortisations and amounts written down at the end of the period",
			fr: "Amortissements et réductions de valeur au terme de l'exercice",
			nl: "Afschrijvingen en waardeverminderingen per einde van het boekjaar",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:mdp": "mdp:m6",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
		},
		code: "8079",
		label: "Recorded",
		labels: {
			de: "Gebucht",
			en: "Recorded",
			fr: "Actés",
			nl: "Geboekt",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:mdp": "mdp:m6",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
			"dim:rut": "rut:m1",
		},
		code: "8079",
		label: "Recorded",
		labels: {
			de: "Gebucht",
			en: "Recorded",
			fr: "Actés",
			nl: "Geboekt",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:mdp": "mdp:m10",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
		},
		code: "8089",
		label: "Written back",
		labels: {
			de: "Zurückgenommen",
			en: "Written back",
			fr: "Repris",
			nl: "Teruggenomen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:mdp": "mdp:m10",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
			"dim:rut": "rut:m1",
		},
		code: "8089",
		label: "Written back",
		labels: {
			de: "Zurückgenommen",
			en: "Written back",
			fr: "Repris",
			nl: "Teruggenomen",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:mdp": "mdp:m11",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
		},
		code: "8099",
		label: "Acquisitions from third parties",
		labels: {
			de: "Von Dritten erworben",
			en: "Acquisitions from third parties",
			fr: "Acquis de tiers",
			nl: "Verworven van derden",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:mdp": "mdp:m11",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
			"dim:rut": "rut:m1",
		},
		code: "8099",
		label: "Acquisitions from third parties",
		labels: {
			de: "Von Dritten erworben",
			en: "Acquisitions from third parties",
			fr: "Acquis de tiers",
			nl: "Verworven van derden",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:mdp": "mdp:m12",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
		},
		code: "8109",
		label: "Cancelled owing to sales and disposals",
		labels: {
			de: "Aufgrund von Veräußerungen und Außerdienststellungen gelöscht",
			en: "Cancelled owing to sales and disposals",
			fr: "Annulés à la suite de cessions et désaffectations",
			nl: "Afgeboekt na overdrachten en buitengebruikstellingen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:mdp": "mdp:m12",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
			"dim:rut": "rut:m1",
		},
		code: "8109",
		label: "Cancelled owing to sales and disposals",
		labels: {
			de: "Aufgrund von Veräußerungen und Außerdienststellungen gelöscht",
			en: "Cancelled owing to sales and disposals",
			fr: "Annulés à la suite de cessions et désaffectations",
			nl: "Afgeboekt na overdrachten en buitengebruikstellingen",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:mdp": "mdp:m9",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
		},
		code: "8119",
		label: "Transferred from one heading to another",
		labels: {
			de: "Von einem Posten in einen anderen umgebucht",
			en: "Transferred from one heading to another",
			fr: "Transférés d'une rubrique à une autre",
			nl: "Overgeboekt van een post naar een andere",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:mdp": "mdp:m9",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
			"dim:rut": "rut:m1",
		},
		code: "8119",
		label: "Transferred from one heading to another",
		labels: {
			de: "Von einem Posten in einen anderen umgebucht",
			en: "Transferred from one heading to another",
			fr: "Transférés d'une rubrique à une autre",
			nl: "Overgeboekt van een post naar een andere",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bkd": "bkd:m1",
			"dim:part": "part:m6",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
		},
		code: "8129",
		label: "Amortisations and amounts written down at the end of the period",
		labels: {
			de: "Abschreibungen und Wertminderungen am Ende des Geschäftsjahres",
			en: "Amortisations and amounts written down at the end of the period",
			fr: "Amortissements et réductions de valeur au terme de l'exercice",
			nl: "Afschrijvingen en waardeverminderingen per einde van het boekjaar",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:bkd": "bkd:m1",
			"dim:part": "part:m6",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
			"dim:rut": "rut:m1",
		},
		code: "8129",
		label: "Amortisations and amounts written down at the end of the period",
		labels: {
			de: "Abschreibungen und Wertminderungen am Ende des Geschäftsjahres",
			en: "Amortisations and amounts written down at the end of the period",
			fr: "Amortissements et réductions de valeur au terme de l'exercice",
			nl: "Afschrijvingen en waardeverminderingen per einde van het boekjaar",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m1",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
		},
		code: "(21)",
		label: "NET BOOK VALUE AT THE END OF THE PERIOD",
		labels: {
			de: "NETTOBUCHWERT AM ENDE DES GESCHÄFTSJAHRES",
			en: "NET BOOK VALUE AT THE END OF THE PERIOD",
			fr: "VALEUR COMPTABLE NETTE AU TERME DE L'EXERCICE",
			nl: "NETTOBOEKWAARDE PER EINDE VAN HET BOEKJAAR",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.1.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m1",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m1",
			"dim:rut": "rut:m1",
		},
		code: "(21)",
		label: "NET BOOK VALUE AT THE END OF THE PERIOD",
		labels: {
			de: "NETTOBUCHWERT AM ENDE DES GESCHÄFTSJAHRES",
			en: "NET BOOK VALUE AT THE END OF THE PERIOD",
			fr: "VALEUR COMPTABLE NETTE AU TERME DE L'EXERCICE",
			nl: "NETTOBOEKWAARDE PER EINDE VAN HET BOEKJAAR",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
		},
		code: "8329",
		label: "Depreciations and amounts written down at the end of the period",
		labels: {
			de: "Abschreibungen und Wertminderungen am Ende des Geschäftsjahres",
			en: "Depreciations and amounts written down at the end of the period",
			fr: "Amortissements et réductions de valeur au terme de l'exercice",
			nl: "Afschrijvingen en waardeverminderingen per einde van het boekjaar",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:rut": "rut:m1",
		},
		code: "8329",
		label: "Depreciations and amounts written down at the end of the period",
		labels: {
			de: "Abschreibungen und Wertminderungen am Ende des Geschäftsjahres",
			en: "Depreciations and amounts written down at the end of the period",
			fr: "Amortissements et réductions de valeur au terme de l'exercice",
			nl: "Afschrijvingen en waardeverminderingen per einde van het boekjaar",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:mdp": "mdp:m9",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
		},
		code: "8319",
		label: "Transferred from one heading to another",
		labels: {
			de: "Von einem Posten in einen anderen umgebucht",
			en: "Transferred from one heading to another",
			fr: "Transférés d'une rubrique à une autre",
			nl: "Overgeboekt van een post naar een andere",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:mdp": "mdp:m9",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:rut": "rut:m1",
		},
		code: "8319",
		label: "Transferred from one heading to another",
		labels: {
			de: "Von einem Posten in einen anderen umgebucht",
			en: "Transferred from one heading to another",
			fr: "Transférés d'une rubrique à une autre",
			nl: "Overgeboekt van een post naar een andere",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:mdp": "mdp:m12",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
		},
		code: "8309",
		label: "Cancelled owing to sales and disposals",
		labels: {
			de: "Aufgrund von Veräußerungen und Außerdienststellungen gelöscht",
			en: "Cancelled owing to sales and disposals",
			fr: "Annulés à la suite de cessions et désaffectations",
			nl: "Afgeboekt na overdrachten en buitengebruikstellingen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:mdp": "mdp:m12",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:rut": "rut:m1",
		},
		code: "8309",
		label: "Cancelled owing to sales and disposals",
		labels: {
			de: "Aufgrund von Veräußerungen und Außerdienststellungen gelöscht",
			en: "Cancelled owing to sales and disposals",
			fr: "Annulés à la suite de cessions et désaffectations",
			nl: "Afgeboekt na overdrachten en buitengebruikstellingen",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:mdp": "mdp:m11",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
		},
		code: "8299",
		label: "Acquisitions from third parties",
		labels: {
			de: "Von Dritten erworben",
			en: "Acquisitions from third parties",
			fr: "Acquis de tiers",
			nl: "Verworven van derden",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:mdp": "mdp:m11",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:rut": "rut:m1",
		},
		code: "8299",
		label: "Acquisitions from third parties",
		labels: {
			de: "Von Dritten erworben",
			en: "Acquisitions from third parties",
			fr: "Acquis de tiers",
			nl: "Verworven van derden",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:mdp": "mdp:m10",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
		},
		code: "8289",
		label: "Written back",
		labels: {
			de: "Zurückgenommen",
			en: "Written back",
			fr: "Repris",
			nl: "Teruggenomen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:mdp": "mdp:m10",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:rut": "rut:m1",
		},
		code: "8289",
		label: "Written back",
		labels: {
			de: "Zurückgenommen",
			en: "Written back",
			fr: "Repris",
			nl: "Teruggenomen",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:mdp": "mdp:m6",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
		},
		code: "8279",
		label: "Recorded",
		labels: {
			de: "Gebucht",
			en: "Recorded",
			fr: "Actés",
			nl: "Geboekt",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:mdp": "mdp:m6",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:rut": "rut:m1",
		},
		code: "8279",
		label: "Recorded",
		labels: {
			de: "Gebucht",
			en: "Recorded",
			fr: "Actés",
			nl: "Geboekt",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m1",
			"dim:aln": "aln:m1",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:rut": "rut:m1",
		},
		code: "8329P",
		label: "Depreciations and amounts written down at the end of the period",
		labels: {
			de: "Abschreibungen und Wertminderungen am Ende des Geschäftsjahres",
			en: "Depreciations and amounts written down at the end of the period",
			fr: "Amortissements et réductions de valeur au terme de l'exercice",
			nl: "Afschrijvingen en waardeverminderingen per einde van het boekjaar",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m4",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
		},
		code: "8199",
		label: "Acquisition value at the end of the period",
		labels: {
			de: "Anschaffungswert am Ende des Geschäftsjahres",
			en: "Acquisition value at the end of the period",
			fr: "Valeur d'acquisition au terme de l'exercice",
			nl: "Aanschaffingswaarde per einde van het boekjaar",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m4",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:rut": "rut:m1",
		},
		code: "8199",
		label: "Acquisition value at the end of the period",
		labels: {
			de: "Anschaffungswert am Ende des Geschäftsjahres",
			en: "Acquisition value at the end of the period",
			fr: "Valeur d'acquisition au terme de l'exercice",
			nl: "Aanschaffingswaarde per einde van het boekjaar",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m4",
			"dim:mdp": "mdp:m9",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
		},
		code: "8189",
		label: "Transfers from one heading to another",
		labels: {
			de: "Umbuchungen von einem Posten in einen anderen",
			en: "Transfers from one heading to another",
			fr: "Transferts d'une rubrique à une autre",
			nl: "Overboekingen van een post naar een andere",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m4",
			"dim:mdp": "mdp:m9",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:rut": "rut:m1",
		},
		code: "8189",
		label: "Transfers from one heading to another",
		labels: {
			de: "Umbuchungen von einem Posten in einen anderen",
			en: "Transfers from one heading to another",
			fr: "Transferts d'une rubrique à une autre",
			nl: "Overboekingen van een post naar een andere",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m4",
			"dim:mdp": "mdp:m8",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
		},
		code: "8179",
		label: "Sales and disposals",
		labels: {
			de: "Veräußerungen und Außerdienststellungen",
			en: "Sales and disposals",
			fr: "Cessions et désaffectations",
			nl: "Overdrachten en buitengebruikstellingen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m4",
			"dim:mdp": "mdp:m8",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:rut": "rut:m1",
		},
		code: "8179",
		label: "Sales and disposals",
		labels: {
			de: "Veräußerungen und Außerdienststellungen",
			en: "Sales and disposals",
			fr: "Cessions et désaffectations",
			nl: "Overdrachten en buitengebruikstellingen",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m4",
			"dim:mdp": "mdp:m6",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
		},
		code: "8169",
		label: "Acquisitions, including produced fixed assets",
		labels: {
			de: "Anschaffungen einschließlich aktivierter Eigenleistungen",
			en: "Acquisitions, including produced fixed assets",
			fr: "Acquisitions, y compris la production immobilisée",
			nl: "Aanschaffingen, met inbegrip van de geproduceerde vaste activa",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m4",
			"dim:mdp": "mdp:m6",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:rut": "rut:m1",
		},
		code: "8169",
		label: "Acquisitions, including produced fixed assets",
		labels: {
			de: "Anschaffungen einschließlich aktivierter Eigenleistungen",
			en: "Acquisitions, including produced fixed assets",
			fr: "Acquisitions, y compris la production immobilisée",
			nl: "Aanschaffingen, met inbegrip van de geproduceerde vaste activa",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m4",
			"dim:aln": "aln:m1",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:rut": "rut:m1",
		},
		code: "8199P",
		label: "Acquisition value at the end of the period",
		labels: {
			de: "Anschaffungswert am Ende des Geschäftsjahres",
			en: "Acquisition value at the end of the period",
			fr: "Valeur d'acquisition au terme de l'exercice",
			nl: "Aanschaffingswaarde per einde van het boekjaar",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m5",
			"dim:aln": "aln:m1",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:rut": "rut:m1",
		},
		code: "8259P",
		label: "Revaluation surpluses at the end of the period",
		labels: {
			de: "Mehrwerte am Ende des Geschäftsjahres",
			en: "Revaluation surpluses at the end of the period",
			fr: "Plus-values au terme de l'exercice",
			nl: "Meerwaarden per einde van het boekjaar",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m5",
			"dim:mdp": "mdp:m6",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
		},
		code: "8219",
		label: "Recorded",
		labels: {
			de: "Gebucht",
			en: "Recorded",
			fr: "Actées",
			nl: "Geboekt",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m5",
			"dim:mdp": "mdp:m6",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:rut": "rut:m1",
		},
		code: "8219",
		label: "Recorded",
		labels: {
			de: "Gebucht",
			en: "Recorded",
			fr: "Actées",
			nl: "Geboekt",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m5",
			"dim:mdp": "mdp:m11",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
		},
		code: "8229",
		label: "Acquisitions from third parties",
		labels: {
			de: "Von Dritten erworben",
			en: "Acquisitions from third parties",
			fr: "Acquises de tiers",
			nl: "Verworven van derden",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m5",
			"dim:mdp": "mdp:m11",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:rut": "rut:m1",
		},
		code: "8229",
		label: "Acquisitions from third parties",
		labels: {
			de: "Von Dritten erworben",
			en: "Acquisitions from third parties",
			fr: "Acquises de tiers",
			nl: "Verworven van derden",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m5",
			"dim:mdp": "mdp:m12",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
		},
		code: "8239",
		label: "Cancelled",
		labels: {
			de: "Gelöscht",
			en: "Cancelled",
			fr: "Annulées",
			nl: "Afgeboekt",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m5",
			"dim:mdp": "mdp:m12",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:rut": "rut:m1",
		},
		code: "8239",
		label: "Cancelled",
		labels: {
			de: "Gelöscht",
			en: "Cancelled",
			fr: "Annulées",
			nl: "Afgeboekt",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m5",
			"dim:mdp": "mdp:m9",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
		},
		code: "8249",
		label: "Transferred from one heading to another",
		labels: {
			de: "Von einem Posten in einen anderen umgebucht",
			en: "Transferred from one heading to another",
			fr: "Transférées d'une rubrique à une autre",
			nl: "Overgeboekt van een post naar een andere",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m5",
			"dim:mdp": "mdp:m9",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:rut": "rut:m1",
		},
		code: "8249",
		label: "Transferred from one heading to another",
		labels: {
			de: "Von einem Posten in einen anderen umgebucht",
			en: "Transferred from one heading to another",
			fr: "Transférées d'une rubrique à une autre",
			nl: "Overgeboekt van een post naar een andere",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m5",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
		},
		code: "8259",
		label: "Revaluation surpluses at the end of the period",
		labels: {
			de: "Mehrwerte am Ende des Geschäftsjahres",
			en: "Revaluation surpluses at the end of the period",
			fr: "Plus-values au terme de l'exercice",
			nl: "Meerwaarden per einde van het boekjaar",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:bkd": "bkd:m5",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:rut": "rut:m1",
		},
		code: "8259",
		label: "Revaluation surpluses at the end of the period",
		labels: {
			de: "Mehrwerte am Ende des Geschäftsjahres",
			en: "Revaluation surpluses at the end of the period",
			fr: "Plus-values au terme de l'exercice",
			nl: "Meerwaarden per einde van het boekjaar",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:spec": "spec:m96",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
		},
		code: "8349",
		label: "Owned by the association or the foundation in full property",
		labels: {
			de: "Im Volleigentum der Vereinigung oder Stiftung",
			en: "Owned by the association or the foundation in full property",
			fr: "Appartenant à l'association ou à la fondation en pleine propriété",
			nl: "In volle eigendom van de vereniging of stichting",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:spec": "spec:m96",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:rut": "rut:m1",
		},
		code: "8349",
		label: "Owned by the association or the foundation in full property",
		labels: {
			de: "Im Volleigentum der Vereinigung oder Stiftung",
			en: "Owned by the association or the foundation in full property",
			fr: "Appartenant à l'association ou à la fondation en pleine propriété",
			nl: "In volle eigendom van de vereniging of stichting",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m1",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
		},
		code: "(22/27)",
		label: "NET BOOK VALUE AT THE END OF THE PERIOD",
		labels: {
			de: "NETTOBUCHWERT AM ENDE DES GESCHÄFTSJAHRES",
			en: "NET BOOK VALUE AT THE END OF THE PERIOD",
			fr: "VALEUR COMPTABLE NETTE AU TERME DE L'EXERCICE",
			nl: "NETTOBOEKWAARDE PER EINDE VAN HET BOEKJAAR",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.2.hi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m1",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m2",
			"dim:rut": "rut:m1",
		},
		code: "(22/27)",
		label: "NET BOOK VALUE AT THE END OF THE PERIOD",
		labels: {
			de: "NETTOBUCHWERT AM ENDE DES GESCHÄFTSJAHRES",
			en: "NET BOOK VALUE AT THE END OF THE PERIOD",
			fr: "VALEUR COMPTABLE NETTE AU TERME DE L'EXERCICE",
			nl: "NETTOBOEKWAARDE PER EINDE VAN HET BOEKJAAR",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m2",
			"dim:bas": "bas:m2",
		},
		code: "8525",
		label: "Amounts written down at the end of the period",
		labels: {
			de: "Abschreibungen und Wertminderungen am Ende des Geschäftsjahres",
			en: "Amounts written down at the end of the period",
			fr: "Réductions de valeur au terme de l'exercice",
			nl: "Waardeverminderingen per einde van het boekjaar",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m2",
			"dim:bas": "bas:m2",
			"dim:rut": "rut:m1",
		},
		code: "8525",
		label: "Amounts written down at the end of the period",
		labels: {
			de: "Abschreibungen und Wertminderungen am Ende des Geschäftsjahres",
			en: "Amounts written down at the end of the period",
			fr: "Réductions de valeur au terme de l'exercice",
			nl: "Waardeverminderingen per einde van het boekjaar",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m2",
			"dim:bas": "bas:m2",
			"dim:mdp": "mdp:m9",
		},
		code: "8515",
		label: "Transferred from one heading to another",
		labels: {
			de: "Von einem Posten in einen anderen umgebucht",
			en: "Transferred from one heading to another",
			fr: "Transférées d'une rubrique à une autre",
			nl: "Overgeboekt van een post naar een andere",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m2",
			"dim:bas": "bas:m2",
			"dim:mdp": "mdp:m9",
			"dim:rut": "rut:m1",
		},
		code: "8515",
		label: "Transferred from one heading to another",
		labels: {
			de: "Von einem Posten in einen anderen umgebucht",
			en: "Transferred from one heading to another",
			fr: "Transférées d'une rubrique à une autre",
			nl: "Overgeboekt van een post naar een andere",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m2",
			"dim:bas": "bas:m2",
			"dim:mdp": "mdp:m12",
		},
		code: "8505",
		label: "Cancelled owing to sales and disposals",
		labels: {
			de: "Aufgrund von Veräußerungen und Außerdienststellungen gelöscht",
			en: "Cancelled owing to sales and disposals",
			fr: "Annulées à la suite de cessions et retraits",
			nl: "Afgeboekt na overdrachten en buitengebruikstellingen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m2",
			"dim:bas": "bas:m2",
			"dim:mdp": "mdp:m12",
			"dim:rut": "rut:m1",
		},
		code: "8505",
		label: "Cancelled owing to sales and disposals",
		labels: {
			de: "Aufgrund von Veräußerungen und Außerdienststellungen gelöscht",
			en: "Cancelled owing to sales and disposals",
			fr: "Annulées à la suite de cessions et retraits",
			nl: "Afgeboekt na overdrachten en buitengebruikstellingen",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m2",
			"dim:bas": "bas:m2",
			"dim:mdp": "mdp:m11",
		},
		code: "8495",
		label: "Acquisitions from third parties",
		labels: {
			de: "Von Dritten erworben",
			en: "Acquisitions from third parties",
			fr: "Acquises de tiers",
			nl: "Verworven van derden",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m2",
			"dim:bas": "bas:m2",
			"dim:mdp": "mdp:m11",
			"dim:rut": "rut:m1",
		},
		code: "8495",
		label: "Acquisitions from third parties",
		labels: {
			de: "Von Dritten erworben",
			en: "Acquisitions from third parties",
			fr: "Acquises de tiers",
			nl: "Verworven van derden",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m2",
			"dim:bas": "bas:m2",
			"dim:mdp": "mdp:m10",
		},
		code: "8485",
		label: "Written back",
		labels: {
			de: "Zurückgenommen",
			en: "Written back",
			fr: "Reprises",
			nl: "Teruggenomen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m2",
			"dim:bas": "bas:m2",
			"dim:mdp": "mdp:m10",
			"dim:rut": "rut:m1",
		},
		code: "8485",
		label: "Written back",
		labels: {
			de: "Zurückgenommen",
			en: "Written back",
			fr: "Reprises",
			nl: "Teruggenomen",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m2",
			"dim:bas": "bas:m2",
			"dim:mdp": "mdp:m6",
		},
		code: "8475",
		label: "Recorded",
		labels: {
			de: "Gebucht",
			en: "Recorded",
			fr: "Actées",
			nl: "Geboekt",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m2",
			"dim:bas": "bas:m2",
			"dim:mdp": "mdp:m6",
			"dim:rut": "rut:m1",
		},
		code: "8475",
		label: "Recorded",
		labels: {
			de: "Gebucht",
			en: "Recorded",
			fr: "Actées",
			nl: "Geboekt",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m2",
			"dim:aln": "aln:m1",
			"dim:bas": "bas:m2",
			"dim:rut": "rut:m1",
		},
		code: "8525P",
		label: "Amounts written down at the end of the period",
		labels: {
			de: "Abschreibungen und Wertminderungen am Ende des Geschäftsjahres",
			en: "Amounts written down at the end of the period",
			fr: "Réductions de valeur au terme de l'exercice",
			nl: "Waardeverminderingen per einde van het boekjaar",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m4",
			"dim:bas": "bas:m70",
		},
		code: "8395",
		label: "Acquisition value at the end of the period",
		labels: {
			de: "Anschaffungswert am Ende des Geschäftsjahres",
			en: "Acquisition value at the end of the period",
			fr: "Valeur d'acquisition au terme de l'exercice",
			nl: "Aanschaffingswaarde per einde van het boekjaar",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m4",
			"dim:bas": "bas:m70",
			"dim:rut": "rut:m1",
		},
		code: "8395",
		label: "Acquisition value at the end of the period",
		labels: {
			de: "Anschaffungswert am Ende des Geschäftsjahres",
			en: "Acquisition value at the end of the period",
			fr: "Valeur d'acquisition au terme de l'exercice",
			nl: "Aanschaffingswaarde per einde van het boekjaar",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m4",
			"dim:bas": "bas:m70",
			"dim:mdp": "mdp:m9",
		},
		code: "8385",
		label: "Transfers from one heading to another",
		labels: {
			de: "Umbuchungen von einem Posten in einen anderen",
			en: "Transfers from one heading to another",
			fr: "Transferts d'une rubrique à une autre",
			nl: "Overboekingen van een post naar een andere",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m4",
			"dim:bas": "bas:m70",
			"dim:mdp": "mdp:m9",
			"dim:rut": "rut:m1",
		},
		code: "8385",
		label: "Transfers from one heading to another",
		labels: {
			de: "Umbuchungen von einem Posten in einen anderen",
			en: "Transfers from one heading to another",
			fr: "Transferts d'une rubrique à une autre",
			nl: "Overboekingen van een post naar een andere",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m4",
			"dim:bas": "bas:m70",
			"dim:mdp": "mdp:m8",
		},
		code: "8375",
		label: "Sales and disposals",
		labels: {
			de: "Veräußerungen und Außerdienststellungen",
			en: "Sales and disposals",
			fr: "Cessions et retraits",
			nl: "Overdrachten en buitengebruikstellingen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m4",
			"dim:bas": "bas:m70",
			"dim:mdp": "mdp:m8",
			"dim:rut": "rut:m1",
		},
		code: "8375",
		label: "Sales and disposals",
		labels: {
			de: "Veräußerungen und Außerdienststellungen",
			en: "Sales and disposals",
			fr: "Cessions et retraits",
			nl: "Overdrachten en buitengebruikstellingen",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m4",
			"dim:bas": "bas:m70",
			"dim:mdp": "mdp:m6",
		},
		code: "8365",
		label: "Acquisitions",
		labels: {
			de: "Anschaffungen",
			en: "Acquisitions",
			fr: "Acquisitions",
			nl: "Aanschaffingen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m4",
			"dim:bas": "bas:m70",
			"dim:mdp": "mdp:m6",
			"dim:rut": "rut:m1",
		},
		code: "8365",
		label: "Acquisitions",
		labels: {
			de: "Anschaffungen",
			en: "Acquisitions",
			fr: "Acquisitions",
			nl: "Aanschaffingen",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m4",
			"dim:bas": "bas:m70",
			"dim:mdp": "mdp:m7",
		},
		code: "8386",
		label: "Other movements",
		labels: {
			de: "Sonstige Veränderungen",
			en: "Other movements",
			fr: "Autres mutations",
			nl: "Andere mutaties",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m4",
			"dim:bas": "bas:m70",
			"dim:mdp": "mdp:m7",
			"dim:rut": "rut:m1",
		},
		code: "8386",
		label: "Other movements",
		labels: {
			de: "Sonstige Veränderungen",
			en: "Other movements",
			fr: "Autres mutations",
			nl: "Andere mutaties",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m4",
			"dim:aln": "aln:m1",
			"dim:bas": "bas:m70",
			"dim:rut": "rut:m1",
		},
		code: "8395P",
		label: "Acquisition value at the end of the period",
		labels: {
			de: "Anschaffungswert am Ende des Geschäftsjahres",
			en: "Acquisition value at the end of the period",
			fr: "Valeur d'acquisition au terme de l'exercice",
			nl: "Aanschaffingswaarde per einde van het boekjaar",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m5",
			"dim:aln": "aln:m1",
			"dim:bas": "bas:m2",
			"dim:rut": "rut:m1",
		},
		code: "8455P",
		label: "Revaluation surpluses at the end of the period",
		labels: {
			de: "Mehrwerte am Ende des Geschäftsjahres",
			en: "Revaluation surpluses at the end of the period",
			fr: "Plus-values au terme de l'exercice",
			nl: "Meerwaarden per einde van het boekjaar",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m5",
			"dim:bas": "bas:m2",
			"dim:mdp": "mdp:m6",
		},
		code: "8415",
		label: "Recorded",
		labels: {
			de: "Gebucht",
			en: "Recorded",
			fr: "Actées",
			nl: "Geboekt",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m5",
			"dim:bas": "bas:m2",
			"dim:mdp": "mdp:m6",
			"dim:rut": "rut:m1",
		},
		code: "8415",
		label: "Recorded",
		labels: {
			de: "Gebucht",
			en: "Recorded",
			fr: "Actées",
			nl: "Geboekt",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m5",
			"dim:bas": "bas:m2",
			"dim:mdp": "mdp:m11",
		},
		code: "8425",
		label: "Acquisitions from third parties",
		labels: {
			de: "Von Dritten erworben",
			en: "Acquisitions from third parties",
			fr: "Acquises de tiers",
			nl: "Verworven van derden",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m5",
			"dim:bas": "bas:m2",
			"dim:mdp": "mdp:m11",
			"dim:rut": "rut:m1",
		},
		code: "8425",
		label: "Acquisitions from third parties",
		labels: {
			de: "Von Dritten erworben",
			en: "Acquisitions from third parties",
			fr: "Acquises de tiers",
			nl: "Verworven van derden",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m5",
			"dim:bas": "bas:m2",
			"dim:mdp": "mdp:m12",
		},
		code: "8435",
		label: "Cancelled",
		labels: {
			de: "Gelöscht",
			en: "Cancelled",
			fr: "Annulées",
			nl: "Afgeboekt",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m5",
			"dim:bas": "bas:m2",
			"dim:mdp": "mdp:m12",
			"dim:rut": "rut:m1",
		},
		code: "8435",
		label: "Cancelled",
		labels: {
			de: "Gelöscht",
			en: "Cancelled",
			fr: "Annulées",
			nl: "Afgeboekt",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m5",
			"dim:bas": "bas:m2",
			"dim:mdp": "mdp:m9",
		},
		code: "8445",
		label: "Transferred from one heading to another",
		labels: {
			de: "Von einem Posten in einen anderen umgebucht",
			en: "Transferred from one heading to another",
			fr: "Transférées d'une rubrique à une autre",
			nl: "Overgeboekt van een post naar een andere",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m5",
			"dim:bas": "bas:m2",
			"dim:mdp": "mdp:m9",
			"dim:rut": "rut:m1",
		},
		code: "8445",
		label: "Transferred from one heading to another",
		labels: {
			de: "Von einem Posten in einen anderen umgebucht",
			en: "Transferred from one heading to another",
			fr: "Transférées d'une rubrique à une autre",
			nl: "Overgeboekt van een post naar een andere",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m5",
			"dim:bas": "bas:m2",
		},
		code: "8455",
		label: "Revaluation surpluses at the end of the period",
		labels: {
			de: "Mehrwerte am Ende des Geschäftsjahres",
			en: "Revaluation surpluses at the end of the period",
			fr: "Plus-values au terme de l'exercice",
			nl: "Meerwaarden per einde van het boekjaar",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m5",
			"dim:bas": "bas:m2",
			"dim:rut": "rut:m1",
		},
		code: "8455",
		label: "Revaluation surpluses at the end of the period",
		labels: {
			de: "Mehrwerte am Ende des Geschäftsjahres",
			en: "Revaluation surpluses at the end of the period",
			fr: "Plus-values au terme de l'exercice",
			nl: "Meerwaarden per einde van het boekjaar",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m6",
			"dim:aln": "aln:m1",
			"dim:bas": "bas:m2",
			"dim:rut": "rut:m1",
		},
		code: "8555P",
		label: "Uncalled amounts at the end of the period",
		labels: {
			de: "Nicht eingeforderte Beträge am Ende des Geschäftsjahres",
			en: "Uncalled amounts at the end of the period",
			fr: "Montants non appelés au terme de l'exercice",
			nl: "Niet-opgevraagde bedragen per einde van het boekjaar",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m6",
			"dim:mdp": "mdp:m2",
			"dim:bas": "bas:m2",
		},
		code: "8545",
		label: "Movements during the period",
		labels: {
			de: "Veränderungen im Geschäftsjahr",
			en: "Movements during the period",
			fr: "Mutations de l'exercice",
			nl: "Mutaties tijdens het boekjaar",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m6",
			"dim:mdp": "mdp:m2",
			"dim:bas": "bas:m2",
			"dim:rut": "rut:m1",
		},
		code: "8545",
		label: "Movements during the period",
		labels: {
			de: "Veränderungen im Geschäftsjahr",
			en: "Movements during the period",
			fr: "Mutations de l'exercice",
			nl: "Mutaties tijdens het boekjaar",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m6",
			"dim:bas": "bas:m2",
		},
		code: "8555",
		label: "Uncalled amounts at the end of the period",
		labels: {
			de: "Nicht eingeforderte Beträge am Ende des Geschäftsjahres",
			en: "Uncalled amounts at the end of the period",
			fr: "Montants non appelés au terme de l'exercice",
			nl: "Niet-opgevraagde bedragen per einde van het boekjaar",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:psn": "psn:m1",
			"dim:rls": "rls:m1",
			"dim:bkd": "bkd:m6",
			"dim:bas": "bas:m2",
			"dim:rut": "rut:m1",
		},
		code: "8555",
		label: "Uncalled amounts at the end of the period",
		labels: {
			de: "Nicht eingeforderte Beträge am Ende des Geschäftsjahres",
			en: "Uncalled amounts at the end of the period",
			fr: "Montants non appelés au terme de l'exercice",
			nl: "Niet-opgevraagde bedragen per einde van het boekjaar",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m1",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m3",
		},
		code: "(28)",
		label: "NET BOOK VALUE AT THE END OF THE PERIOD",
		labels: {
			de: "NETTOBUCHWERT AM ENDE DES GESCHÄFTSJAHRES",
			en: "NET BOOK VALUE AT THE END OF THE PERIOD",
			fr: "VALEUR COMPTABLE NETTE AU TERME DE L'EXERCICE",
			nl: "NETTOBOEKWAARDE PER EINDE VAN HET BOEKJAAR",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.01.3.cdefhi",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m1",
			"dim:bas": "bas:m2",
			"dim:ntr": "ntr:m3",
			"dim:rut": "rut:m1",
		},
		code: "(28)",
		label: "NET BOOK VALUE AT THE END OF THE PERIOD",
		labels: {
			de: "NETTOBUCHWERT AM ENDE DES GESCHÄFTSJAHRES",
			en: "NET BOOK VALUE AT THE END OF THE PERIOD",
			fr: "VALEUR COMPTABLE NETTE AU TERME DE L'EXERCICE",
			nl: "NETTOBOEKWAARDE PER EINDE VAN HET BOEKJAAR",
		},
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m12",
			"dim:spu": "spu:m14",
			"dim:typ": "typ:m20",
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:gtd": "gtd:m3",
		},
		openDimensions: ["dim:open"],
		code: "9150",
		label: "Bills of exchange in circulation endorsed by the association or the foundation",
		labels: {
			de: "Durch die Vereinigung oder Stiftung indossierte, in Umlauf befindliche Handelswechsel",
			en: "Bills of exchange in circulation endorsed by the association or the foundation",
			fr: "Effets de commerce en circulation endossés par l'association ou la fondation",
			nl: "Door de vereniging of stichting geëndosseerde handelseffecten in omloop",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:ntr": "ntr:m12",
			"dim:spu": "spu:m14",
			"dim:typ": "typ:m20",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "9150",
		label: "Bills of exchange in circulation endorsed by the association or the foundation",
		labels: {
			de: "Durch die Vereinigung oder Stiftung indossierte, in Umlauf befindliche Handelswechsel",
			en: "Bills of exchange in circulation endorsed by the association or the foundation",
			fr: "Effets de commerce en circulation endossés par l'association ou la fondation",
			nl: "Door de vereniging of stichting geëndosseerde handelseffecten in omloop",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m12",
			"dim:spu": "spu:m14",
			"dim:typ": "typ:m20",
		},
		openDimensions: ["dim:open"],
		code: "9150",
		label: "Bills of exchange in circulation endorsed by the association or the foundation",
		labels: {
			de: "Durch die Vereinigung oder Stiftung indossierte, in Umlauf befindliche Handelswechsel",
			en: "Bills of exchange in circulation endorsed by the association or the foundation",
			fr: "Effets de commerce en circulation endossés par l'association ou la fondation",
			nl: "Door de vereniging of stichting geëndosseerde handelseffecten in omloop",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:ntr": "ntr:m12",
			"dim:spu": "spu:m14",
			"dim:typ": "typ:m20",
		},
		openDimensions: ["dim:open"],
		code: "9150",
		label: "Bills of exchange in circulation endorsed by the association or the foundation",
		labels: {
			de: "Durch die Vereinigung oder Stiftung indossierte, in Umlauf befindliche Handelswechsel",
			en: "Bills of exchange in circulation endorsed by the association or the foundation",
			fr: "Effets de commerce en circulation endossés par l'association ou la fondation",
			nl: "Door de vereniging of stichting geëndosseerde handelseffecten in omloop",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m12",
			"dim:spu": "spu:m14",
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:gtd": "gtd:m3",
		},
		openDimensions: ["dim:open"],
		code: "9149",
		label: "PERSONAL GUARANTEES PROVIDED OR IRREVOCABLY PROMISED BY THE ASSOCIATION OR FOUNDATION AS SECURITY FOR DEBTS AND COMMITMENTS OF THIRD PARTIES",
		labels: {
			de: "PERSÖNLICHE SICHERHEITEN, DIE DURCH DIE VEREINIGUNG ODER STIFTUNG ZUR BESICHERUNG VON VERBINDLICHKEITEN ODER VERPFLICHTUNGEN DRITTER GESTELLT ODER UNWIDERRUFLICH ZUGESAGT WURDEN",
			en: "PERSONAL GUARANTEES PROVIDED OR IRREVOCABLY PROMISED BY THE ASSOCIATION OR FOUNDATION AS SECURITY FOR DEBTS AND COMMITMENTS OF THIRD PARTIES",
			fr: "GARANTIES PERSONNELLES CONSTITUÉES OU IRRÉVOCABLEMENT PROMISES PAR L'ASSOCIATION OU LA FONDATION POUR SÛRETÉ DE DETTES OU D'ENGAGEMENTS DE TIERS",
			nl: "DOOR DE VERENIGING OF STICHTING GESTELDE OF ONHERROEPELIJK BELOOFDE PERSOONLIJKE ZEKERHEDEN ALS WAARBORG VOOR SCHULDEN OF VERPLICHTINGEN VAN DERDEN",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:ntr": "ntr:m12",
			"dim:spu": "spu:m14",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "9149",
		label: "PERSONAL GUARANTEES PROVIDED OR IRREVOCABLY PROMISED BY THE ASSOCIATION OR FOUNDATION AS SECURITY FOR DEBTS AND COMMITMENTS OF THIRD PARTIES",
		labels: {
			de: "PERSÖNLICHE SICHERHEITEN, DIE DURCH DIE VEREINIGUNG ODER STIFTUNG ZUR BESICHERUNG VON VERBINDLICHKEITEN ODER VERPFLICHTUNGEN DRITTER GESTELLT ODER UNWIDERRUFLICH ZUGESAGT WURDEN",
			en: "PERSONAL GUARANTEES PROVIDED OR IRREVOCABLY PROMISED BY THE ASSOCIATION OR FOUNDATION AS SECURITY FOR DEBTS AND COMMITMENTS OF THIRD PARTIES",
			fr: "GARANTIES PERSONNELLES CONSTITUÉES OU IRRÉVOCABLEMENT PROMISES PAR L'ASSOCIATION OU LA FONDATION POUR SÛRETÉ DE DETTES OU D'ENGAGEMENTS DE TIERS",
			nl: "DOOR DE VERENIGING OF STICHTING GESTELDE OF ONHERROEPELIJK BELOOFDE PERSOONLIJKE ZEKERHEDEN ALS WAARBORG VOOR SCHULDEN OF VERPLICHTINGEN VAN DERDEN",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m12",
			"dim:spu": "spu:m14",
		},
		openDimensions: ["dim:open"],
		code: "9149",
		label: "PERSONAL GUARANTEES PROVIDED OR IRREVOCABLY PROMISED BY THE ASSOCIATION OR FOUNDATION AS SECURITY FOR DEBTS AND COMMITMENTS OF THIRD PARTIES",
		labels: {
			de: "PERSÖNLICHE SICHERHEITEN, DIE DURCH DIE VEREINIGUNG ODER STIFTUNG ZUR BESICHERUNG VON VERBINDLICHKEITEN ODER VERPFLICHTUNGEN DRITTER GESTELLT ODER UNWIDERRUFLICH ZUGESAGT WURDEN",
			en: "PERSONAL GUARANTEES PROVIDED OR IRREVOCABLY PROMISED BY THE ASSOCIATION OR FOUNDATION AS SECURITY FOR DEBTS AND COMMITMENTS OF THIRD PARTIES",
			fr: "GARANTIES PERSONNELLES CONSTITUÉES OU IRRÉVOCABLEMENT PROMISES PAR L'ASSOCIATION OU LA FONDATION POUR SÛRETÉ DE DETTES OU D'ENGAGEMENTS DE TIERS",
			nl: "DOOR DE VERENIGING OF STICHTING GESTELDE OF ONHERROEPELIJK BELOOFDE PERSOONLIJKE ZEKERHEDEN ALS WAARBORG VOOR SCHULDEN OF VERPLICHTINGEN VAN DERDEN",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:ntr": "ntr:m12",
			"dim:spu": "spu:m14",
		},
		openDimensions: ["dim:open"],
		code: "9149",
		label: "PERSONAL GUARANTEES PROVIDED OR IRREVOCABLY PROMISED BY THE ASSOCIATION OR FOUNDATION AS SECURITY FOR DEBTS AND COMMITMENTS OF THIRD PARTIES",
		labels: {
			de: "PERSÖNLICHE SICHERHEITEN, DIE DURCH DIE VEREINIGUNG ODER STIFTUNG ZUR BESICHERUNG VON VERBINDLICHKEITEN ODER VERPFLICHTUNGEN DRITTER GESTELLT ODER UNWIDERRUFLICH ZUGESAGT WURDEN",
			en: "PERSONAL GUARANTEES PROVIDED OR IRREVOCABLY PROMISED BY THE ASSOCIATION OR FOUNDATION AS SECURITY FOR DEBTS AND COMMITMENTS OF THIRD PARTIES",
			fr: "GARANTIES PERSONNELLES CONSTITUÉES OU IRRÉVOCABLEMENT PROMISES PAR L'ASSOCIATION OU LA FONDATION POUR SÛRETÉ DE DETTES OU D'ENGAGEMENTS DE TIERS",
			nl: "DOOR DE VERENIGING OF STICHTING GESTELDE OF ONHERROEPELIJK BELOOFDE PERSOONLIJKE ZEKERHEDEN ALS WAARBORG VOOR SCHULDEN OF VERPLICHTINGEN VAN DERDEN",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:qlt": "qlt:m2",
			"dim:typ": "typ:m22",
			"dim:spec": "spec:m60",
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:gtd": "gtd:m3",
		},
		openDimensions: ["dim:open"],
		code: "91611",
		label: "Book value of the immovable properties mortgaged",
		labels: {
			de: "Buchwert der belasteten Aktiva",
			en: "Book value of the immovable properties mortgaged",
			fr: "Valeur comptable des immeubles grevés",
			nl: "Boekwaarde van de bezwaarde activa",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:qlt": "qlt:m2",
			"dim:typ": "typ:m22",
			"dim:spec": "spec:m60",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "91611",
		label: "Book value of the immovable properties mortgaged",
		labels: {
			de: "Buchwert der belasteten Aktiva",
			en: "Book value of the immovable properties mortgaged",
			fr: "Valeur comptable des immeubles grevés",
			nl: "Boekwaarde van de bezwaarde activa",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:qlt": "qlt:m2",
			"dim:typ": "typ:m22",
			"dim:spec": "spec:m60",
		},
		openDimensions: ["dim:open"],
		code: "91611",
		label: "Book value of the immovable properties mortgaged",
		labels: {
			de: "Buchwert der belasteten Aktiva",
			en: "Book value of the immovable properties mortgaged",
			fr: "Valeur comptable des immeubles grevés",
			nl: "Boekwaarde van de bezwaarde activa",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:qlt": "qlt:m2",
			"dim:typ": "typ:m22",
			"dim:spec": "spec:m60",
		},
		openDimensions: ["dim:open"],
		code: "91611",
		label: "Book value of the immovable properties mortgaged",
		labels: {
			de: "Buchwert der belasteten Aktiva",
			en: "Book value of the immovable properties mortgaged",
			fr: "Valeur comptable des immeubles grevés",
			nl: "Boekwaarde van de bezwaarde activa",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m22",
			"dim:spec": "spec:m61",
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:gtd": "gtd:m3",
		},
		openDimensions: ["dim:open"],
		code: "91621",
		label: "Amount of registration",
		labels: {
			de: "Betrag der Eintragung",
			en: "Amount of registration",
			fr: "Montant de l'inscription",
			nl: "Bedrag van de inschrijving",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m22",
			"dim:spec": "spec:m61",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "91621",
		label: "Amount of registration",
		labels: {
			de: "Betrag der Eintragung",
			en: "Amount of registration",
			fr: "Montant de l'inscription",
			nl: "Bedrag van de inschrijving",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m22",
			"dim:spec": "spec:m61",
		},
		openDimensions: ["dim:open"],
		code: "91621",
		label: "Amount of registration",
		labels: {
			de: "Betrag der Eintragung",
			en: "Amount of registration",
			fr: "Montant de l'inscription",
			nl: "Bedrag van de inschrijving",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m22",
			"dim:spec": "spec:m61",
		},
		openDimensions: ["dim:open"],
		code: "91621",
		label: "Amount of registration",
		labels: {
			de: "Betrag der Eintragung",
			en: "Amount of registration",
			fr: "Montant de l'inscription",
			nl: "Bedrag van de inschrijving",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m23",
			"dim:spec": "spec:m62",
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:gtd": "gtd:m3",
		},
		openDimensions: ["dim:open"],
		code: "91631",
		label: "For irrevocable mortgage mandates, the amount for which the agent can take registration",
		labels: {
			de: "Im Falle von unwiderruflichen Hypothekarmandaten: der Betrag, den der Bevollmächtigte aufgrund des Mandats eintragen kann",
			en: "For irrevocable mortgage mandates, the amount for which the agent can take registration",
			fr: "Pour les mandats irrévocables d'hypothéquer, le montant pour lequel le mandataire est autorisé à prendre inscription en vertu du mandat",
			nl: "Voor de onherroepelijke mandaten tot hypothekeren, het bedrag waarvoor de volmachthebber krachtens het mandaat inschrijving mag nemen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m23",
			"dim:spec": "spec:m62",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "91631",
		label: "For irrevocable mortgage mandates, the amount for which the agent can take registration",
		labels: {
			de: "Im Falle von unwiderruflichen Hypothekarmandaten: der Betrag, den der Bevollmächtigte aufgrund des Mandats eintragen kann",
			en: "For irrevocable mortgage mandates, the amount for which the agent can take registration",
			fr: "Pour les mandats irrévocables d'hypothéquer, le montant pour lequel le mandataire est autorisé à prendre inscription en vertu du mandat",
			nl: "Voor de onherroepelijke mandaten tot hypothekeren, het bedrag waarvoor de volmachthebber krachtens het mandaat inschrijving mag nemen",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m23",
			"dim:spec": "spec:m62",
		},
		openDimensions: ["dim:open"],
		code: "91631",
		label: "For irrevocable mortgage mandates, the amount for which the agent can take registration",
		labels: {
			de: "Im Falle von unwiderruflichen Hypothekarmandaten: der Betrag, den der Bevollmächtigte aufgrund des Mandats eintragen kann",
			en: "For irrevocable mortgage mandates, the amount for which the agent can take registration",
			fr: "Pour les mandats irrévocables d'hypothéquer, le montant pour lequel le mandataire est autorisé à prendre inscription en vertu du mandat",
			nl: "Voor de onherroepelijke mandaten tot hypothekeren, het bedrag waarvoor de volmachthebber krachtens het mandaat inschrijving mag nemen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m23",
			"dim:spec": "spec:m62",
		},
		openDimensions: ["dim:open"],
		code: "91631",
		label: "For irrevocable mortgage mandates, the amount for which the agent can take registration",
		labels: {
			de: "Im Falle von unwiderruflichen Hypothekarmandaten: der Betrag, den der Bevollmächtigte aufgrund des Mandats eintragen kann",
			en: "For irrevocable mortgage mandates, the amount for which the agent can take registration",
			fr: "Pour les mandats irrévocables d'hypothéquer, le montant pour lequel le mandataire est autorisé à prendre inscription en vertu du mandat",
			nl: "Voor de onherroepelijke mandaten tot hypothekeren, het bedrag waarvoor de volmachthebber krachtens het mandaat inschrijving mag nemen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m24",
			"dim:spec": "spec:m63",
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:gtd": "gtd:m3",
		},
		openDimensions: ["dim:open"],
		code: "91711",
		label: "Maximum amount up to which the debt is secured and which is the subject of registration",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung und der Eintragung",
			en: "Maximum amount up to which the debt is secured and which is the subject of registration",
			fr: "Le montant maximum à concurrence duquel la dette est garantie et qui fait l'objet de l'enregistrement",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd en waarvoor registratie plaatsvindt",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m24",
			"dim:spec": "spec:m63",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "91711",
		label: "Maximum amount up to which the debt is secured and which is the subject of registration",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung und der Eintragung",
			en: "Maximum amount up to which the debt is secured and which is the subject of registration",
			fr: "Le montant maximum à concurrence duquel la dette est garantie et qui fait l'objet de l'enregistrement",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd en waarvoor registratie plaatsvindt",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m24",
			"dim:spec": "spec:m63",
		},
		openDimensions: ["dim:open"],
		code: "91711",
		label: "Maximum amount up to which the debt is secured and which is the subject of registration",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung und der Eintragung",
			en: "Maximum amount up to which the debt is secured and which is the subject of registration",
			fr: "Le montant maximum à concurrence duquel la dette est garantie et qui fait l'objet de l'enregistrement",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd en waarvoor registratie plaatsvindt",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m24",
			"dim:spec": "spec:m63",
		},
		openDimensions: ["dim:open"],
		code: "91711",
		label: "Maximum amount up to which the debt is secured and which is the subject of registration",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung und der Eintragung",
			en: "Maximum amount up to which the debt is secured and which is the subject of registration",
			fr: "Le montant maximum à concurrence duquel la dette est garantie et qui fait l'objet de l'enregistrement",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd en waarvoor registratie plaatsvindt",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m25",
			"dim:spec": "spec:m64",
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:gtd": "gtd:m3",
		},
		openDimensions: ["dim:open"],
		code: "91721",
		label: "For irrevocable mandates to pledge goodwill, the amount for which the agent can take the inscription",
		labels: {
			de: "Im Falle von unwiderruflichen Mandaten zur Verpfändung des Firmenwertes: der Betrag, den der Bevollmächtigte aufgrund des Mandats eintragen kann",
			en: "For irrevocable mandates to pledge goodwill, the amount for which the agent can take the inscription",
			fr: "Pour les mandats irrévocables de mise en gage du fonds de commerce, le montant pour lequel le mandataire est autorisé à procéder à l'enregistrement en vertu du mandat",
			nl: "Voor de onherroepelijke mandaten tot verpanding van het handelsfonds, het bedrag waarvoor de volmachthebber krachtens het mandaat tot registratie mag overgaan",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m25",
			"dim:spec": "spec:m64",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "91721",
		label: "For irrevocable mandates to pledge goodwill, the amount for which the agent can take the inscription",
		labels: {
			de: "Im Falle von unwiderruflichen Mandaten zur Verpfändung des Firmenwertes: der Betrag, den der Bevollmächtigte aufgrund des Mandats eintragen kann",
			en: "For irrevocable mandates to pledge goodwill, the amount for which the agent can take the inscription",
			fr: "Pour les mandats irrévocables de mise en gage du fonds de commerce, le montant pour lequel le mandataire est autorisé à procéder à l'enregistrement en vertu du mandat",
			nl: "Voor de onherroepelijke mandaten tot verpanding van het handelsfonds, het bedrag waarvoor de volmachthebber krachtens het mandaat tot registratie mag overgaan",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m25",
			"dim:spec": "spec:m64",
		},
		openDimensions: ["dim:open"],
		code: "91721",
		label: "For irrevocable mandates to pledge goodwill, the amount for which the agent can take the inscription",
		labels: {
			de: "Im Falle von unwiderruflichen Mandaten zur Verpfändung des Firmenwertes: der Betrag, den der Bevollmächtigte aufgrund des Mandats eintragen kann",
			en: "For irrevocable mandates to pledge goodwill, the amount for which the agent can take the inscription",
			fr: "Pour les mandats irrévocables de mise en gage du fonds de commerce, le montant pour lequel le mandataire est autorisé à procéder à l'enregistrement en vertu du mandat",
			nl: "Voor de onherroepelijke mandaten tot verpanding van het handelsfonds, het bedrag waarvoor de volmachthebber krachtens het mandaat tot registratie mag overgaan",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m25",
			"dim:spec": "spec:m64",
		},
		openDimensions: ["dim:open"],
		code: "91721",
		label: "For irrevocable mandates to pledge goodwill, the amount for which the agent can take the inscription",
		labels: {
			de: "Im Falle von unwiderruflichen Mandaten zur Verpfändung des Firmenwertes: der Betrag, den der Bevollmächtigte aufgrund des Mandats eintragen kann",
			en: "For irrevocable mandates to pledge goodwill, the amount for which the agent can take the inscription",
			fr: "Pour les mandats irrévocables de mise en gage du fonds de commerce, le montant pour lequel le mandataire est autorisé à procéder à l'enregistrement en vertu du mandat",
			nl: "Voor de onherroepelijke mandaten tot verpanding van het handelsfonds, het bedrag waarvoor de volmachthebber krachtens het mandaat tot registratie mag overgaan",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m26",
			"dim:qlt": "qlt:m2",
			"dim:spec": "spec:m60",
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:gtd": "gtd:m3",
		},
		openDimensions: ["dim:open"],
		code: "91811",
		label: "Book value of the immovable properties mortgaged",
		labels: {
			de: "Buchwert der belasteten Aktiva",
			en: "Book value of the immovable properties mortgaged",
			fr: "La valeur comptable des actifs grevés",
			nl: "Boekwaarde van de bezwaarde activa",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m26",
			"dim:qlt": "qlt:m2",
			"dim:spec": "spec:m60",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "91811",
		label: "Book value of the immovable properties mortgaged",
		labels: {
			de: "Buchwert der belasteten Aktiva",
			en: "Book value of the immovable properties mortgaged",
			fr: "La valeur comptable des actifs grevés",
			nl: "Boekwaarde van de bezwaarde activa",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m26",
			"dim:qlt": "qlt:m2",
			"dim:spec": "spec:m60",
		},
		openDimensions: ["dim:open"],
		code: "91811",
		label: "Book value of the immovable properties mortgaged",
		labels: {
			de: "Buchwert der belasteten Aktiva",
			en: "Book value of the immovable properties mortgaged",
			fr: "La valeur comptable des actifs grevés",
			nl: "Boekwaarde van de bezwaarde activa",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m26",
			"dim:qlt": "qlt:m2",
			"dim:spec": "spec:m60",
		},
		openDimensions: ["dim:open"],
		code: "91811",
		label: "Book value of the immovable properties mortgaged",
		labels: {
			de: "Buchwert der belasteten Aktiva",
			en: "Book value of the immovable properties mortgaged",
			fr: "La valeur comptable des actifs grevés",
			nl: "Boekwaarde van de bezwaarde activa",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m26",
			"dim:spec": "spec:m65",
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:gtd": "gtd:m3",
		},
		openDimensions: ["dim:open"],
		code: "91821",
		label: "Maximum amount up to which the debt is secured",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung",
			en: "Maximum amount up to which the debt is secured",
			fr: "Le montant maximum à concurrence duquel la dette est garantie",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m26",
			"dim:spec": "spec:m65",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "91821",
		label: "Maximum amount up to which the debt is secured",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung",
			en: "Maximum amount up to which the debt is secured",
			fr: "Le montant maximum à concurrence duquel la dette est garantie",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m26",
			"dim:spec": "spec:m65",
		},
		openDimensions: ["dim:open"],
		code: "91821",
		label: "Maximum amount up to which the debt is secured",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung",
			en: "Maximum amount up to which the debt is secured",
			fr: "Le montant maximum à concurrence duquel la dette est garantie",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m26",
			"dim:spec": "spec:m65",
		},
		openDimensions: ["dim:open"],
		code: "91821",
		label: "Maximum amount up to which the debt is secured",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung",
			en: "Maximum amount up to which the debt is secured",
			fr: "Le montant maximum à concurrence duquel la dette est garantie",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m27",
			"dim:spec": "spec:m66",
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:gtd": "gtd:m3",
		},
		openDimensions: ["dim:open"],
		code: "91911",
		label: "Amount of assets in question",
		labels: {
			de: "Betrag der betreffenden Aktiva",
			en: "Amount of assets in question",
			fr: "Le montant des actifs en cause",
			nl: "Bedrag van de betrokken activa",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m27",
			"dim:spec": "spec:m66",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "91911",
		label: "Amount of assets in question",
		labels: {
			de: "Betrag der betreffenden Aktiva",
			en: "Amount of assets in question",
			fr: "Le montant des actifs en cause",
			nl: "Bedrag van de betrokken activa",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m27",
			"dim:spec": "spec:m66",
		},
		openDimensions: ["dim:open"],
		code: "91911",
		label: "Amount of assets in question",
		labels: {
			de: "Betrag der betreffenden Aktiva",
			en: "Amount of assets in question",
			fr: "Le montant des actifs en cause",
			nl: "Bedrag van de betrokken activa",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m27",
			"dim:spec": "spec:m66",
		},
		openDimensions: ["dim:open"],
		code: "91911",
		label: "Amount of assets in question",
		labels: {
			de: "Betrag der betreffenden Aktiva",
			en: "Amount of assets in question",
			fr: "Le montant des actifs en cause",
			nl: "Bedrag van de betrokken activa",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m27",
			"dim:spec": "spec:m65",
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:gtd": "gtd:m3",
		},
		openDimensions: ["dim:open"],
		code: "91921",
		label: "Maximum amount up to which the debt is secured",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung",
			en: "Maximum amount up to which the debt is secured",
			fr: "Le montant maximum à concurrence duquel la dette est garantie",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m27",
			"dim:spec": "spec:m65",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "91921",
		label: "Maximum amount up to which the debt is secured",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung",
			en: "Maximum amount up to which the debt is secured",
			fr: "Le montant maximum à concurrence duquel la dette est garantie",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m27",
			"dim:spec": "spec:m65",
		},
		openDimensions: ["dim:open"],
		code: "91921",
		label: "Maximum amount up to which the debt is secured",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung",
			en: "Maximum amount up to which the debt is secured",
			fr: "Le montant maximum à concurrence duquel la dette est garantie",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m27",
			"dim:spec": "spec:m65",
		},
		openDimensions: ["dim:open"],
		code: "91921",
		label: "Maximum amount up to which the debt is secured",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung",
			en: "Maximum amount up to which the debt is secured",
			fr: "Le montant maximum à concurrence duquel la dette est garantie",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m28",
			"dim:qlt": "qlt:m2",
			"dim:spec": "spec:m67",
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:gtd": "gtd:m3",
		},
		openDimensions: ["dim:open"],
		code: "92011",
		label: "Book value of sold goods",
		labels: {
			de: "Buchwert des verkauften Guts",
			en: "Book value of sold goods",
			fr: "La valeur comptable du bien vendu",
			nl: "Boekwaarde van het verkochte goed",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m28",
			"dim:qlt": "qlt:m2",
			"dim:spec": "spec:m67",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "92011",
		label: "Book value of sold goods",
		labels: {
			de: "Buchwert des verkauften Guts",
			en: "Book value of sold goods",
			fr: "La valeur comptable du bien vendu",
			nl: "Boekwaarde van het verkochte goed",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m28",
			"dim:qlt": "qlt:m2",
			"dim:spec": "spec:m67",
		},
		openDimensions: ["dim:open"],
		code: "92011",
		label: "Book value of sold goods",
		labels: {
			de: "Buchwert des verkauften Guts",
			en: "Book value of sold goods",
			fr: "La valeur comptable du bien vendu",
			nl: "Boekwaarde van het verkochte goed",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m28",
			"dim:qlt": "qlt:m2",
			"dim:spec": "spec:m67",
		},
		openDimensions: ["dim:open"],
		code: "92011",
		label: "Book value of sold goods",
		labels: {
			de: "Buchwert des verkauften Guts",
			en: "Book value of sold goods",
			fr: "La valeur comptable du bien vendu",
			nl: "Boekwaarde van het verkochte goed",
		},
		currentPeriod: "prd:m1",
	},
];

const datapoints1: readonly Datapoint[] = [
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m28",
			"dim:spec": "spec:m68",
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:gtd": "gtd:m3",
		},
		openDimensions: ["dim:open"],
		code: "92021",
		label: "Amount of the unpaid price",
		labels: {
			de: "Betrag des unbezahlten Preises",
			en: "Amount of the unpaid price",
			fr: "Le montant du prix non payé",
			nl: "Bedrag van de niet-betaalde prijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m28",
			"dim:spec": "spec:m68",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "92021",
		label: "Amount of the unpaid price",
		labels: {
			de: "Betrag des unbezahlten Preises",
			en: "Amount of the unpaid price",
			fr: "Le montant du prix non payé",
			nl: "Bedrag van de niet-betaalde prijs",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m28",
			"dim:spec": "spec:m68",
		},
		openDimensions: ["dim:open"],
		code: "92021",
		label: "Amount of the unpaid price",
		labels: {
			de: "Betrag des unbezahlten Preises",
			en: "Amount of the unpaid price",
			fr: "Le montant du prix non payé",
			nl: "Bedrag van de niet-betaalde prijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m25",
			"dim:typ": "typ:m28",
			"dim:spec": "spec:m68",
		},
		openDimensions: ["dim:open"],
		code: "92021",
		label: "Amount of the unpaid price",
		labels: {
			de: "Betrag des unbezahlten Preises",
			en: "Amount of the unpaid price",
			fr: "Le montant du prix non payé",
			nl: "Bedrag van de niet-betaalde prijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:qlt": "qlt:m2",
			"dim:typ": "typ:m22",
			"dim:spec": "spec:m60",
		},
		openDimensions: ["dim:open"],
		code: "91612",
		label: "Book value of the immovable properties mortgaged",
		labels: {
			de: "Buchwert der belasteten Aktiva",
			en: "Book value of the immovable properties mortgaged",
			fr: "Valeur comptable des immeubles grevés",
			nl: "Boekwaarde van de bezwaarde activa",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:qlt": "qlt:m2",
			"dim:typ": "typ:m22",
			"dim:spec": "spec:m60",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "91612",
		label: "Book value of the immovable properties mortgaged",
		labels: {
			de: "Buchwert der belasteten Aktiva",
			en: "Book value of the immovable properties mortgaged",
			fr: "Valeur comptable des immeubles grevés",
			nl: "Boekwaarde van de bezwaarde activa",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:qlt": "qlt:m2",
			"dim:typ": "typ:m22",
			"dim:spec": "spec:m60",
		},
		openDimensions: ["dim:open"],
		code: "91612",
		label: "Book value of the immovable properties mortgaged",
		labels: {
			de: "Buchwert der belasteten Aktiva",
			en: "Book value of the immovable properties mortgaged",
			fr: "Valeur comptable des immeubles grevés",
			nl: "Boekwaarde van de bezwaarde activa",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m22",
			"dim:spec": "spec:m61",
		},
		openDimensions: ["dim:open"],
		code: "91622",
		label: "Amount of registration",
		labels: {
			de: "Betrag der Eintragung",
			en: "Amount of registration",
			fr: "Montant de l'inscription",
			nl: "Bedrag van de inschrijving",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m22",
			"dim:spec": "spec:m61",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "91622",
		label: "Amount of registration",
		labels: {
			de: "Betrag der Eintragung",
			en: "Amount of registration",
			fr: "Montant de l'inscription",
			nl: "Bedrag van de inschrijving",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m22",
			"dim:spec": "spec:m61",
		},
		openDimensions: ["dim:open"],
		code: "91622",
		label: "Amount of registration",
		labels: {
			de: "Betrag der Eintragung",
			en: "Amount of registration",
			fr: "Montant de l'inscription",
			nl: "Bedrag van de inschrijving",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m23",
			"dim:spec": "spec:m62",
		},
		openDimensions: ["dim:open"],
		code: "91632",
		label: "For irrevocable mortgage mandates, the amount for which the agent can take registration",
		labels: {
			de: "Im Falle von unwiderruflichen Hypothekarmandaten: der Betrag, den der Bevollmächtigte aufgrund des Mandats eintragen kann",
			en: "For irrevocable mortgage mandates, the amount for which the agent can take registration",
			fr: "Pour les mandats irrévocables d'hypothéquer, le montant pour lequel le mandataire est autorisé à prendre inscription en vertu du mandat",
			nl: "Voor de onherroepelijke mandaten tot hypothekeren, het bedrag waarvoor de volmachthebber krachtens het mandaat inschrijving mag nemen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m23",
			"dim:spec": "spec:m62",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "91632",
		label: "For irrevocable mortgage mandates, the amount for which the agent can take registration",
		labels: {
			de: "Im Falle von unwiderruflichen Hypothekarmandaten: der Betrag, den der Bevollmächtigte aufgrund des Mandats eintragen kann",
			en: "For irrevocable mortgage mandates, the amount for which the agent can take registration",
			fr: "Pour les mandats irrévocables d'hypothéquer, le montant pour lequel le mandataire est autorisé à prendre inscription en vertu du mandat",
			nl: "Voor de onherroepelijke mandaten tot hypothekeren, het bedrag waarvoor de volmachthebber krachtens het mandaat inschrijving mag nemen",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m23",
			"dim:spec": "spec:m62",
		},
		openDimensions: ["dim:open"],
		code: "91632",
		label: "For irrevocable mortgage mandates, the amount for which the agent can take registration",
		labels: {
			de: "Im Falle von unwiderruflichen Hypothekarmandaten: der Betrag, den der Bevollmächtigte aufgrund des Mandats eintragen kann",
			en: "For irrevocable mortgage mandates, the amount for which the agent can take registration",
			fr: "Pour les mandats irrévocables d'hypothéquer, le montant pour lequel le mandataire est autorisé à prendre inscription en vertu du mandat",
			nl: "Voor de onherroepelijke mandaten tot hypothekeren, het bedrag waarvoor de volmachthebber krachtens het mandaat inschrijving mag nemen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m24",
			"dim:spec": "spec:m63",
		},
		openDimensions: ["dim:open"],
		code: "91712",
		label: "Maximum amount up to which the debt is secured and which is the subject of registration",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung und der Eintragung",
			en: "Maximum amount up to which the debt is secured and which is the subject of registration",
			fr: "Le montant maximum à concurrence duquel la dette est garantie et qui fait l'objet de l'enregistrement",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd en waarvoor registratie plaatsvindt",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m24",
			"dim:spec": "spec:m63",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "91712",
		label: "Maximum amount up to which the debt is secured and which is the subject of registration",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung und der Eintragung",
			en: "Maximum amount up to which the debt is secured and which is the subject of registration",
			fr: "Le montant maximum à concurrence duquel la dette est garantie et qui fait l'objet de l'enregistrement",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd en waarvoor registratie plaatsvindt",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m24",
			"dim:spec": "spec:m63",
		},
		openDimensions: ["dim:open"],
		code: "91712",
		label: "Maximum amount up to which the debt is secured and which is the subject of registration",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung und der Eintragung",
			en: "Maximum amount up to which the debt is secured and which is the subject of registration",
			fr: "Le montant maximum à concurrence duquel la dette est garantie et qui fait l'objet de l'enregistrement",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd en waarvoor registratie plaatsvindt",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m25",
			"dim:spec": "spec:m64",
		},
		openDimensions: ["dim:open"],
		code: "91722",
		label: "For irrevocable mandates to pledge goodwill, the amount for which the agent can take the inscription",
		labels: {
			de: "Im Falle von unwiderruflichen Mandaten zur Verpfändung des Firmenwertes: der Betrag, den der Bevollmächtigte aufgrund des Mandats eintragen kann",
			en: "For irrevocable mandates to pledge goodwill, the amount for which the agent can take the inscription",
			fr: "Pour les mandats irrévocables de mise en gage du fonds de commerce, le montant pour lequel le mandataire est autorisé à procéder à l'enregistrement en vertu du mandat",
			nl: "Voor de onherroepelijke mandaten tot verpanding van het handelsfonds, het bedrag waarvoor de volmachthebber krachtens het mandaat tot registratie mag overgaan",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m25",
			"dim:spec": "spec:m64",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "91722",
		label: "For irrevocable mandates to pledge goodwill, the amount for which the agent can take the inscription",
		labels: {
			de: "Im Falle von unwiderruflichen Mandaten zur Verpfändung des Firmenwertes: der Betrag, den der Bevollmächtigte aufgrund des Mandats eintragen kann",
			en: "For irrevocable mandates to pledge goodwill, the amount for which the agent can take the inscription",
			fr: "Pour les mandats irrévocables de mise en gage du fonds de commerce, le montant pour lequel le mandataire est autorisé à procéder à l'enregistrement en vertu du mandat",
			nl: "Voor de onherroepelijke mandaten tot verpanding van het handelsfonds, het bedrag waarvoor de volmachthebber krachtens het mandaat tot registratie mag overgaan",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m25",
			"dim:spec": "spec:m64",
		},
		openDimensions: ["dim:open"],
		code: "91722",
		label: "For irrevocable mandates to pledge goodwill, the amount for which the agent can take the inscription",
		labels: {
			de: "Im Falle von unwiderruflichen Mandaten zur Verpfändung des Firmenwertes: der Betrag, den der Bevollmächtigte aufgrund des Mandats eintragen kann",
			en: "For irrevocable mandates to pledge goodwill, the amount for which the agent can take the inscription",
			fr: "Pour les mandats irrévocables de mise en gage du fonds de commerce, le montant pour lequel le mandataire est autorisé à procéder à l'enregistrement en vertu du mandat",
			nl: "Voor de onherroepelijke mandaten tot verpanding van het handelsfonds, het bedrag waarvoor de volmachthebber krachtens het mandaat tot registratie mag overgaan",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m26",
			"dim:qlt": "qlt:m2",
			"dim:spec": "spec:m60",
		},
		openDimensions: ["dim:open"],
		code: "91812",
		label: "Book value of the immovable properties mortgaged",
		labels: {
			de: "Buchwert der belasteten Aktiva",
			en: "Book value of the immovable properties mortgaged",
			fr: "La valeur comptable des actifs grevés",
			nl: "Boekwaarde van de bezwaarde activa",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m26",
			"dim:qlt": "qlt:m2",
			"dim:spec": "spec:m60",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "91812",
		label: "Book value of the immovable properties mortgaged",
		labels: {
			de: "Buchwert der belasteten Aktiva",
			en: "Book value of the immovable properties mortgaged",
			fr: "La valeur comptable des actifs grevés",
			nl: "Boekwaarde van de bezwaarde activa",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m26",
			"dim:qlt": "qlt:m2",
			"dim:spec": "spec:m60",
		},
		openDimensions: ["dim:open"],
		code: "91812",
		label: "Book value of the immovable properties mortgaged",
		labels: {
			de: "Buchwert der belasteten Aktiva",
			en: "Book value of the immovable properties mortgaged",
			fr: "La valeur comptable des actifs grevés",
			nl: "Boekwaarde van de bezwaarde activa",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m26",
			"dim:spec": "spec:m65",
		},
		openDimensions: ["dim:open"],
		code: "91822",
		label: "Maximum amount up to which the debt is secured",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung",
			en: "Maximum amount up to which the debt is secured",
			fr: "Le montant maximum à concurrence duquel la dette est garantie",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m26",
			"dim:spec": "spec:m65",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "91822",
		label: "Maximum amount up to which the debt is secured",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung",
			en: "Maximum amount up to which the debt is secured",
			fr: "Le montant maximum à concurrence duquel la dette est garantie",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m26",
			"dim:spec": "spec:m65",
		},
		openDimensions: ["dim:open"],
		code: "91822",
		label: "Maximum amount up to which the debt is secured",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung",
			en: "Maximum amount up to which the debt is secured",
			fr: "Le montant maximum à concurrence duquel la dette est garantie",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m27",
			"dim:spec": "spec:m66",
		},
		openDimensions: ["dim:open"],
		code: "91912",
		label: "Amount of assets in question",
		labels: {
			de: "Betrag der betreffenden Aktiva",
			en: "Amount of assets in question",
			fr: "Le montant des actifs en cause",
			nl: "Bedrag van de betrokken activa",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m27",
			"dim:spec": "spec:m66",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "91912",
		label: "Amount of assets in question",
		labels: {
			de: "Betrag der betreffenden Aktiva",
			en: "Amount of assets in question",
			fr: "Le montant des actifs en cause",
			nl: "Bedrag van de betrokken activa",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m27",
			"dim:spec": "spec:m66",
		},
		openDimensions: ["dim:open"],
		code: "91912",
		label: "Amount of assets in question",
		labels: {
			de: "Betrag der betreffenden Aktiva",
			en: "Amount of assets in question",
			fr: "Le montant des actifs en cause",
			nl: "Bedrag van de betrokken activa",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m27",
			"dim:spec": "spec:m65",
		},
		openDimensions: ["dim:open"],
		code: "91922",
		label: "Maximum amount up to which the debt is secured",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung",
			en: "Maximum amount up to which the debt is secured",
			fr: "Le montant maximum à concurrence duquel la dette est garantie",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m27",
			"dim:spec": "spec:m65",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "91922",
		label: "Maximum amount up to which the debt is secured",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung",
			en: "Maximum amount up to which the debt is secured",
			fr: "Le montant maximum à concurrence duquel la dette est garantie",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m27",
			"dim:spec": "spec:m65",
		},
		openDimensions: ["dim:open"],
		code: "91922",
		label: "Maximum amount up to which the debt is secured",
		labels: {
			de: "Höchstbetrag der Schuldbesicherung",
			en: "Maximum amount up to which the debt is secured",
			fr: "Le montant maximum à concurrence duquel la dette est garantie",
			nl: "Maximumbedrag waarvoor de schuld is gewaarborgd",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m28",
			"dim:qlt": "qlt:m2",
			"dim:spec": "spec:m67",
		},
		openDimensions: ["dim:open"],
		code: "92012",
		label: "Book value of sold goods",
		labels: {
			de: "Buchwert des verkauften Guts",
			en: "Book value of sold goods",
			fr: "La valeur comptable du bien vendu",
			nl: "Boekwaarde van het verkochte goed",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m28",
			"dim:qlt": "qlt:m2",
			"dim:spec": "spec:m67",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "92012",
		label: "Book value of sold goods",
		labels: {
			de: "Buchwert des verkauften Guts",
			en: "Book value of sold goods",
			fr: "La valeur comptable du bien vendu",
			nl: "Boekwaarde van het verkochte goed",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m28",
			"dim:qlt": "qlt:m2",
			"dim:spec": "spec:m67",
		},
		openDimensions: ["dim:open"],
		code: "92012",
		label: "Book value of sold goods",
		labels: {
			de: "Buchwert des verkauften Guts",
			en: "Book value of sold goods",
			fr: "La valeur comptable du bien vendu",
			nl: "Boekwaarde van het verkochte goed",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m28",
			"dim:spec": "spec:m68",
		},
		openDimensions: ["dim:open"],
		code: "92022",
		label: "Amount of the unpaid price",
		labels: {
			de: "Betrag des unbezahlten Preises",
			en: "Amount of the unpaid price",
			fr: "Le montant du prix non payé",
			nl: "Bedrag van de niet-betaalde prijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m28",
			"dim:spec": "spec:m68",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "92022",
		label: "Amount of the unpaid price",
		labels: {
			de: "Betrag des unbezahlten Preises",
			en: "Amount of the unpaid price",
			fr: "Le montant du prix non payé",
			nl: "Bedrag van de niet-betaalde prijs",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:ntr": "ntr:m13",
			"dim:spu": "spu:m17",
			"dim:gtd": "gtd:m3",
			"dim:typ": "typ:m28",
			"dim:spec": "spec:m68",
		},
		openDimensions: ["dim:open"],
		code: "92022",
		label: "Amount of the unpaid price",
		labels: {
			de: "Betrag des unbezahlten Preises",
			en: "Amount of the unpaid price",
			fr: "Le montant du prix non payé",
			nl: "Bedrag van de niet-betaalde prijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:dcl": "dcl:m16",
			"dim:bas": "bas:m87",
			"dim:gtd": "gtd:m3",
		},
		openDimensions: ["dim:open"],
		label: "Brief description",
		labels: {
			de: "Kurze Darstellung",
			en: "Brief description",
			fr: "Description succincte",
			nl: "Beknopte beschrijving",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:dcl": "dcl:m16",
			"dim:bas": "bas:m92",
		},
		openDimensions: ["dim:open"],
		label: "Brief description",
		labels: {
			de: "Kurze Darstellung",
			en: "Brief description",
			fr: "Description succincte",
			nl: "Beknopte beschrijving",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:dcl": "dcl:m16",
			"dim:bas": "bas:m92",
		},
		openDimensions: ["dim:open"],
		label: "Brief description",
		labels: {
			de: "Kurze Darstellung",
			en: "Brief description",
			fr: "Description succincte",
			nl: "Beknopte beschrijving",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:dcl": "dcl:m16",
			"dim:bas": "bas:m92",
		},
		openDimensions: ["dim:open"],
		label: "Brief description",
		labels: {
			de: "Kurze Darstellung",
			en: "Brief description",
			fr: "Description succincte",
			nl: "Beknopte beschrijving",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:dcl": "dcl:m16",
			"dim:bas": "bas:m87",
			"dim:spec": "spec:m72",
			"dim:gtd": "gtd:m3",
		},
		openDimensions: ["dim:open"],
		label: "Measures taken by the enterprise to cover the resulting charges",
		labels: {
			de: "Getroffene Maßnahmen zur Deckung der sich hieraus ergebenden Aufwendungen",
			en: "Measures taken by the enterprise to cover the resulting charges",
			fr: "Mesures prises pour en couvrir la charge",
			nl: "Genomen maatregelen om de daaruit voortvloeiende kosten te dekken",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:dcl": "dcl:m16",
			"dim:bas": "bas:m93",
			"dim:spec": "spec:m72",
		},
		openDimensions: ["dim:open"],
		label: "Measures taken by the enterprise to cover the resulting charges",
		labels: {
			de: "Getroffene Maßnahmen zur Deckung der sich hieraus ergebenden Aufwendungen",
			en: "Measures taken by the enterprise to cover the resulting charges",
			fr: "Mesures prises pour en couvrir la charge",
			nl: "Genomen maatregelen om de daaruit voortvloeiende kosten te dekken",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m6",
			"dim:dcl": "dcl:m16",
			"dim:bas": "bas:m93",
			"dim:spec": "spec:m72",
		},
		openDimensions: ["dim:open"],
		label: "Measures taken by the enterprise to cover the resulting charges",
		labels: {
			de: "Getroffene Maßnahmen zur Deckung der sich hieraus ergebenden Aufwendungen",
			en: "Measures taken by the enterprise to cover the resulting charges",
			fr: "Mesures prises pour en couvrir la charge",
			nl: "Genomen maatregelen om de daaruit voortvloeiende kosten te dekken",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m6",
			"dim:dcl": "dcl:m16",
			"dim:bas": "bas:m93",
			"dim:spec": "spec:m72",
		},
		openDimensions: ["dim:open"],
		label: "Measures taken by the enterprise to cover the resulting charges",
		labels: {
			de: "Getroffene Maßnahmen zur Deckung der sich hieraus ergebenden Aufwendungen",
			en: "Measures taken by the enterprise to cover the resulting charges",
			fr: "Mesures prises pour en couvrir la charge",
			nl: "Genomen maatregelen om de daaruit voortvloeiende kosten te dekken",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:spec": "spec:m97",
			"dim:gtd": "gtd:m3",
		},
		openDimensions: ["dim:open"],
		code: "9220",
		label: "Estimated amount of the commitments resulting from past services",
		labels: {
			de: "Geschätzter Betrag für die sich aus schon geleisteter Arbeit ergebenden Verpflichtungen",
			en: "Estimated amount of the commitments resulting from past services",
			fr: "Montant estimé des engagements résultant de prestations déjà effectuées",
			nl: "Geschat bedrag van de verplichtingen die voortvloeien uit reeds gepresteerd werk",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m94",
			"dim:part": "part:m6",
			"dim:spec": "spec:m97",
		},
		openDimensions: ["dim:open"],
		code: "9220",
		label: "Estimated amount of the commitments resulting from past services",
		labels: {
			de: "Geschätzter Betrag für die sich aus schon geleisteter Arbeit ergebenden Verpflichtungen",
			en: "Estimated amount of the commitments resulting from past services",
			fr: "Montant estimé des engagements résultant de prestations déjà effectuées",
			nl: "Geschat bedrag van de verplichtingen die voortvloeien uit reeds gepresteerd werk",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:bas": "bas:m94",
			"dim:part": "part:m6",
			"dim:spec": "spec:m97",
			"dim:dcl": "dcl:m16",
		},
		openDimensions: ["dim:open"],
		code: "9220",
		label: "Estimated amount of the commitments resulting from past services",
		labels: {
			de: "Geschätzter Betrag für die sich aus schon geleisteter Arbeit ergebenden Verpflichtungen",
			en: "Estimated amount of the commitments resulting from past services",
			fr: "Montant estimé des engagements résultant de prestations déjà effectuées",
			nl: "Geschat bedrag van de verplichtingen die voortvloeien uit reeds gepresteerd werk",
		},
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m94",
			"dim:part": "part:m6",
			"dim:spec": "spec:m97",
		},
		openDimensions: ["dim:open"],
		code: "9220",
		label: "Estimated amount of the commitments resulting from past services",
		labels: {
			de: "Geschätzter Betrag für die sich aus schon geleisteter Arbeit ergebenden Verpflichtungen",
			en: "Estimated amount of the commitments resulting from past services",
			fr: "Montant estimé des engagements résultant de prestations déjà effectuées",
			nl: "Geschat bedrag van de verplichtingen die voortvloeien uit reeds gepresteerd werk",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m87",
			"dim:part": "part:m6",
			"dim:dcl": "dcl:m16",
			"dim:spec": "spec:m97",
			"dim:gtd": "gtd:m3",
		},
		openDimensions: ["dim:open"],
		label: "Methods of estimation",
		labels: {
			de: "Berechnungsgrundlage und -weise",
			en: "Methods of estimation",
			fr: "Bases et méthodes de cette estimation",
			nl: "Basis en wijze waarop dit bedrag wordt berekend",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "str1",
		dimensions: {
			"dim:bas": "bas:m95",
			"dim:part": "part:m6",
			"dim:dcl": "dcl:m16",
			"dim:spec": "spec:m97",
		},
		openDimensions: ["dim:open"],
		label: "Methods of estimation",
		labels: {
			de: "Berechnungsgrundlage und -weise",
			en: "Methods of estimation",
			fr: "Bases et méthodes de cette estimation",
			nl: "Basis en wijze waarop dit bedrag wordt berekend",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m95",
			"dim:part": "part:m6",
			"dim:dcl": "dcl:m16",
			"dim:spec": "spec:m97",
		},
		openDimensions: ["dim:open"],
		label: "Methods of estimation",
		labels: {
			de: "Berechnungsgrundlage und -weise",
			en: "Methods of estimation",
			fr: "Bases et méthodes de cette estimation",
			nl: "Basis en wijze waarop dit bedrag wordt berekend",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.02.0.i",
		metric: "am2",
		dimensions: {
			"dim:bas": "bas:m95",
			"dim:part": "part:m6",
			"dim:dcl": "dcl:m16",
			"dim:spec": "spec:m97",
		},
		openDimensions: ["dim:open"],
		label: "Methods of estimation",
		labels: {
			de: "Berechnungsgrundlage und -weise",
			en: "Methods of estimation",
			fr: "Bases et méthodes de cette estimation",
			nl: "Basis en wijze waarop dit bedrag wordt berekend",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.03.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m9",
			"dim:psn": "psn:m31",
			"dim:ntr": "ntr:m3",
			"dim:spec": "spec:m74",
			"dim:part": "part:m6",
		},
		code: "9500",
		label: "Amounts receivable from these persons",
		labels: {
			de: "Forderungen an obengenannte Personen",
			en: "Amounts receivable from these persons",
			fr: "Créances sur les personnes précitées",
			nl: "Uitstaande vorderingen op deze personen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.03.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m9",
			"dim:part": "part:m6",
			"dim:psn": "psn:m31",
			"dim:dcl": "dcl:m18",
			"dim:spec": "spec:m74",
		},
		label: "Conditions on amounts receivable, rate, duration, possibly reimbursed amounts, canceled amounts or renounced amounts",
		labels: {
			de: "Wichtigste Bedingungen hinsichtlich Forderungen, Zinssatz, Laufzeit, gegebenenfalls abgelöste oder abgeschriebene Beträge oder Beträge auf die verzichtet wurde",
			en: "Conditions on amounts receivable, rate, duration, possibly reimbursed amounts, canceled amounts or renounced amounts",
			fr: "Conditions principales des créances, taux d'intérêt, durée, montants éventuellement remboursés, annulés ou auxquels il a été renoncé",
			nl: "Voornaamste voorwaarden betreffende de vorderingen, interestvoet, looptijd, eventueel afgeloste of afgeschreven bedragen of bedragen waarvan werd afgezien",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.03.0.i",
		metric: "str1",
		dimensions: {
			"dim:bas": "bas:m9",
			"dim:part": "part:m6",
			"dim:psn": "psn:m31",
			"dim:dcl": "dcl:m18",
			"dim:spec": "spec:m74",
		},
		label: "Conditions on amounts receivable, rate, duration, possibly reimbursed amounts, canceled amounts or renounced amounts",
		labels: {
			de: "Wichtigste Bedingungen hinsichtlich Forderungen, Zinssatz, Laufzeit, gegebenenfalls abgelöste oder abgeschriebene Beträge oder Beträge auf die verzichtet wurde",
			en: "Conditions on amounts receivable, rate, duration, possibly reimbursed amounts, canceled amounts or renounced amounts",
			fr: "Conditions principales des créances, taux d'intérêt, durée, montants éventuellement remboursés, annulés ou auxquels il a été renoncé",
			nl: "Voornaamste voorwaarden betreffende de vorderingen, interestvoet, looptijd, eventueel afgeloste of afgeschreven bedragen of bedragen waarvan werd afgezien",
		},
	},
	{
		section: "s.06.03.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m102",
			"dim:psn": "psn:m31",
			"dim:gtd": "gtd:m6",
			"dim:bnf": "bnf:m6",
			"dim:part": "part:m6",
		},
		code: "9501",
		label: "Guarantees provided in their favour",
		labels: {
			de: "Zu ihren Gunsten gestellte Sicherheiten",
			en: "Guarantees provided in their favour",
			fr: "Garanties constituées en leur faveur",
			nl: "Waarborgen toegestaan in hun voordeel",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.03.0.i",
		metric: "am1",
		dimensions: {
			"dim:bas": "bas:m89",
			"dim:psn": "psn:m31",
			"dim:typ": "typ:m2",
			"dim:sts": "sts:m23",
			"dim:gtd": "gtd:m6",
			"dim:bnf": "bnf:m6",
			"dim:part": "part:m6",
		},
		code: "9502",
		label: "Other significant commitments undertaken in their favour",
		labels: {
			de: "Sonstige wesentliche Verpflichtungen, die zu ihren Gunsten eingegangen wurden",
			en: "Other significant commitments undertaken in their favour",
			fr: "Autres engagements significatifs souscrits en leur faveur",
			nl: "Andere betekenisvolle verplichtingen aangegaan in hun voordeel",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m73",
			"dim:typ": "typ:m16",
		},
		openDimensions: ["dim:open"],
		code: "1001-1002-1003-1003P",
		label: "Average number of employees",
		labels: {
			de: "Durchschnittliche Anzahl der Arbeitnehmer",
			en: "Average number of employees",
			fr: "Nombre moyen de travailleurs",
			nl: "Gemiddeld aantal werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m73",
			"dim:typ": "typ:m16",
		},
		openDimensions: ["dim:open"],
		code: "1001-1002-1003-1003P",
		label: "Average number of employees",
		labels: {
			de: "Durchschnittliche Anzahl der Arbeitnehmer",
			en: "Average number of employees",
			fr: "Nombre moyen de travailleurs",
			nl: "Gemiddeld aantal werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m73",
			"dim:typ": "typ:m16",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "1001-1002-1003-1003P",
		label: "Average number of employees",
		labels: {
			de: "Durchschnittliche Anzahl der Arbeitnehmer",
			en: "Average number of employees",
			fr: "Nombre moyen de travailleurs",
			nl: "Gemiddeld aantal werknemers",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m73",
			"dim:typ": "typ:m16",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "1001-1002-1003-1003P",
		label: "Average number of employees",
		labels: {
			de: "Durchschnittliche Anzahl der Arbeitnehmer",
			en: "Average number of employees",
			fr: "Nombre moyen de travailleurs",
			nl: "Gemiddeld aantal werknemers",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m73",
			"dim:typ": "typ:m16",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "1001-1002-1003-1003P",
		label: "Average number of employees",
		labels: {
			de: "Durchschnittliche Anzahl der Arbeitnehmer",
			en: "Average number of employees",
			fr: "Nombre moyen de travailleurs",
			nl: "Gemiddeld aantal werknemers",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m73",
			"dim:typ": "typ:m16",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "1001-1002-1003-1003P",
		label: "Average number of employees",
		labels: {
			de: "Durchschnittliche Anzahl der Arbeitnehmer",
			en: "Average number of employees",
			fr: "Nombre moyen de travailleurs",
			nl: "Gemiddeld aantal werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m73",
			"dim:typ": "typ:m16",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "1001-1002-1003-1003P",
		label: "Average number of employees",
		labels: {
			de: "Durchschnittliche Anzahl der Arbeitnehmer",
			en: "Average number of employees",
			fr: "Nombre moyen de travailleurs",
			nl: "Gemiddeld aantal werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m73",
			"dim:typ": "typ:m16",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "1001-1002-1003-1003P",
		label: "Average number of employees",
		labels: {
			de: "Durchschnittliche Anzahl der Arbeitnehmer",
			en: "Average number of employees",
			fr: "Nombre moyen de travailleurs",
			nl: "Gemiddeld aantal werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m2",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m73",
			"dim:typ": "typ:m16",
		},
		openDimensions: ["dim:open"],
		code: "1001-1002-1003-1003P",
		label: "Average number of employees",
		labels: {
			de: "Durchschnittliche Anzahl der Arbeitnehmer",
			en: "Average number of employees",
			fr: "Nombre moyen de travailleurs",
			nl: "Gemiddeld aantal werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m3",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m73",
			"dim:typ": "typ:m16",
		},
		openDimensions: ["dim:open"],
		code: "1001-1002-1003-1003P",
		label: "Average number of employees",
		labels: {
			de: "Durchschnittliche Anzahl der Arbeitnehmer",
			en: "Average number of employees",
			fr: "Nombre moyen de travailleurs",
			nl: "Gemiddeld aantal werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m1",
		},
		openDimensions: ["dim:open"],
		code: "1011-1012-1013-1013P",
		label: "Number of hours actually worked",
		labels: {
			de: "Anzahl der tatsächlich geleisteten Arbeitsstunden",
			en: "Number of hours actually worked",
			fr: "Nombre d'heures effectivement prestées",
			nl: "Aantal daadwerkelijk gepresteerde uren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m1",
		},
		openDimensions: ["dim:open"],
		code: "1011-1012-1013-1013P",
		label: "Number of hours actually worked",
		labels: {
			de: "Anzahl der tatsächlich geleisteten Arbeitsstunden",
			en: "Number of hours actually worked",
			fr: "Nombre d'heures effectivement prestées",
			nl: "Aantal daadwerkelijk gepresteerde uren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int4",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m1",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "1011-1012-1013-1013P",
		label: "Number of hours actually worked",
		labels: {
			de: "Anzahl der tatsächlich geleisteten Arbeitsstunden",
			en: "Number of hours actually worked",
			fr: "Nombre d'heures effectivement prestées",
			nl: "Aantal daadwerkelijk gepresteerde uren",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int4",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m1",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "1011-1012-1013-1013P",
		label: "Number of hours actually worked",
		labels: {
			de: "Anzahl der tatsächlich geleisteten Arbeitsstunden",
			en: "Number of hours actually worked",
			fr: "Nombre d'heures effectivement prestées",
			nl: "Aantal daadwerkelijk gepresteerde uren",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int4",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m1",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "1011-1012-1013-1013P",
		label: "Number of hours actually worked",
		labels: {
			de: "Anzahl der tatsächlich geleisteten Arbeitsstunden",
			en: "Number of hours actually worked",
			fr: "Nombre d'heures effectivement prestées",
			nl: "Aantal daadwerkelijk gepresteerde uren",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m1",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "1011-1012-1013-1013P",
		label: "Number of hours actually worked",
		labels: {
			de: "Anzahl der tatsächlich geleisteten Arbeitsstunden",
			en: "Number of hours actually worked",
			fr: "Nombre d'heures effectivement prestées",
			nl: "Aantal daadwerkelijk gepresteerde uren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m1",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "1011-1012-1013-1013P",
		label: "Number of hours actually worked",
		labels: {
			de: "Anzahl der tatsächlich geleisteten Arbeitsstunden",
			en: "Number of hours actually worked",
			fr: "Nombre d'heures effectivement prestées",
			nl: "Aantal daadwerkelijk gepresteerde uren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m1",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "1011-1012-1013-1013P",
		label: "Number of hours actually worked",
		labels: {
			de: "Anzahl der tatsächlich geleisteten Arbeitsstunden",
			en: "Number of hours actually worked",
			fr: "Nombre d'heures effectivement prestées",
			nl: "Aantal daadwerkelijk gepresteerde uren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int4",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m2",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m1",
		},
		openDimensions: ["dim:open"],
		code: "1011-1012-1013-1013P",
		label: "Number of hours actually worked",
		labels: {
			de: "Anzahl der tatsächlich geleisteten Arbeitsstunden",
			en: "Number of hours actually worked",
			fr: "Nombre d'heures effectivement prestées",
			nl: "Aantal daadwerkelijk gepresteerde uren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int4",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m3",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m1",
		},
		openDimensions: ["dim:open"],
		code: "1011-1012-1013-1013P",
		label: "Number of hours actually worked",
		labels: {
			de: "Anzahl der tatsächlich geleisteten Arbeitsstunden",
			en: "Number of hours actually worked",
			fr: "Nombre d'heures effectivement prestées",
			nl: "Aantal daadwerkelijk gepresteerde uren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m7",
		},
		openDimensions: ["dim:open"],
		code: "1021-1022-1023-1023P",
		label: "Personnel costs",
		labels: {
			de: "Personalaufwand",
			en: "Personnel costs",
			fr: "Frais de personnel",
			nl: "Personeelskosten",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m7",
		},
		openDimensions: ["dim:open"],
		code: "1021-1022-1023-1023P",
		label: "Personnel costs",
		labels: {
			de: "Personalaufwand",
			en: "Personnel costs",
			fr: "Frais de personnel",
			nl: "Personeelskosten",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m7",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "1021-1022-1023-1023P",
		label: "Personnel costs",
		labels: {
			de: "Personalaufwand",
			en: "Personnel costs",
			fr: "Frais de personnel",
			nl: "Personeelskosten",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m7",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "1021-1022-1023-1023P",
		label: "Personnel costs",
		labels: {
			de: "Personalaufwand",
			en: "Personnel costs",
			fr: "Frais de personnel",
			nl: "Personeelskosten",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m7",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "1021-1022-1023-1023P",
		label: "Personnel costs",
		labels: {
			de: "Personalaufwand",
			en: "Personnel costs",
			fr: "Frais de personnel",
			nl: "Personeelskosten",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m7",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "1021-1022-1023-1023P",
		label: "Personnel costs",
		labels: {
			de: "Personalaufwand",
			en: "Personnel costs",
			fr: "Frais de personnel",
			nl: "Personeelskosten",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m7",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "1021-1022-1023-1023P",
		label: "Personnel costs",
		labels: {
			de: "Personalaufwand",
			en: "Personnel costs",
			fr: "Frais de personnel",
			nl: "Personeelskosten",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m1",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m7",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "1021-1022-1023-1023P",
		label: "Personnel costs",
		labels: {
			de: "Personalaufwand",
			en: "Personnel costs",
			fr: "Frais de personnel",
			nl: "Personeelskosten",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m2",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m7",
		},
		openDimensions: ["dim:open"],
		code: "1021-1022-1023-1023P",
		label: "Personnel costs",
		labels: {
			de: "Personalaufwand",
			en: "Personnel costs",
			fr: "Frais de personnel",
			nl: "Personeelskosten",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am2",
		dimensions: {
			"dim:part": "part:m8",
			"dim:gdr": "gdr:m3",
			"dim:wkr": "wkr:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m7",
		},
		openDimensions: ["dim:open"],
		code: "1021-1022-1023-1023P",
		label: "Personnel costs",
		labels: {
			de: "Personalaufwand",
			en: "Personnel costs",
			fr: "Frais de personnel",
			nl: "Personeelskosten",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
		},
		openDimensions: ["dim:open"],
		code: "1051-1052-1053",
		label: "Number of employees",
		labels: {
			de: "Anzahl der Arbeitnehmer",
			en: "Number of employees",
			fr: "Nombre de travailleurs",
			nl: "Aantal werknemers",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
		},
		openDimensions: ["dim:open"],
		code: "1051-1052-1053",
		label: "Number of employees",
		labels: {
			de: "Anzahl der Arbeitnehmer",
			en: "Number of employees",
			fr: "Nombre de travailleurs",
			nl: "Aantal werknemers",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "1051-1052-1053",
		label: "Number of employees",
		labels: {
			de: "Anzahl der Arbeitnehmer",
			en: "Number of employees",
			fr: "Nombre de travailleurs",
			nl: "Aantal werknemers",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "1051-1052-1053",
		label: "Number of employees",
		labels: {
			de: "Anzahl der Arbeitnehmer",
			en: "Number of employees",
			fr: "Nombre de travailleurs",
			nl: "Aantal werknemers",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "1051-1052-1053",
		label: "Number of employees",
		labels: {
			de: "Anzahl der Arbeitnehmer",
			en: "Number of employees",
			fr: "Nombre de travailleurs",
			nl: "Aantal werknemers",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:epc": "epc:m1",
		},
		openDimensions: ["dim:open"],
		code: "1101-1102-1103",
		label: "Contract for an indefinite period",
		labels: {
			de: "Unbefristeter Vertrag",
			en: "Contract for an indefinite period",
			fr: "Contrat à durée indéterminée",
			nl: "Overeenkomst voor een onbepaalde tijd",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:epc": "epc:m1",
		},
		openDimensions: ["dim:open"],
		code: "1101-1102-1103",
		label: "Contract for an indefinite period",
		labels: {
			de: "Unbefristeter Vertrag",
			en: "Contract for an indefinite period",
			fr: "Contrat à durée indéterminée",
			nl: "Overeenkomst voor een onbepaalde tijd",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:epc": "epc:m1",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "1101-1102-1103",
		label: "Contract for an indefinite period",
		labels: {
			de: "Unbefristeter Vertrag",
			en: "Contract for an indefinite period",
			fr: "Contrat à durée indéterminée",
			nl: "Overeenkomst voor een onbepaalde tijd",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:epc": "epc:m1",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "1101-1102-1103",
		label: "Contract for an indefinite period",
		labels: {
			de: "Unbefristeter Vertrag",
			en: "Contract for an indefinite period",
			fr: "Contrat à durée indéterminée",
			nl: "Overeenkomst voor een onbepaalde tijd",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:epc": "epc:m1",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "1101-1102-1103",
		label: "Contract for an indefinite period",
		labels: {
			de: "Unbefristeter Vertrag",
			en: "Contract for an indefinite period",
			fr: "Contrat à durée indéterminée",
			nl: "Overeenkomst voor een onbepaalde tijd",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:epc": "epc:m2",
		},
		openDimensions: ["dim:open"],
		code: "1111-1112-1113",
		label: "Contract for a definite period",
		labels: {
			de: "Befristeter Vertrag",
			en: "Contract for a definite period",
			fr: "Contrat à durée déterminée",
			nl: "Overeenkomst voor een bepaalde tijd",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:epc": "epc:m2",
		},
		openDimensions: ["dim:open"],
		code: "1111-1112-1113",
		label: "Contract for a definite period",
		labels: {
			de: "Befristeter Vertrag",
			en: "Contract for a definite period",
			fr: "Contrat à durée déterminée",
			nl: "Overeenkomst voor een bepaalde tijd",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:epc": "epc:m2",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "1111-1112-1113",
		label: "Contract for a definite period",
		labels: {
			de: "Befristeter Vertrag",
			en: "Contract for a definite period",
			fr: "Contrat à durée déterminée",
			nl: "Overeenkomst voor een bepaalde tijd",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:epc": "epc:m2",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "1111-1112-1113",
		label: "Contract for a definite period",
		labels: {
			de: "Befristeter Vertrag",
			en: "Contract for a definite period",
			fr: "Contrat à durée déterminée",
			nl: "Overeenkomst voor een bepaalde tijd",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:epc": "epc:m2",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "1111-1112-1113",
		label: "Contract for a definite period",
		labels: {
			de: "Befristeter Vertrag",
			en: "Contract for a definite period",
			fr: "Contrat à durée déterminée",
			nl: "Overeenkomst voor een bepaalde tijd",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:epc": "epc:m3",
		},
		openDimensions: ["dim:open"],
		code: "1121-1122-1123",
		label: "Contract for the execution of a specifically assigned work",
		labels: {
			de: "Vertrag zur Durchführung einer genau bestimmten Arbeit",
			en: "Contract for the execution of a specifically assigned work",
			fr: "Contrat pour l'exécution d'un travail nettement défini",
			nl: "Overeenkomst voor een duidelijk omschreven werk",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:epc": "epc:m3",
		},
		openDimensions: ["dim:open"],
		code: "1121-1122-1123",
		label: "Contract for the execution of a specifically assigned work",
		labels: {
			de: "Vertrag zur Durchführung einer genau bestimmten Arbeit",
			en: "Contract for the execution of a specifically assigned work",
			fr: "Contrat pour l'exécution d'un travail nettement défini",
			nl: "Overeenkomst voor een duidelijk omschreven werk",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:epc": "epc:m3",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "1121-1122-1123",
		label: "Contract for the execution of a specifically assigned work",
		labels: {
			de: "Vertrag zur Durchführung einer genau bestimmten Arbeit",
			en: "Contract for the execution of a specifically assigned work",
			fr: "Contrat pour l'exécution d'un travail nettement défini",
			nl: "Overeenkomst voor een duidelijk omschreven werk",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:epc": "epc:m3",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "1121-1122-1123",
		label: "Contract for the execution of a specifically assigned work",
		labels: {
			de: "Vertrag zur Durchführung einer genau bestimmten Arbeit",
			en: "Contract for the execution of a specifically assigned work",
			fr: "Contrat pour l'exécution d'un travail nettement défini",
			nl: "Overeenkomst voor een duidelijk omschreven werk",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:epc": "epc:m3",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "1121-1122-1123",
		label: "Contract for the execution of a specifically assigned work",
		labels: {
			de: "Vertrag zur Durchführung einer genau bestimmten Arbeit",
			en: "Contract for the execution of a specifically assigned work",
			fr: "Contrat pour l'exécution d'un travail nettement défini",
			nl: "Overeenkomst voor een duidelijk omschreven werk",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:epc": "epc:m4",
		},
		openDimensions: ["dim:open"],
		code: "1131-1132-1133",
		label: "Replacement contract",
		labels: {
			de: "Vertretungsvertrag",
			en: "Replacement contract",
			fr: "Contrat de remplacement",
			nl: "Vervangingsovereenkomst",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:epc": "epc:m4",
		},
		openDimensions: ["dim:open"],
		code: "1131-1132-1133",
		label: "Replacement contract",
		labels: {
			de: "Vertretungsvertrag",
			en: "Replacement contract",
			fr: "Contrat de remplacement",
			nl: "Vervangingsovereenkomst",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:epc": "epc:m4",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "1131-1132-1133",
		label: "Replacement contract",
		labels: {
			de: "Vertretungsvertrag",
			en: "Replacement contract",
			fr: "Contrat de remplacement",
			nl: "Vervangingsovereenkomst",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:epc": "epc:m4",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "1131-1132-1133",
		label: "Replacement contract",
		labels: {
			de: "Vertretungsvertrag",
			en: "Replacement contract",
			fr: "Contrat de remplacement",
			nl: "Vervangingsovereenkomst",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:epc": "epc:m4",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "1131-1132-1133",
		label: "Replacement contract",
		labels: {
			de: "Vertretungsvertrag",
			en: "Replacement contract",
			fr: "Contrat de remplacement",
			nl: "Vervangingsovereenkomst",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:dgl": "dgl:m1",
		},
		openDimensions: ["dim:open"],
		code: "12001-12002-12003",
		label: "primary education",
		labels: {
			de: "Primarschulunterricht",
			en: "primary education",
			fr: "de niveau primaire",
			nl: "lager onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:dgl": "dgl:m1",
		},
		openDimensions: ["dim:open"],
		code: "12001-12002-12003",
		label: "primary education",
		labels: {
			de: "Primarschulunterricht",
			en: "primary education",
			fr: "de niveau primaire",
			nl: "lager onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:dgl": "dgl:m1",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "12001-12002-12003",
		label: "primary education",
		labels: {
			de: "Primarschulunterricht",
			en: "primary education",
			fr: "de niveau primaire",
			nl: "lager onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:dgl": "dgl:m1",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "12001-12002-12003",
		label: "primary education",
		labels: {
			de: "Primarschulunterricht",
			en: "primary education",
			fr: "de niveau primaire",
			nl: "lager onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:dgl": "dgl:m1",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "12001-12002-12003",
		label: "primary education",
		labels: {
			de: "Primarschulunterricht",
			en: "primary education",
			fr: "de niveau primaire",
			nl: "lager onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:dgl": "dgl:m2",
		},
		openDimensions: ["dim:open"],
		code: "12011-12012-12013",
		label: "secondary education",
		labels: {
			de: "Sekundarschulunterricht",
			en: "secondary education",
			fr: "de niveau secondaire",
			nl: "secundair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:dgl": "dgl:m2",
		},
		openDimensions: ["dim:open"],
		code: "12011-12012-12013",
		label: "secondary education",
		labels: {
			de: "Sekundarschulunterricht",
			en: "secondary education",
			fr: "de niveau secondaire",
			nl: "secundair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:dgl": "dgl:m2",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "12011-12012-12013",
		label: "secondary education",
		labels: {
			de: "Sekundarschulunterricht",
			en: "secondary education",
			fr: "de niveau secondaire",
			nl: "secundair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:dgl": "dgl:m2",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "12011-12012-12013",
		label: "secondary education",
		labels: {
			de: "Sekundarschulunterricht",
			en: "secondary education",
			fr: "de niveau secondaire",
			nl: "secundair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:dgl": "dgl:m2",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "12011-12012-12013",
		label: "secondary education",
		labels: {
			de: "Sekundarschulunterricht",
			en: "secondary education",
			fr: "de niveau secondaire",
			nl: "secundair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:dgl": "dgl:m3",
		},
		openDimensions: ["dim:open"],
		code: "12021-12022-12023",
		label: "higher non-university education",
		labels: {
			de: "Nichtuniversitärer Hochschulunterricht",
			en: "higher non-university education",
			fr: "de niveau supérieur non universitaire",
			nl: "hoger niet-universitair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:dgl": "dgl:m3",
		},
		openDimensions: ["dim:open"],
		code: "12021-12022-12023",
		label: "higher non-university education",
		labels: {
			de: "Nichtuniversitärer Hochschulunterricht",
			en: "higher non-university education",
			fr: "de niveau supérieur non universitaire",
			nl: "hoger niet-universitair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:dgl": "dgl:m3",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "12021-12022-12023",
		label: "higher non-university education",
		labels: {
			de: "Nichtuniversitärer Hochschulunterricht",
			en: "higher non-university education",
			fr: "de niveau supérieur non universitaire",
			nl: "hoger niet-universitair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:dgl": "dgl:m3",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "12021-12022-12023",
		label: "higher non-university education",
		labels: {
			de: "Nichtuniversitärer Hochschulunterricht",
			en: "higher non-university education",
			fr: "de niveau supérieur non universitaire",
			nl: "hoger niet-universitair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:dgl": "dgl:m3",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "12021-12022-12023",
		label: "higher non-university education",
		labels: {
			de: "Nichtuniversitärer Hochschulunterricht",
			en: "higher non-university education",
			fr: "de niveau supérieur non universitaire",
			nl: "hoger niet-universitair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:dgl": "dgl:m4",
		},
		openDimensions: ["dim:open"],
		code: "12031-12032-12033",
		label: "university education",
		labels: {
			de: "Universitätsunterricht",
			en: "university education",
			fr: "de niveau universitaire",
			nl: "universitair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:dgl": "dgl:m4",
		},
		openDimensions: ["dim:open"],
		code: "12031-12032-12033",
		label: "university education",
		labels: {
			de: "Universitätsunterricht",
			en: "university education",
			fr: "de niveau universitaire",
			nl: "universitair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:dgl": "dgl:m4",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "12031-12032-12033",
		label: "university education",
		labels: {
			de: "Universitätsunterricht",
			en: "university education",
			fr: "de niveau universitaire",
			nl: "universitair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:dgl": "dgl:m4",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "12031-12032-12033",
		label: "university education",
		labels: {
			de: "Universitätsunterricht",
			en: "university education",
			fr: "de niveau universitaire",
			nl: "universitair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:dgl": "dgl:m4",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "12031-12032-12033",
		label: "university education",
		labels: {
			de: "Universitätsunterricht",
			en: "university education",
			fr: "de niveau universitaire",
			nl: "universitair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
		},
		openDimensions: ["dim:open"],
		code: "1201-1202-1203",
		label: "Men",
		labels: {
			de: "Männer",
			en: "Men",
			fr: "Hommes",
			nl: "Mannen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
		},
		openDimensions: ["dim:open"],
		code: "1201-1202-1203",
		label: "Men",
		labels: {
			de: "Männer",
			en: "Men",
			fr: "Hommes",
			nl: "Mannen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "1201-1202-1203",
		label: "Men",
		labels: {
			de: "Männer",
			en: "Men",
			fr: "Hommes",
			nl: "Mannen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "1201-1202-1203",
		label: "Men",
		labels: {
			de: "Männer",
			en: "Men",
			fr: "Hommes",
			nl: "Mannen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m2",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "1201-1202-1203",
		label: "Men",
		labels: {
			de: "Männer",
			en: "Men",
			fr: "Hommes",
			nl: "Mannen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:dgl": "dgl:m1",
		},
		openDimensions: ["dim:open"],
		code: "12101-12102-12103",
		label: "primary education",
		labels: {
			de: "Primarschulunterricht",
			en: "primary education",
			fr: "de niveau primaire",
			nl: "lager onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:dgl": "dgl:m1",
		},
		openDimensions: ["dim:open"],
		code: "12101-12102-12103",
		label: "primary education",
		labels: {
			de: "Primarschulunterricht",
			en: "primary education",
			fr: "de niveau primaire",
			nl: "lager onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:dgl": "dgl:m1",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "12101-12102-12103",
		label: "primary education",
		labels: {
			de: "Primarschulunterricht",
			en: "primary education",
			fr: "de niveau primaire",
			nl: "lager onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:dgl": "dgl:m1",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "12101-12102-12103",
		label: "primary education",
		labels: {
			de: "Primarschulunterricht",
			en: "primary education",
			fr: "de niveau primaire",
			nl: "lager onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:dgl": "dgl:m1",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "12101-12102-12103",
		label: "primary education",
		labels: {
			de: "Primarschulunterricht",
			en: "primary education",
			fr: "de niveau primaire",
			nl: "lager onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:dgl": "dgl:m2",
		},
		openDimensions: ["dim:open"],
		code: "12111-12112-12113",
		label: "secondary education",
		labels: {
			de: "Sekundarschulunterricht",
			en: "secondary education",
			fr: "de niveau secondaire",
			nl: "secundair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:dgl": "dgl:m2",
		},
		openDimensions: ["dim:open"],
		code: "12111-12112-12113",
		label: "secondary education",
		labels: {
			de: "Sekundarschulunterricht",
			en: "secondary education",
			fr: "de niveau secondaire",
			nl: "secundair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:dgl": "dgl:m2",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "12111-12112-12113",
		label: "secondary education",
		labels: {
			de: "Sekundarschulunterricht",
			en: "secondary education",
			fr: "de niveau secondaire",
			nl: "secundair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:dgl": "dgl:m2",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "12111-12112-12113",
		label: "secondary education",
		labels: {
			de: "Sekundarschulunterricht",
			en: "secondary education",
			fr: "de niveau secondaire",
			nl: "secundair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:dgl": "dgl:m2",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "12111-12112-12113",
		label: "secondary education",
		labels: {
			de: "Sekundarschulunterricht",
			en: "secondary education",
			fr: "de niveau secondaire",
			nl: "secundair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:dgl": "dgl:m3",
		},
		openDimensions: ["dim:open"],
		code: "12121-12122-12123",
		label: "higher non-university education",
		labels: {
			de: "Nichtuniversitärer Hochschulunterricht",
			en: "higher non-university education",
			fr: "de niveau supérieur non universitaire",
			nl: "hoger niet-universitair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:dgl": "dgl:m3",
		},
		openDimensions: ["dim:open"],
		code: "12121-12122-12123",
		label: "higher non-university education",
		labels: {
			de: "Nichtuniversitärer Hochschulunterricht",
			en: "higher non-university education",
			fr: "de niveau supérieur non universitaire",
			nl: "hoger niet-universitair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:dgl": "dgl:m3",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "12121-12122-12123",
		label: "higher non-university education",
		labels: {
			de: "Nichtuniversitärer Hochschulunterricht",
			en: "higher non-university education",
			fr: "de niveau supérieur non universitaire",
			nl: "hoger niet-universitair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:dgl": "dgl:m3",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "12121-12122-12123",
		label: "higher non-university education",
		labels: {
			de: "Nichtuniversitärer Hochschulunterricht",
			en: "higher non-university education",
			fr: "de niveau supérieur non universitaire",
			nl: "hoger niet-universitair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:dgl": "dgl:m3",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "12121-12122-12123",
		label: "higher non-university education",
		labels: {
			de: "Nichtuniversitärer Hochschulunterricht",
			en: "higher non-university education",
			fr: "de niveau supérieur non universitaire",
			nl: "hoger niet-universitair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:dgl": "dgl:m4",
		},
		openDimensions: ["dim:open"],
		code: "12131-12132-12133",
		label: "university education",
		labels: {
			de: "Universitätsunterricht",
			en: "university education",
			fr: "de niveau universitaire",
			nl: "universitair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:dgl": "dgl:m4",
		},
		openDimensions: ["dim:open"],
		code: "12131-12132-12133",
		label: "university education",
		labels: {
			de: "Universitätsunterricht",
			en: "university education",
			fr: "de niveau universitaire",
			nl: "universitair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:dgl": "dgl:m4",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "12131-12132-12133",
		label: "university education",
		labels: {
			de: "Universitätsunterricht",
			en: "university education",
			fr: "de niveau universitaire",
			nl: "universitair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:dgl": "dgl:m4",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "12131-12132-12133",
		label: "university education",
		labels: {
			de: "Universitätsunterricht",
			en: "university education",
			fr: "de niveau universitaire",
			nl: "universitair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:dgl": "dgl:m4",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "12131-12132-12133",
		label: "university education",
		labels: {
			de: "Universitätsunterricht",
			en: "university education",
			fr: "de niveau universitaire",
			nl: "universitair onderwijs",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
		},
		openDimensions: ["dim:open"],
		code: "1211-1212-1213",
		label: "Women",
		labels: {
			de: "Frauen",
			en: "Women",
			fr: "Femmes",
			nl: "Vrouwen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
		},
		openDimensions: ["dim:open"],
		code: "1211-1212-1213",
		label: "Women",
		labels: {
			de: "Frauen",
			en: "Women",
			fr: "Femmes",
			nl: "Vrouwen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "1211-1212-1213",
		label: "Women",
		labels: {
			de: "Frauen",
			en: "Women",
			fr: "Femmes",
			nl: "Vrouwen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "1211-1212-1213",
		label: "Women",
		labels: {
			de: "Frauen",
			en: "Women",
			fr: "Femmes",
			nl: "Vrouwen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:gdr": "gdr:m3",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "1211-1212-1213",
		label: "Women",
		labels: {
			de: "Frauen",
			en: "Women",
			fr: "Femmes",
			nl: "Vrouwen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:pfc": "pfc:m1",
		},
		openDimensions: ["dim:open"],
		code: "1301-1302-1303",
		label: "Management staff",
		labels: {
			de: "Führungskräfte",
			en: "Management staff",
			fr: "Personnel de direction",
			nl: "Directiepersoneel",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:pfc": "pfc:m1",
		},
		openDimensions: ["dim:open"],
		code: "1301-1302-1303",
		label: "Management staff",
		labels: {
			de: "Führungskräfte",
			en: "Management staff",
			fr: "Personnel de direction",
			nl: "Directiepersoneel",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:pfc": "pfc:m1",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "1301-1302-1303",
		label: "Management staff",
		labels: {
			de: "Führungskräfte",
			en: "Management staff",
			fr: "Personnel de direction",
			nl: "Directiepersoneel",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:pfc": "pfc:m1",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "1301-1302-1303",
		label: "Management staff",
		labels: {
			de: "Führungskräfte",
			en: "Management staff",
			fr: "Personnel de direction",
			nl: "Directiepersoneel",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:pfc": "pfc:m1",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "1301-1302-1303",
		label: "Management staff",
		labels: {
			de: "Führungskräfte",
			en: "Management staff",
			fr: "Personnel de direction",
			nl: "Directiepersoneel",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:pfc": "pfc:m2",
		},
		openDimensions: ["dim:open"],
		code: "1341-1342-1343",
		label: "Employees",
		labels: {
			de: "Angestellte",
			en: "Employees",
			fr: "Employés",
			nl: "Bedienden",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:pfc": "pfc:m2",
		},
		openDimensions: ["dim:open"],
		code: "1341-1342-1343",
		label: "Employees",
		labels: {
			de: "Angestellte",
			en: "Employees",
			fr: "Employés",
			nl: "Bedienden",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:pfc": "pfc:m2",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "1341-1342-1343",
		label: "Employees",
		labels: {
			de: "Angestellte",
			en: "Employees",
			fr: "Employés",
			nl: "Bedienden",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:pfc": "pfc:m2",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "1341-1342-1343",
		label: "Employees",
		labels: {
			de: "Angestellte",
			en: "Employees",
			fr: "Employés",
			nl: "Bedienden",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:pfc": "pfc:m2",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "1341-1342-1343",
		label: "Employees",
		labels: {
			de: "Angestellte",
			en: "Employees",
			fr: "Employés",
			nl: "Bedienden",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:pfc": "pfc:m3",
		},
		openDimensions: ["dim:open"],
		code: "1321-1322-1323",
		label: "Workers",
		labels: {
			de: "Arbeiter",
			en: "Workers",
			fr: "Ouvriers",
			nl: "Arbeiders",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:pfc": "pfc:m3",
		},
		openDimensions: ["dim:open"],
		code: "1321-1322-1323",
		label: "Workers",
		labels: {
			de: "Arbeiter",
			en: "Workers",
			fr: "Ouvriers",
			nl: "Arbeiders",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:pfc": "pfc:m3",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "1321-1322-1323",
		label: "Workers",
		labels: {
			de: "Arbeiter",
			en: "Workers",
			fr: "Ouvriers",
			nl: "Arbeiders",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:pfc": "pfc:m3",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "1321-1322-1323",
		label: "Workers",
		labels: {
			de: "Arbeiter",
			en: "Workers",
			fr: "Ouvriers",
			nl: "Arbeiders",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:pfc": "pfc:m3",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "1321-1322-1323",
		label: "Workers",
		labels: {
			de: "Arbeiter",
			en: "Workers",
			fr: "Ouvriers",
			nl: "Arbeiders",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:pfc": "pfc:m4",
		},
		openDimensions: ["dim:open"],
		code: "1331-1332-1333",
		label: "Others",
		labels: {
			de: "Sonstige",
			en: "Others",
			fr: "Autres",
			nl: "Andere",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:pfc": "pfc:m4",
		},
		openDimensions: ["dim:open"],
		code: "1331-1332-1333",
		label: "Others",
		labels: {
			de: "Sonstige",
			en: "Others",
			fr: "Autres",
			nl: "Andere",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:pfc": "pfc:m4",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "1331-1332-1333",
		label: "Others",
		labels: {
			de: "Sonstige",
			en: "Others",
			fr: "Autres",
			nl: "Andere",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:pfc": "pfc:m4",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "1331-1332-1333",
		label: "Others",
		labels: {
			de: "Sonstige",
			en: "Others",
			fr: "Autres",
			nl: "Andere",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mmt": "mmt:m2",
			"dim:pfc": "pfc:m4",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "1331-1332-1333",
		label: "Others",
		labels: {
			de: "Sonstige",
			en: "Others",
			fr: "Autres",
			nl: "Andere",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mdp": "mdp:m19",
		},
		openDimensions: ["dim:open"],
		code: "2051-2052-2053",
		label: "Number of employees for whom the association or the foundation submitted a DIMONA declaration or who have been recorded in the general personnel register during the financial year",
		labels: {
			de: "Anzahl der Arbeitnehmer, für die die Vereinigung oder Stiftung eine DIMONA-Meldung eingereicht hat oder die im Laufe des Geschäftsjahres in das allgemeine Personalregister eingetragen wurden",
			en: "Number of employees for whom the association or the foundation submitted a DIMONA declaration or who have been recorded in the general personnel register during the financial year",
			fr: "Nombre de travailleurs pour lesquels l'association ou la fondation a introduit une déclaration DIMONA ou qui ont été inscrits au registre général du personnel au cours de l'exercice",
			nl: "Aantal werknemers waarvoor de vereniging of stichting tijdens het boekjaar een DIMONA-verklaring heeft ingediend of die tijdens het boekjaar werden ingeschreven in het algemeen personeelsregister",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mdp": "mdp:m19",
		},
		openDimensions: ["dim:open"],
		code: "2051-2052-2053",
		label: "Number of employees for whom the association or the foundation submitted a DIMONA declaration or who have been recorded in the general personnel register during the financial year",
		labels: {
			de: "Anzahl der Arbeitnehmer, für die die Vereinigung oder Stiftung eine DIMONA-Meldung eingereicht hat oder die im Laufe des Geschäftsjahres in das allgemeine Personalregister eingetragen wurden",
			en: "Number of employees for whom the association or the foundation submitted a DIMONA declaration or who have been recorded in the general personnel register during the financial year",
			fr: "Nombre de travailleurs pour lesquels l'association ou la fondation a introduit une déclaration DIMONA ou qui ont été inscrits au registre général du personnel au cours de l'exercice",
			nl: "Aantal werknemers waarvoor de vereniging of stichting tijdens het boekjaar een DIMONA-verklaring heeft ingediend of die tijdens het boekjaar werden ingeschreven in het algemeen personeelsregister",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mdp": "mdp:m19",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "2051-2052-2053",
		label: "Number of employees for whom the association or the foundation submitted a DIMONA declaration or who have been recorded in the general personnel register during the financial year",
		labels: {
			de: "Anzahl der Arbeitnehmer, für die die Vereinigung oder Stiftung eine DIMONA-Meldung eingereicht hat oder die im Laufe des Geschäftsjahres in das allgemeine Personalregister eingetragen wurden",
			en: "Number of employees for whom the association or the foundation submitted a DIMONA declaration or who have been recorded in the general personnel register during the financial year",
			fr: "Nombre de travailleurs pour lesquels l'association ou la fondation a introduit une déclaration DIMONA ou qui ont été inscrits au registre général du personnel au cours de l'exercice",
			nl: "Aantal werknemers waarvoor de vereniging of stichting tijdens het boekjaar een DIMONA-verklaring heeft ingediend of die tijdens het boekjaar werden ingeschreven in het algemeen personeelsregister",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mdp": "mdp:m19",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "2051-2052-2053",
		label: "Number of employees for whom the association or the foundation submitted a DIMONA declaration or who have been recorded in the general personnel register during the financial year",
		labels: {
			de: "Anzahl der Arbeitnehmer, für die die Vereinigung oder Stiftung eine DIMONA-Meldung eingereicht hat oder die im Laufe des Geschäftsjahres in das allgemeine Personalregister eingetragen wurden",
			en: "Number of employees for whom the association or the foundation submitted a DIMONA declaration or who have been recorded in the general personnel register during the financial year",
			fr: "Nombre de travailleurs pour lesquels l'association ou la fondation a introduit une déclaration DIMONA ou qui ont été inscrits au registre général du personnel au cours de l'exercice",
			nl: "Aantal werknemers waarvoor de vereniging of stichting tijdens het boekjaar een DIMONA-verklaring heeft ingediend of die tijdens het boekjaar werden ingeschreven in het algemeen personeelsregister",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mdp": "mdp:m19",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "2051-2052-2053",
		label: "Number of employees for whom the association or the foundation submitted a DIMONA declaration or who have been recorded in the general personnel register during the financial year",
		labels: {
			de: "Anzahl der Arbeitnehmer, für die die Vereinigung oder Stiftung eine DIMONA-Meldung eingereicht hat oder die im Laufe des Geschäftsjahres in das allgemeine Personalregister eingetragen wurden",
			en: "Number of employees for whom the association or the foundation submitted a DIMONA declaration or who have been recorded in the general personnel register during the financial year",
			fr: "Nombre de travailleurs pour lesquels l'association ou la fondation a introduit une déclaration DIMONA ou qui ont été inscrits au registre général du personnel au cours de l'exercice",
			nl: "Aantal werknemers waarvoor de vereniging of stichting tijdens het boekjaar een DIMONA-verklaring heeft ingediend of die tijdens het boekjaar werden ingeschreven in het algemeen personeelsregister",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mdp": "mdp:m20",
		},
		openDimensions: ["dim:open"],
		code: "3051-3052-3053",
		label: "Number of employees whose contract-termination date has been entered in DIMONA declaration or in the general personnel register during the financial year",
		labels: {
			de: "Anzahl der Arbeitnehmer, bei denen das Datum der Beendigung des Arbeitsverhältnisses in einer DIMONA-Meldung steht oder im Laufe des Geschäftsjahres in das allgemeine Personalregister eingetragen wurde",
			en: "Number of employees whose contract-termination date has been entered in DIMONA declaration or in the general personnel register during the financial year",
			fr: "Nombre de travailleurs dont la date de fin de contrat a été inscrite dans une déclaration DIMONA ou au registre général du personnel au cours de l'exercice",
			nl: "Aantal werknemers met een in de DIMONA-verklaring aangegeven of een in het algemeen personeelsregister opgetekende datum waarop hun overeenkomst tijdens het boekjaar een einde nam",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mdp": "mdp:m20",
		},
		openDimensions: ["dim:open"],
		code: "3051-3052-3053",
		label: "Number of employees whose contract-termination date has been entered in DIMONA declaration or in the general personnel register during the financial year",
		labels: {
			de: "Anzahl der Arbeitnehmer, bei denen das Datum der Beendigung des Arbeitsverhältnisses in einer DIMONA-Meldung steht oder im Laufe des Geschäftsjahres in das allgemeine Personalregister eingetragen wurde",
			en: "Number of employees whose contract-termination date has been entered in DIMONA declaration or in the general personnel register during the financial year",
			fr: "Nombre de travailleurs dont la date de fin de contrat a été inscrite dans une déclaration DIMONA ou au registre général du personnel au cours de l'exercice",
			nl: "Aantal werknemers met een in de DIMONA-verklaring aangegeven of een in het algemeen personeelsregister opgetekende datum waarop hun overeenkomst tijdens het boekjaar een einde nam",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mdp": "mdp:m20",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "3051-3052-3053",
		label: "Number of employees whose contract-termination date has been entered in DIMONA declaration or in the general personnel register during the financial year",
		labels: {
			de: "Anzahl der Arbeitnehmer, bei denen das Datum der Beendigung des Arbeitsverhältnisses in einer DIMONA-Meldung steht oder im Laufe des Geschäftsjahres in das allgemeine Personalregister eingetragen wurde",
			en: "Number of employees whose contract-termination date has been entered in DIMONA declaration or in the general personnel register during the financial year",
			fr: "Nombre de travailleurs dont la date de fin de contrat a été inscrite dans une déclaration DIMONA ou au registre général du personnel au cours de l'exercice",
			nl: "Aantal werknemers met een in de DIMONA-verklaring aangegeven of een in het algemeen personeelsregister opgetekende datum waarop hun overeenkomst tijdens het boekjaar een einde nam",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mdp": "mdp:m20",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "3051-3052-3053",
		label: "Number of employees whose contract-termination date has been entered in DIMONA declaration or in the general personnel register during the financial year",
		labels: {
			de: "Anzahl der Arbeitnehmer, bei denen das Datum der Beendigung des Arbeitsverhältnisses in einer DIMONA-Meldung steht oder im Laufe des Geschäftsjahres in das allgemeine Personalregister eingetragen wurde",
			en: "Number of employees whose contract-termination date has been entered in DIMONA declaration or in the general personnel register during the financial year",
			fr: "Nombre de travailleurs dont la date de fin de contrat a été inscrite dans une déclaration DIMONA ou au registre général du personnel au cours de l'exercice",
			nl: "Aantal werknemers met een in de DIMONA-verklaring aangegeven of een in het algemeen personeelsregister opgetekende datum waarop hun overeenkomst tijdens het boekjaar een einde nam",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:bas": "bas:m73",
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:mdp": "mdp:m20",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "3051-3052-3053",
		label: "Number of employees whose contract-termination date has been entered in DIMONA declaration or in the general personnel register during the financial year",
		labels: {
			de: "Anzahl der Arbeitnehmer, bei denen das Datum der Beendigung des Arbeitsverhältnisses in einer DIMONA-Meldung steht oder im Laufe des Geschäftsjahres in das allgemeine Personalregister eingetragen wurde",
			en: "Number of employees whose contract-termination date has been entered in DIMONA declaration or in the general personnel register during the financial year",
			fr: "Nombre de travailleurs dont la date de fin de contrat a été inscrite dans une déclaration DIMONA ou au registre général du personnel au cours de l'exercice",
			nl: "Aantal werknemers met een in de DIMONA-verklaring aangegeven of een in het algemeen personeelsregister opgetekende datum waarop hun overeenkomst tijdens het boekjaar een einde nam",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m73",
		},
		openDimensions: ["dim:open"],
		code: "5801-5811",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m73",
		},
		openDimensions: ["dim:open"],
		code: "5801-5811",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m73",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "5801-5811",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m73",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "5801-5811",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m73",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "5801-5811",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m73",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "5801-5811",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m73",
			"dim:gdr": "gdr:m2",
		},
		openDimensions: ["dim:open"],
		code: "5801-5811",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m73",
			"dim:gdr": "gdr:m3",
		},
		openDimensions: ["dim:open"],
		code: "5801-5811",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
		},
		openDimensions: ["dim:open"],
		code: "5802-5812",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
		},
		openDimensions: ["dim:open"],
		code: "5802-5812",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int4",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "5802-5812",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int4",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "5802-5812",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int4",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "5802-5812",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "5802-5812",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "5802-5812",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "5802-5812",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int4",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:gdr": "gdr:m2",
		},
		openDimensions: ["dim:open"],
		code: "5802-5812",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int4",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:gdr": "gdr:m3",
		},
		openDimensions: ["dim:open"],
		code: "5802-5812",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m17",
			"dim:spec": "spec:m84",
		},
		openDimensions: ["dim:open"],
		code: "58031-58131",
		label: "of which gross costs directly linked to training",
		labels: {
			de: "wovon Bruttokosten die direkt mit der Weiterbildung verbunden sind",
			en: "of which gross costs directly linked to training",
			fr: "dont coût brut directement lié aux formations",
			nl: "waarvan brutokosten rechtstreeks verbonden met de opleiding",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m17",
			"dim:spec": "spec:m84",
		},
		openDimensions: ["dim:open"],
		code: "58031-58131",
		label: "of which gross costs directly linked to training",
		labels: {
			de: "wovon Bruttokosten die direkt mit der Weiterbildung verbunden sind",
			en: "of which gross costs directly linked to training",
			fr: "dont coût brut directement lié aux formations",
			nl: "waarvan brutokosten rechtstreeks verbonden met de opleiding",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m17",
			"dim:spec": "spec:m84",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "58031-58131",
		label: "of which gross costs directly linked to training",
		labels: {
			de: "wovon Bruttokosten die direkt mit der Weiterbildung verbunden sind",
			en: "of which gross costs directly linked to training",
			fr: "dont coût brut directement lié aux formations",
			nl: "waarvan brutokosten rechtstreeks verbonden met de opleiding",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m17",
			"dim:spec": "spec:m84",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "58031-58131",
		label: "of which gross costs directly linked to training",
		labels: {
			de: "wovon Bruttokosten die direkt mit der Weiterbildung verbunden sind",
			en: "of which gross costs directly linked to training",
			fr: "dont coût brut directement lié aux formations",
			nl: "waarvan brutokosten rechtstreeks verbonden met de opleiding",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m17",
			"dim:spec": "spec:m84",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "58031-58131",
		label: "of which gross costs directly linked to training",
		labels: {
			de: "wovon Bruttokosten die direkt mit der Weiterbildung verbunden sind",
			en: "of which gross costs directly linked to training",
			fr: "dont coût brut directement lié aux formations",
			nl: "waarvan brutokosten rechtstreeks verbonden met de opleiding",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m17",
			"dim:spec": "spec:m84",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "58031-58131",
		label: "of which gross costs directly linked to training",
		labels: {
			de: "wovon Bruttokosten die direkt mit der Weiterbildung verbunden sind",
			en: "of which gross costs directly linked to training",
			fr: "dont coût brut directement lié aux formations",
			nl: "waarvan brutokosten rechtstreeks verbonden met de opleiding",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m17",
			"dim:spec": "spec:m84",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "58031-58131",
		label: "of which gross costs directly linked to training",
		labels: {
			de: "wovon Bruttokosten die direkt mit der Weiterbildung verbunden sind",
			en: "of which gross costs directly linked to training",
			fr: "dont coût brut directement lié aux formations",
			nl: "waarvan brutokosten rechtstreeks verbonden met de opleiding",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m17",
			"dim:spec": "spec:m84",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "58031-58131",
		label: "of which gross costs directly linked to training",
		labels: {
			de: "wovon Bruttokosten die direkt mit der Weiterbildung verbunden sind",
			en: "of which gross costs directly linked to training",
			fr: "dont coût brut directement lié aux formations",
			nl: "waarvan brutokosten rechtstreeks verbonden met de opleiding",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m17",
			"dim:spec": "spec:m84",
			"dim:gdr": "gdr:m2",
		},
		openDimensions: ["dim:open"],
		code: "58031-58131",
		label: "of which gross costs directly linked to training",
		labels: {
			de: "wovon Bruttokosten die direkt mit der Weiterbildung verbunden sind",
			en: "of which gross costs directly linked to training",
			fr: "dont coût brut directement lié aux formations",
			nl: "waarvan brutokosten rechtstreeks verbonden met de opleiding",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m17",
			"dim:spec": "spec:m84",
			"dim:gdr": "gdr:m3",
		},
		openDimensions: ["dim:open"],
		code: "58031-58131",
		label: "of which gross costs directly linked to training",
		labels: {
			de: "wovon Bruttokosten die direkt mit der Weiterbildung verbunden sind",
			en: "of which gross costs directly linked to training",
			fr: "dont coût brut directement lié aux formations",
			nl: "waarvan brutokosten rechtstreeks verbonden met de opleiding",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:typ": "typ:m31",
			"dim:bnf": "bnf:m5",
		},
		openDimensions: ["dim:open"],
		code: "58032-58132",
		label: "of which fees paid and paiments to collective funds",
		labels: {
			de: "wovon gezahlte Beiträge und Einzahlungen an Kollektivfonds",
			en: "of which fees paid and paiments to collective funds",
			fr: "dont cotisations payées et versements à des fonds collectifs",
			nl: "waarvan betaalde bijdragen en stortingen aan collectieve fondsen",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:typ": "typ:m31",
			"dim:bnf": "bnf:m5",
		},
		openDimensions: ["dim:open"],
		code: "58032-58132",
		label: "of which fees paid and paiments to collective funds",
		labels: {
			de: "wovon gezahlte Beiträge und Einzahlungen an Kollektivfonds",
			en: "of which fees paid and paiments to collective funds",
			fr: "dont cotisations payées et versements à des fonds collectifs",
			nl: "waarvan betaalde bijdragen en stortingen aan collectieve fondsen",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:typ": "typ:m31",
			"dim:bnf": "bnf:m5",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "58032-58132",
		label: "of which fees paid and paiments to collective funds",
		labels: {
			de: "wovon gezahlte Beiträge und Einzahlungen an Kollektivfonds",
			en: "of which fees paid and paiments to collective funds",
			fr: "dont cotisations payées et versements à des fonds collectifs",
			nl: "waarvan betaalde bijdragen en stortingen aan collectieve fondsen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:typ": "typ:m31",
			"dim:bnf": "bnf:m5",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "58032-58132",
		label: "of which fees paid and paiments to collective funds",
		labels: {
			de: "wovon gezahlte Beiträge und Einzahlungen an Kollektivfonds",
			en: "of which fees paid and paiments to collective funds",
			fr: "dont cotisations payées et versements à des fonds collectifs",
			nl: "waarvan betaalde bijdragen en stortingen aan collectieve fondsen",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:typ": "typ:m31",
			"dim:bnf": "bnf:m5",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "58032-58132",
		label: "of which fees paid and paiments to collective funds",
		labels: {
			de: "wovon gezahlte Beiträge und Einzahlungen an Kollektivfonds",
			en: "of which fees paid and paiments to collective funds",
			fr: "dont cotisations payées et versements à des fonds collectifs",
			nl: "waarvan betaalde bijdragen en stortingen aan collectieve fondsen",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:typ": "typ:m31",
			"dim:bnf": "bnf:m5",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "58032-58132",
		label: "of which fees paid and paiments to collective funds",
		labels: {
			de: "wovon gezahlte Beiträge und Einzahlungen an Kollektivfonds",
			en: "of which fees paid and paiments to collective funds",
			fr: "dont cotisations payées et versements à des fonds collectifs",
			nl: "waarvan betaalde bijdragen en stortingen aan collectieve fondsen",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:typ": "typ:m31",
			"dim:bnf": "bnf:m5",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "58032-58132",
		label: "of which fees paid and paiments to collective funds",
		labels: {
			de: "wovon gezahlte Beiträge und Einzahlungen an Kollektivfonds",
			en: "of which fees paid and paiments to collective funds",
			fr: "dont cotisations payées et versements à des fonds collectifs",
			nl: "waarvan betaalde bijdragen en stortingen aan collectieve fondsen",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:typ": "typ:m31",
			"dim:bnf": "bnf:m5",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "58032-58132",
		label: "of which fees paid and paiments to collective funds",
		labels: {
			de: "wovon gezahlte Beiträge und Einzahlungen an Kollektivfonds",
			en: "of which fees paid and paiments to collective funds",
			fr: "dont cotisations payées et versements à des fonds collectifs",
			nl: "waarvan betaalde bijdragen en stortingen aan collectieve fondsen",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:typ": "typ:m31",
			"dim:bnf": "bnf:m5",
			"dim:gdr": "gdr:m2",
		},
		openDimensions: ["dim:open"],
		code: "58032-58132",
		label: "of which fees paid and paiments to collective funds",
		labels: {
			de: "wovon gezahlte Beiträge und Einzahlungen an Kollektivfonds",
			en: "of which fees paid and paiments to collective funds",
			fr: "dont cotisations payées et versements à des fonds collectifs",
			nl: "waarvan betaalde bijdragen en stortingen aan collectieve fondsen",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:typ": "typ:m31",
			"dim:bnf": "bnf:m5",
			"dim:gdr": "gdr:m3",
		},
		openDimensions: ["dim:open"],
		code: "58032-58132",
		label: "of which fees paid and paiments to collective funds",
		labels: {
			de: "wovon gezahlte Beiträge und Einzahlungen an Kollektivfonds",
			en: "of which fees paid and paiments to collective funds",
			fr: "dont cotisations payées et versements à des fonds collectifs",
			nl: "waarvan betaalde bijdragen en stortingen aan collectieve fondsen",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m112",
			"dim:ntr": "ntr:m9",
			"dim:mdp": "mdp:m21",
		},
		openDimensions: ["dim:open"],
		code: "58033-58133",
		label: "of which grants and other financial advantages received (to deduct)",
		labels: {
			de: "wovon bewilligte Zuschüsse und andere finanzielle Vorteile (in Abzug)",
			en: "of which grants and other financial advantages received (to deduct)",
			fr: "dont subventions et autres avantages financiers reçus (à déduire)",
			nl: "waarvan ontvangen tegemoetkomingen (in mindering)",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m112",
			"dim:ntr": "ntr:m9",
			"dim:mdp": "mdp:m21",
		},
		openDimensions: ["dim:open"],
		code: "58033-58133",
		label: "of which grants and other financial advantages received (to deduct)",
		labels: {
			de: "wovon bewilligte Zuschüsse und andere finanzielle Vorteile (in Abzug)",
			en: "of which grants and other financial advantages received (to deduct)",
			fr: "dont subventions et autres avantages financiers reçus (à déduire)",
			nl: "waarvan ontvangen tegemoetkomingen (in mindering)",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m112",
			"dim:ntr": "ntr:m9",
			"dim:mdp": "mdp:m21",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "58033-58133",
		label: "of which grants and other financial advantages received (to deduct)",
		labels: {
			de: "wovon bewilligte Zuschüsse und andere finanzielle Vorteile (in Abzug)",
			en: "of which grants and other financial advantages received (to deduct)",
			fr: "dont subventions et autres avantages financiers reçus (à déduire)",
			nl: "waarvan ontvangen tegemoetkomingen (in mindering)",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m112",
			"dim:ntr": "ntr:m9",
			"dim:mdp": "mdp:m21",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "58033-58133",
		label: "of which grants and other financial advantages received (to deduct)",
		labels: {
			de: "wovon bewilligte Zuschüsse und andere finanzielle Vorteile (in Abzug)",
			en: "of which grants and other financial advantages received (to deduct)",
			fr: "dont subventions et autres avantages financiers reçus (à déduire)",
			nl: "waarvan ontvangen tegemoetkomingen (in mindering)",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m112",
			"dim:ntr": "ntr:m9",
			"dim:mdp": "mdp:m21",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "58033-58133",
		label: "of which grants and other financial advantages received (to deduct)",
		labels: {
			de: "wovon bewilligte Zuschüsse und andere finanzielle Vorteile (in Abzug)",
			en: "of which grants and other financial advantages received (to deduct)",
			fr: "dont subventions et autres avantages financiers reçus (à déduire)",
			nl: "waarvan ontvangen tegemoetkomingen (in mindering)",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m112",
			"dim:ntr": "ntr:m9",
			"dim:mdp": "mdp:m21",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "58033-58133",
		label: "of which grants and other financial advantages received (to deduct)",
		labels: {
			de: "wovon bewilligte Zuschüsse und andere finanzielle Vorteile (in Abzug)",
			en: "of which grants and other financial advantages received (to deduct)",
			fr: "dont subventions et autres avantages financiers reçus (à déduire)",
			nl: "waarvan ontvangen tegemoetkomingen (in mindering)",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m112",
			"dim:ntr": "ntr:m9",
			"dim:mdp": "mdp:m21",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "58033-58133",
		label: "of which grants and other financial advantages received (to deduct)",
		labels: {
			de: "wovon bewilligte Zuschüsse und andere finanzielle Vorteile (in Abzug)",
			en: "of which grants and other financial advantages received (to deduct)",
			fr: "dont subventions et autres avantages financiers reçus (à déduire)",
			nl: "waarvan ontvangen tegemoetkomingen (in mindering)",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m112",
			"dim:ntr": "ntr:m9",
			"dim:mdp": "mdp:m21",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "58033-58133",
		label: "of which grants and other financial advantages received (to deduct)",
		labels: {
			de: "wovon bewilligte Zuschüsse und andere finanzielle Vorteile (in Abzug)",
			en: "of which grants and other financial advantages received (to deduct)",
			fr: "dont subventions et autres avantages financiers reçus (à déduire)",
			nl: "waarvan ontvangen tegemoetkomingen (in mindering)",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m112",
			"dim:ntr": "ntr:m9",
			"dim:mdp": "mdp:m21",
			"dim:gdr": "gdr:m2",
		},
		openDimensions: ["dim:open"],
		code: "58033-58133",
		label: "of which grants and other financial advantages received (to deduct)",
		labels: {
			de: "wovon bewilligte Zuschüsse und andere finanzielle Vorteile (in Abzug)",
			en: "of which grants and other financial advantages received (to deduct)",
			fr: "dont subventions et autres avantages financiers reçus (à déduire)",
			nl: "waarvan ontvangen tegemoetkomingen (in mindering)",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m112",
			"dim:ntr": "ntr:m9",
			"dim:mdp": "mdp:m21",
			"dim:gdr": "gdr:m3",
		},
		openDimensions: ["dim:open"],
		code: "58033-58133",
		label: "of which grants and other financial advantages received (to deduct)",
		labels: {
			de: "wovon bewilligte Zuschüsse und andere finanzielle Vorteile (in Abzug)",
			en: "of which grants and other financial advantages received (to deduct)",
			fr: "dont subventions et autres avantages financiers reçus (à déduire)",
			nl: "waarvan ontvangen tegemoetkomingen (in mindering)",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
		},
		openDimensions: ["dim:open"],
		code: "5803-5813",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
		},
		openDimensions: ["dim:open"],
		code: "5803-5813",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "5803-5813",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "5803-5813",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "5803-5813",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "5803-5813",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "5803-5813",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "5803-5813",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:gdr": "gdr:m2",
		},
		openDimensions: ["dim:open"],
		code: "5803-5813",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m1",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:gdr": "gdr:m3",
		},
		openDimensions: ["dim:open"],
		code: "5803-5813",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m73",
		},
		openDimensions: ["dim:open"],
		code: "5821-5831",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m73",
		},
		openDimensions: ["dim:open"],
		code: "5821-5831",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m73",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "5821-5831",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m73",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "5821-5831",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m73",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "5821-5831",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m73",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "5821-5831",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m73",
			"dim:gdr": "gdr:m2",
		},
		openDimensions: ["dim:open"],
		code: "5821-5831",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m73",
			"dim:gdr": "gdr:m3",
		},
		openDimensions: ["dim:open"],
		code: "5821-5831",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
		},
		openDimensions: ["dim:open"],
		code: "5822-5832",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
		},
		openDimensions: ["dim:open"],
		code: "5822-5832",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int4",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "5822-5832",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int4",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "5822-5832",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int4",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "5822-5832",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "5822-5832",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "5822-5832",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "5822-5832",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int4",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:gdr": "gdr:m2",
		},
		openDimensions: ["dim:open"],
		code: "5822-5832",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int4",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:gdr": "gdr:m3",
		},
		openDimensions: ["dim:open"],
		code: "5822-5832",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
		},
		openDimensions: ["dim:open"],
		code: "5823-5833",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
		},
		openDimensions: ["dim:open"],
		code: "5823-5833",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "5823-5833",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "5823-5833",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "5823-5833",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "5823-5833",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "5823-5833",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "5823-5833",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:gdr": "gdr:m2",
		},
		openDimensions: ["dim:open"],
		code: "5823-5833",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m2",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:gdr": "gdr:m3",
		},
		openDimensions: ["dim:open"],
		code: "5823-5833",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m73",
		},
		openDimensions: ["dim:open"],
		code: "5841-5851",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m73",
		},
		openDimensions: ["dim:open"],
		code: "5841-5851",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m73",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "5841-5851",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m73",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "5841-5851",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m73",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "5841-5851",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m73",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "5841-5851",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m73",
			"dim:gdr": "gdr:m2",
		},
		openDimensions: ["dim:open"],
		code: "5841-5851",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m73",
			"dim:gdr": "gdr:m3",
		},
		openDimensions: ["dim:open"],
		code: "5841-5851",
		label: "Number of employees involved",
		labels: {
			de: "Anzahl der betroffenen Arbeitnehmer",
			en: "Number of employees involved",
			fr: "Nombre de travailleurs concernés",
			nl: "Aantal betrokken werknemers",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
		},
		openDimensions: ["dim:open"],
		code: "5842-5852",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
		},
		openDimensions: ["dim:open"],
		code: "5842-5852",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int4",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "5842-5852",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int4",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "5842-5852",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "int4",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "5842-5852",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "5842-5852",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "5842-5852",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "5842-5852",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int4",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:gdr": "gdr:m2",
		},
		openDimensions: ["dim:open"],
		code: "5842-5852",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int4",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m73",
			"dim:hrs": "hrs:m2",
			"dim:gdr": "gdr:m3",
		},
		openDimensions: ["dim:open"],
		code: "5842-5852",
		label: "Number of actual training hours",
		labels: {
			de: "Anzahl Stunden der besuchten Ausbildung",
			en: "Number of actual training hours",
			fr: "Nombre d'heures de formation suivies",
			nl: "Aantal gevolgde opleidingsuren",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "list1",
		metricPrefix: "jcc-enum",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
		},
		openDimensions: ["dim:open"],
		code: "5843-5853",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "str2",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
		},
		openDimensions: ["dim:open"],
		code: "5843-5853",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "5843-5853",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "5843-5853",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
		currentPeriod: "prd:m1",
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "5843-5853",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
		currentPeriod: "prd:m1",
		previousPeriod: "prd:m2",
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:wrg": "wrg:m1",
		},
		openDimensions: ["dim:open"],
		code: "5843-5853",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "int3",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:wrg": "wrg:m2",
		},
		openDimensions: ["dim:open"],
		code: "5843-5853",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "dec1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:qlt": "qlt:m3",
			"dim:wrg": "wrg:m3",
		},
		openDimensions: ["dim:open"],
		code: "5843-5853",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:gdr": "gdr:m2",
		},
		openDimensions: ["dim:open"],
		code: "5843-5853",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.04.0.i",
		metric: "am1",
		dimensions: {
			"dim:part": "part:m8",
			"dim:wkr": "wkr:m1",
			"dim:cvt": "cvt:m3",
			"dim:bas": "bas:m1",
			"dim:ntr": "ntr:m9",
			"dim:gdr": "gdr:m3",
		},
		openDimensions: ["dim:open"],
		code: "5843-5853",
		label: "Costs for the association or the foundation",
		labels: {
			de: "Nettokosten für die Vereinigung oder Stiftung",
			en: "Costs for the association or the foundation",
			fr: "Coût net pour l'association ou la fondation",
			nl: "Nettokosten voor de vereniging of stichting",
		},
	},
	{
		section: "s.06.05.0.i",
		metric: "str1",
		dimensions: {
			"dim:bas": "bas:m107",
			"dim:part": "part:m6",
		},
		label: "Valuation Rules - Text",
		labels: {
			de: "Bewertungsvorschriften - Text",
			en: "Valuation Rules - Text",
			fr: "Règles d'évaluation - Texte",
			nl: "Waarderingsregels - Tekst",
		},
	},
	{
		section: "s.06.05.0.i",
		metric: "bsb1",
		dimensions: {
			"dim:bas": "bas:m107",
			"dim:part": "part:m6",
		},
		label: "Valuation Rules - PDF",
		labels: {
			de: "Bewertungsvorschriften - PDF",
			en: "Valuation Rules - PDF",
			fr: "Règles d'évaluation - PDF",
			nl: "Waarderingsregels - PDF",
		},
	},
	{
		section: "s.07.00.0.ghi",
		metric: "str1",
		dimensions: {
			"dim:bas": "bas:m109",
			"dim:part": "part:m7",
			"dim:typ": "typ:m29",
		},
		label: "Annual report - Text",
		labels: {
			de: "Geschäftsbericht - Text",
			en: "Annual report - Text",
			fr: "Rapport de gestion - Texte",
			nl: "Jaarverslag - Tekst",
		},
	},
	{
		section: "s.07.00.0.ghi",
		metric: "bsb1",
		dimensions: {
			"dim:bas": "bas:m109",
			"dim:part": "part:m7",
			"dim:typ": "typ:m29",
		},
		label: "Annual report - PDF",
		labels: {
			de: "Geschäftsbericht - PDF",
			en: "Annual report - PDF",
			fr: "Rapport de gestion - PDF",
			nl: "Jaarverslag - PDF",
		},
	},
	{
		section: "s.08.00.0.abghi",
		metric: "str1",
		dimensions: {
			"dim:bkd": "bkd:m8",
			"dim:bas": "bas:m109",
			"dim:part": "part:m7",
			"dim:psn": "psn:m8",
		},
		label: "Title",
		labels: {
			de: "Titel",
			en: "Title",
			fr: "Titre",
			nl: "Titel",
		},
	},
	{
		section: "s.08.00.0.abghi",
		metric: "str1",
		dimensions: {
			"dim:bkd": "bkd:m9",
			"dim:bas": "bas:m109",
			"dim:part": "part:m7",
			"dim:psn": "psn:m8",
		},
		label: "Introduction",
		labels: {
			de: "Einleitung",
			en: "Introduction",
			fr: "Introduction",
			nl: "Inleiding",
		},
	},
	{
		section: "s.08.00.0.abghi",
		metric: "list1",
		metricPrefix: "opn-enum",
		dimensions: {
			"dim:bkd": "bkd:m10",
			"dim:bas": "bas:m109",
			"dim:part": "part:m7",
			"dim:psn": "psn:m8",
		},
		label: "Opinion",
		labels: {
			de: "Testat",
			en: "Opinion",
			fr: "Opinion",
			nl: "Oordeel",
		},
	},
	{
		section: "s.08.00.0.abghi",
		metric: "str1",
		dimensions: {
			"dim:bkd": "bkd:m11",
			"dim:bas": "bas:m109",
			"dim:part": "part:m7",
			"dim:psn": "psn:m8",
		},
		label: "Explanation",
		labels: {
			de: "Erklärung",
			en: "Explanation",
			fr: "Explication",
			nl: "Uitleg",
		},
	},
	{
		section: "s.08.00.0.abghi",
		metric: "str1",
		dimensions: {
			"dim:bkd": "bkd:m12",
			"dim:bas": "bas:m109",
			"dim:part": "part:m7",
			"dim:psn": "psn:m8",
		},
		label: "Explanation",
		labels: {
			de: "Erklärung",
			en: "Explanation",
			fr: "Explication",
			nl: "Uitleg",
		},
	},
	{
		section: "s.08.00.0.abghi",
		metric: "str2",
		dimensions: {
			"dim:bkd": "bkd:m13",
			"dim:bas": "bas:m109",
			"dim:part": "part:m7",
			"dim:psn": "psn:m8",
		},
		label: "Place",
		labels: {
			de: "Erstellungsort",
			en: "Place",
			fr: "Lieu",
			nl: "Plaats",
		},
	},
	{
		section: "s.08.00.0.abghi",
		metric: "dte1",
		dimensions: {
			"dim:bkd": "bkd:m14",
			"dim:bas": "bas:m109",
			"dim:part": "part:m7",
			"dim:psn": "psn:m8",
		},
		label: "Date",
		labels: {
			de: "Erstellungsdatum",
			en: "Date",
			fr: "Date",
			nl: "Datum",
		},
	},
	{
		section: "s.08.00.0.abghi",
		metric: "bsb1",
		dimensions: {
			"dim:bas": "bas:m109",
			"dim:part": "part:m7",
			"dim:psn": "psn:m8",
		},
		label: "Auditors' report - PDF",
		labels: {
			de: "Bericht der Kommissare - PDF",
			en: "Auditors' report - PDF",
			fr: "Rapport des commissaires - PDF",
			nl: "Verslag van de commissarissen - PDF",
		},
	},
];

const checks0: readonly Check[] = [
	{
		id: "va_03.01.0_0002",
		kind: "legal",
		section: "03.01.0",
		test: "$am1 eq $am12 + $am13 + $am14 + $am15 + $am16 + $am17",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am15",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am16",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am17",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "22/27 = 22 + 23 + 24 + 25 + 26 + 27",
		bindings: [
			{
				am1: "22/27",
				am12: "22",
				am13: "23",
				am14: "24",
				am15: "25",
				am16: "26",
				am17: "27",
			},
		],
	},
	{
		id: "va_03.01.0_0001",
		kind: "legal",
		section: "03.01.0",
		test: "$am1 eq $am12 + $am13 + $am14",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "21/28 = 21 + 22/27 + 28",
		bindings: [
			{
				am1: "21/28",
				am12: "21",
				am13: "22/27",
				am14: "28",
			},
		],
	},
	{
		id: "va_03.01.0_0003",
		kind: "legal",
		section: "03.01.0",
		test: "$am1 eq $am12 + $am13",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "29 = 290 + 291",
		bindings: [
			{
				am1: "29",
				am12: "290",
				am13: "291",
			},
		],
	},
	{
		id: "va_03.01.0_0004",
		kind: "legal",
		section: "03.01.0",
		test: "$am1 eq $am12 + $am13",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "3 = 30/36 + 37",
		bindings: [
			{
				am1: "3",
				am12: "30/36",
				am13: "37",
			},
		],
	},
	{
		id: "va_03.01.0_0005",
		kind: "legal",
		section: "03.01.0",
		test: "$am1 eq $am12 + $am13",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "40/41 = 40 + 41",
		bindings: [
			{
				am1: "40/41",
				am12: "40",
				am13: "41",
			},
		],
	},
	{
		id: "va_03.01.0_0006",
		kind: "legal",
		section: "03.01.0",
		test: "$am1 eq $am12 + $am13 + $am14 + $am15 + $am16 + $am17",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am15",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am16",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am17",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "29/58 = 29 + 3 + 40/41 + 50/53 + 54/58 + 490/1",
		bindings: [
			{
				am1: "29/58",
				am12: "29",
				am13: "3",
				am14: "40/41",
				am15: "50/53",
				am16: "54/58",
				am17: "490/1",
			},
		],
	},
	{
		id: "va_03.01.0_0007",
		kind: "legal",
		section: "03.01.0",
		test: "$am1 eq $am12 + $am13 + $am14",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "20/58 = 20 + 21/28 + 29/58",
		bindings: [
			{
				am1: "20/58",
				am12: "20",
				am13: "21/28",
				am14: "29/58",
			},
		],
	},
	{
		id: "va_03.01.0_0008",
		kind: "legal",
		section: "03.01.0",
		test: "$am1 eq $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "20/58 = 10/49",
		bindings: [
			{
				am1: "20/58",
				am12: "10/49",
			},
		],
	},
	{
		id: "va_03.01.0_0009",
		kind: "legal",
		section: "03.01.0",
		test: "$am1 eq $am12 - $am13",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "21 = 8059 - 8129",
		bindings: [
			{
				am1: "21",
				am12: "8059",
				am13: "8129",
			},
		],
	},
	{
		id: "va_03.01.0_0010",
		kind: "legal",
		section: "03.01.0",
		test: "$am1 eq $am12 + $am13 - $am14",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "22/27 = 8199 + 8259 - 8329",
		bindings: [
			{
				am1: "22/27",
				am12: "8199",
				am13: "8259",
				am14: "8329",
			},
		],
	},
	{
		id: "va_03.01.0_0011",
		kind: "legal",
		section: "03.01.0",
		test: "$am1 ge $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: ["8349"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:spec": "spec:m96",
				},
				codes: ["8349"],
				fallback: "0",
			},
		],
		equation: "22/27 ≥ 8349",
		bindings: [
			{
				am1: "22/27",
				am12: "8349",
			},
		],
	},
	{
		id: "va_03.01.0_0012",
		kind: "legal",
		section: "03.01.0",
		test: "$am1 eq $am12 + $am13 - $am14 - $am15",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am15",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "28 = 8395 + 8455 - 8525 - 8555",
		bindings: [
			{
				am1: "28",
				am12: "8395",
				am13: "8455",
				am14: "8525",
				am15: "8555",
			},
		],
	},
	{
		id: "va_03.02.0_0001",
		kind: "legal",
		section: "03.02.0",
		test: "$am2 eq $am22 + $am1 + $am12 + $am23 + $am13",
		variables: [
			{
				name: "am2",
				metric: "am2",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am22",
				metric: "am2",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am23",
				metric: "am2",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
		],
		equation: "10/15 = 10 + 12 + 13 + 14 + 15",
		bindings: [
			{
				am2: "10/15",
				am22: "10",
				am1: "12",
				am12: "13",
				am23: "14",
				am13: "15",
			},
		],
	},
	{
		id: "va_03.02.0_0002",
		kind: "legal",
		section: "03.02.0",
		test: "$am1 eq $am12 + $am13 + $am14",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "16 = 160/5 + 167 + 168",
		bindings: [
			{
				am1: "16",
				am12: "160/5",
				am13: "167",
				am14: "168",
			},
		],
	},
	{
		id: "va_03.02.0_0003",
		kind: "legal",
		section: "03.02.0",
		test: "$am1 eq $am12 + $am13 + $am14 + $am15 + $am16",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am15",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am16",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "160/5 = 160 + 161 + 162 + 163 + 164/5",
		bindings: [
			{
				am1: "160/5",
				am12: "160",
				am13: "161",
				am14: "162",
				am15: "163",
				am16: "164/5",
			},
		],
	},
	{
		id: "va_03.02.0_0004",
		kind: "legal",
		section: "03.02.0",
		test: "$am1 eq $am12 + $am13",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "170/4 = 172/3 + 174/0",
		bindings: [
			{
				am1: "170/4",
				am12: "172/3",
				am13: "174/0",
			},
		],
	},
	{
		id: "va_03.02.0_0005",
		kind: "legal",
		section: "03.02.0",
		test: "$am1 eq $am12 + $am13 + $am14 + $am15",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am15",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "17 = 170/4 + 175 + 176 + 178/9",
		bindings: [
			{
				am1: "17",
				am12: "170/4",
				am13: "175",
				am14: "176",
				am15: "178/9",
			},
		],
	},
	{
		id: "va_03.02.0_0006",
		kind: "legal",
		section: "03.02.0",
		test: "$am1 eq $am12 + $am13",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "43 = 430/8 + 439",
		bindings: [
			{
				am1: "43",
				am12: "430/8",
				am13: "439",
			},
		],
	},
	{
		id: "va_03.02.0_0007",
		kind: "legal",
		section: "03.02.0",
		test: "$am1 eq $am12 + $am13",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "44 = 440/4 + 441",
		bindings: [
			{
				am1: "44",
				am12: "440/4",
				am13: "441",
			},
		],
	},
	{
		id: "va_03.02.0_0008",
		kind: "legal",
		section: "03.02.0",
		test: "$am1 eq $am12 + $am13",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "45 = 450/3 + 454/9",
		bindings: [
			{
				am1: "45",
				am12: "450/3",
				am13: "454/9",
			},
		],
	},
	{
		id: "va_03.02.0_0009",
		kind: "legal",
		section: "03.02.0",
		test: "$am1 eq $am12 + $am13 + $am14 + $am15 + $am16 + $am17",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am15",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am16",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am17",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "42/48 = 42 + 43 + 44 + 46 + 45 + 48",
		bindings: [
			{
				am1: "42/48",
				am12: "42",
				am13: "43",
				am14: "44",
				am15: "46",
				am16: "45",
				am17: "48",
			},
		],
	},
	{
		id: "va_03.02.0_0010",
		kind: "legal",
		section: "03.02.0",
		test: "$am1 eq $am12 + $am13 + $am14",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "17/49 = 17 + 42/48 + 492/3",
		bindings: [
			{
				am1: "17/49",
				am12: "17",
				am13: "42/48",
				am14: "492/3",
			},
		],
	},
	{
		id: "va_03.02.0_0011",
		kind: "legal",
		section: "03.02.0",
		test: "$am1 eq $am2 + $am12 + $am13",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
		],
		equation: "10/49 = 10/15 + 16 + 17/49",
		bindings: [
			{
				am1: "10/49",
				am2: "10/15",
				am12: "16",
				am13: "17/49",
			},
		],
	},
	{
		id: "va_03.02.0_0012",
		kind: "legal",
		section: "03.02.0",
		test: "$am2 eq $am22 + $am1 - $am12",
		variables: [
			{
				name: "am2",
				metric: "am2",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am22",
				metric: "am2",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
		],
		equation: "14 = 9906 + 791 - 691",
		bindings: [
			{
				am2: "14",
				am22: "9906",
				am1: "791",
				am12: "691",
			},
		],
	},
	{
		id: "va_04.00.0_0002",
		kind: "legal",
		section: "04.00.0",
		test: "$am1 eq $am12 + $am13",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:part": "part:m4",
					"dim:bas": "bas:m52",
				},
				codes: ["75", "75/76B", "76B"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:part": "part:m4",
					"dim:typ": "typ:m8",
					"dim:bas": "bas:m52",
				},
				codes: ["75"],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {
					"dim:part": "part:m4",
					"dim:typ": "typ:m7",
					"dim:bas": "bas:m52",
				},
				codes: ["76B"],
				fallback: "0",
			},
		],
		equation: "75/76B = 75 + 76B",
		bindings: [
			{
				am1: "75/76B",
				am12: "75",
				am13: "76B",
			},
		],
	},
	{
		id: "va_04.00.0_0001",
		kind: "legal",
		section: "04.00.0",
		test: "$am2 eq $am22 - $am23 - $am1 - $am24 - $am25 - $am12 - $am3 - $am26",
		variables: [
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:part": "part:m4",
					"dim:ntr": "ntr:m6",
				},
				codes: ["631/4", "66A", "9901"],
				fallback: "0",
			},
			{
				name: "am22",
				metric: "am2",
				dimensions: {
					"dim:part": "part:m4",
					"dim:ntr": "ntr:m17",
					"dim:bas": "bas:m118",
				},
				codes: ["9900"],
				fallback: "0",
			},
			{
				name: "am23",
				metric: "am2",
				dimensions: {
					"dim:part": "part:m4",
					"dim:ntr": "ntr:m7",
				},
				codes: ["62"],
				fallback: "0",
			},
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:part": "part:m4",
					"dim:ntr": "ntr:m6",
					"dim:spec": "spec:m9",
					"dim:bkd": "bkd:m1",
				},
				codes: ["630"],
				fallback: "0",
			},
			{
				name: "am24",
				metric: "am2",
				dimensions: {
					"dim:part": "part:m4",
					"dim:ntr": "ntr:m6",
					"dim:mdp": "mdp:m2",
					"dim:spec": "spec:m10",
					"dim:bkd": "bkd:m2",
				},
				codes: ["631/4"],
				fallback: "0",
			},
			{
				name: "am25",
				metric: "am2",
				dimensions: {
					"dim:part": "part:m4",
				},
				codes: [
					"(9905)",
					"62",
					"631/4",
					"635/9",
					"65",
					"65/66B",
					"66A",
					"66B",
					"67/77",
					"73",
					"9900",
					"9901",
					"9903",
					"9904",
					"9905",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:part": "part:m4",
					"dim:typ": "typ:m2",
					"dim:ntr": "ntr:m6",
				},
				codes: ["640/8"],
				fallback: "0",
			},
			{
				name: "am3",
				metric: "am3",
				dimensions: {
					"dim:part": "part:m4",
					"dim:ntr": "ntr:m6",
					"dim:mdp": "mdp:m3",
					"dim:spec": "spec:m11",
				},
				codes: ["649"],
				fallback: "0",
			},
			{
				name: "am26",
				metric: "am2",
				dimensions: {
					"dim:part": "part:m4",
					"dim:typ": "typ:m7",
					"dim:ntr": "ntr:m6",
				},
				codes: ["66A"],
				fallback: "0",
			},
		],
		equation: "9901 = 9900 - 62 - 630 - 631/4 - 635/9 - 640/8 - 649 - 66A",
		bindings: [
			{
				am2: "9901",
				am22: "9900",
				am23: "62",
				am1: "630",
				am24: "631/4",
				am25: "635/9",
				am12: "640/8",
				am3: "649",
				am26: "66A",
			},
		],
	},
	{
		id: "va_04.00.0_0003",
		kind: "legal",
		section: "04.00.0",
		test: "$am2 eq $am22 + $am23",
		variables: [
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:part": "part:m4",
				},
				codes: ["65", "65/66B", "66B"],
				fallback: "0",
			},
			{
				name: "am22",
				metric: "am2",
				dimensions: {
					"dim:part": "part:m4",
					"dim:typ": "typ:m8",
				},
				codes: ["65"],
				fallback: "0",
			},
			{
				name: "am23",
				metric: "am2",
				dimensions: {
					"dim:part": "part:m4",
					"dim:typ": "typ:m7",
				},
				codes: ["66B"],
				fallback: "0",
			},
		],
		equation: "65/66B = 65 + 66B",
		bindings: [
			{
				am2: "65/66B",
				am22: "65",
				am23: "66B",
			},
		],
	},
	{
		id: "va_04.00.0_0004",
		kind: "legal",
		section: "04.00.0",
		test: "$am2 eq $am22 + $am1 - $am23",
		variables: [
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:part": "part:m4",
					"dim:bas": "bas:m59",
					"dim:spec": "spec:m16",
				},
				codes: ["9903"],
				fallback: "0",
			},
			{
				name: "am22",
				metric: "am2",
				dimensions: {
					"dim:part": "part:m4",
					"dim:ntr": "ntr:m6",
				},
				codes: ["631/4", "66A", "9901"],
				fallback: "0",
			},
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:part": "part:m4",
					"dim:bas": "bas:m52",
				},
				codes: ["75", "75/76B", "76A", "76B"],
				fallback: "0",
			},
			{
				name: "am23",
				metric: "am2",
				dimensions: {
					"dim:part": "part:m4",
				},
				codes: [
					"(9905)",
					"62",
					"631/4",
					"635/9",
					"65",
					"65/66B",
					"66A",
					"66B",
					"67/77",
					"73",
					"9900",
					"9901",
					"9903",
					"9904",
					"9905",
				],
				fallback: "0",
			},
		],
		equation: "9903 = 9901 + 75/76B - 65/66B",
		bindings: [
			{
				am2: "9903",
				am22: "9901",
				am1: "75/76B",
				am23: "65/66B",
			},
		],
	},
	{
		id: "va_04.00.0_0005",
		kind: "legal",
		section: "04.00.0",
		test: "$am2 eq $am22 + $am1 - $am12 - $am23",
		variables: [
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:part": "part:m4",
					"dim:bas": "bas:m59",
				},
				codes: ["9904"],
				fallback: "0",
			},
			{
				name: "am22",
				metric: "am2",
				dimensions: {
					"dim:part": "part:m4",
					"dim:bas": "bas:m59",
					"dim:spec": "spec:m16",
				},
				codes: ["9903"],
				fallback: "0",
			},
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:part": "part:m4",
					"dim:mdp": "mdp:m4",
				},
				codes: ["780", "789"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:part": "part:m4",
					"dim:mdp": "mdp:m5",
				},
				codes: ["680", "689"],
				fallback: "0",
			},
			{
				name: "am23",
				metric: "am2",
				dimensions: {
					"dim:part": "part:m4",
					"dim:spec": "spec:m17",
				},
				codes: ["67/77"],
				fallback: "0",
			},
		],
		equation: "9904 = 9903 + 780 - 680 - 67/77",
		bindings: [
			{
				am2: "9904",
				am22: "9903",
				am1: "780",
				am12: "680",
				am23: "67/77",
			},
		],
	},
	{
		id: "va_04.00.0_0006",
		kind: "legal",
		section: "04.00.0",
		test: "$am2 eq $am22 + $am1 - $am12",
		variables: [
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:sts": "sts:m11",
					"dim:part": "part:m4",
					"dim:bas": "bas:m59",
				},
				codes: ["(9905)", "9905"],
				fallback: "0",
			},
			{
				name: "am22",
				metric: "am2",
				dimensions: {
					"dim:part": "part:m4",
					"dim:bas": "bas:m59",
				},
				codes: ["9904"],
				fallback: "0",
			},
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:sts": "sts:m6",
					"dim:part": "part:m4",
					"dim:bas": "bas:m43",
					"dim:mdp": "mdp:m4",
				},
				codes: ["789"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:sts": "sts:m6",
					"dim:part": "part:m4",
					"dim:bas": "bas:m43",
					"dim:mdp": "mdp:m5",
				},
				codes: ["689"],
				fallback: "0",
			},
		],
		equation: "9905 = 9904 + 789 - 689",
		bindings: [
			{
				am2: "9905",
				am22: "9904",
				am1: "789",
				am12: "689",
			},
		],
	},
	{
		id: "va_04.00.0_0007",
		kind: "legal",
		section: "04.00.0",
		test: "$am1 ge $am12 + $am13",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:part": "part:m4",
					"dim:typ": "typ:m7",
					"dim:ntr": "ntr:m6",
					"dim:bas": "bas:m52",
				},
				codes: ["76A"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:bkd": "bkd:m1",
				},
				codes: ["8129", "8329"],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {
					"dim:bkd": "bkd:m1",
				},
				codes: ["8129", "8329"],
				fallback: "0",
			},
		],
		equation: "76A ≥ 8089 + 8289",
		bindings: [
			{
				am1: "76A",
				am12: "8089",
				am13: "8289",
			},
		],
	},
	{
		id: "va_04.00.0_0008",
		kind: "legal",
		section: "04.00.0",
		test: "$am1 ge $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:part": "part:m4",
					"dim:typ": "typ:m7",
					"dim:bas": "bas:m52",
				},
				codes: ["76A", "76B"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:bkd": "bkd:m2",
				},
				codes: ["8475", "8485", "8495", "8505", "8525", "8525P"],
				fallback: "0",
			},
		],
		equation: "76B ≥ 8485",
		bindings: [
			{
				am1: "76B",
				am12: "8485",
			},
		],
	},
	{
		id: "va_05.00.0_0001",
		kind: "legal",
		section: "05.00.0",
		test: "$am2 eq $am22 + $am23",
		variables: [
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:part": "part:m5",
				},
				codes: ["14P"],
				fallback: "0",
			},
			{
				name: "am22",
				metric: "am2",
				dimensions: {},
				codes: ["14P"],
				fallback: "0",
			},
			{
				name: "am23",
				metric: "am2",
				dimensions: {
					"dim:part": "part:m5",
					"dim:bas": "bas:m61",
				},
				codes: ["14P"],
				fallback: "0",
			},
		],
		equation: "9906 = 9905 + 14P",
		bindings: [
			{
				am2: "9906",
				am22: "9905",
				am23: "14P",
			},
		],
	},
	{
		id: "va_06.01.1_0001",
		kind: "legal",
		section: "06.01.1",
		test: "$am1 eq $am12 + $am13 - $am14 + $am2",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:part": "part:m6",
					"dim:bkd": "bkd:m4",
				},
				codes: ["8059", "8199", "8395"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:aln": "aln:m1",
					"dim:rut": "rut:m1",
					"dim:part": "part:m6",
					"dim:bkd": "bkd:m4",
				},
				codes: ["8059P", "8199P", "8395P"],
				period: "prd:m2",
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {
					"dim:part": "part:m6",
					"dim:mdp": "mdp:m6",
					"dim:bkd": "bkd:m4",
				},
				codes: ["8029", "8169", "8365"],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {
					"dim:part": "part:m6",
					"dim:mdp": "mdp:m8",
				},
				codes: ["8039", "8179", "8375"],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:part": "part:m6",
					"dim:mdp": "mdp:m9",
				},
				codes: ["8049", "8189", "8385"],
				fallback: "0",
			},
		],
		equation: "8059 = 8059P + 8029 - 8039 + 8049",
		bindings: [
			{
				am1: "8059",
				am12: "8059P",
				am13: "8029",
				am14: "8039",
				am2: "8049",
			},
			{
				am1: "8199",
				am12: "8199P",
				am13: "8169",
				am14: "8179",
				am2: "8189",
			},
			{
				am1: "8395",
				am12: "8395P",
				am13: "8365",
				am14: "8375",
				am2: "8385",
			},
		],
	},
	{
		id: "va_06.01.1_0002",
		kind: "legal",
		section: "06.01.1",
		test: "$am1 eq $am12 + $am13 - $am14 + $am15 - $am16 + $am2",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:part": "part:m6",
				},
				codes: ["8129", "8329", "8525"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:aln": "aln:m1",
					"dim:rut": "rut:m1",
					"dim:part": "part:m6",
				},
				codes: ["8129P", "8329P", "8525P"],
				period: "prd:m2",
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {
					"dim:part": "part:m6",
					"dim:mdp": "mdp:m6",
				},
				codes: ["8079", "8279", "8475"],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {
					"dim:part": "part:m6",
					"dim:mdp": "mdp:m10",
				},
				codes: ["8089", "8289", "8485"],
				fallback: "0",
			},
			{
				name: "am15",
				metric: "am1",
				dimensions: {
					"dim:part": "part:m6",
					"dim:mdp": "mdp:m11",
				},
				codes: ["8099", "8299", "8495"],
				fallback: "0",
			},
			{
				name: "am16",
				metric: "am1",
				dimensions: {
					"dim:part": "part:m6",
					"dim:mdp": "mdp:m12",
				},
				codes: ["8109", "8309", "8505"],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:part": "part:m6",
					"dim:mdp": "mdp:m9",
				},
				codes: ["8119", "8319", "8515"],
				fallback: "0",
			},
		],
		equation: "8129 = 8129P + 8079 - 8089 + 8099 - 8109 + 8119",
		bindings: [
			{
				am1: "8129",
				am12: "8129P",
				am13: "8079",
				am14: "8089",
				am15: "8099",
				am16: "8109",
				am2: "8119",
			},
			{
				am1: "8329",
				am12: "8329P",
				am13: "8279",
				am14: "8289",
				am15: "8299",
				am16: "8309",
				am2: "8319",
			},
			{
				am1: "8525",
				am12: "8525P",
				am13: "8475",
				am14: "8485",
				am15: "8495",
				am16: "8505",
				am2: "8515",
			},
		],
	},
	{
		id: "va_06.01.2_0001",
		kind: "legal",
		section: "06.01.2",
		test: "$am1 eq $am12 + $am13 - $am14 + $am2",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
		],
		equation: "8199 = 8199P + 8169 - 8179 + 8189",
		bindings: [
			{
				am1: "8199",
				am12: "8199P",
				am13: "8169",
				am14: "8179",
				am2: "8189",
			},
		],
	},
	{
		id: "va_06.01.2_0002",
		kind: "legal",
		section: "06.01.2",
		test: "$am1 eq $am12 + $am13 + $am14 - $am15 + $am2",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:bkd": "bkd:m5",
				},
				codes: [
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:bkd": "bkd:m5",
				},
				codes: [
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {
					"dim:bkd": "bkd:m5",
				},
				codes: [
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
				],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {
					"dim:bkd": "bkd:m5",
				},
				codes: [
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
				],
				fallback: "0",
			},
			{
				name: "am15",
				metric: "am1",
				dimensions: {
					"dim:bkd": "bkd:m5",
				},
				codes: [
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
				],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:bkd": "bkd:m5",
				},
				codes: ["8249", "8445"],
				fallback: "0",
			},
		],
		equation: "8259 = 8259P + 8219 + 8229 - 8239 + 8249",
		bindings: [
			{
				am1: "8259",
				am12: "8259P",
				am13: "8219",
				am14: "8229",
				am15: "8239",
				am2: "8249",
			},
		],
	},
	{
		id: "va_06.01.2_0003",
		kind: "legal",
		section: "06.01.2",
		test: "$am1 eq $am12 + $am13 - $am14 + $am15 - $am16 + $am2",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am15",
				metric: "am1",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am16",
				metric: "am1",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
		],
		equation: "8329 = 8329P + 8279 - 8289 + 8299 - 8309 + 8319",
		bindings: [
			{
				am1: "8329",
				am12: "8329P",
				am13: "8279",
				am14: "8289",
				am15: "8299",
				am16: "8309",
				am2: "8319",
			},
		],
	},
	{
		id: "va_06.01.3_0001",
		kind: "legal",
		section: "06.01.3",
		test: "$am1 eq $am12 + $am13 - $am14 + $am2 + $am22",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
					"dim:bas": "bas:m70",
				},
				codes: ["8365", "8375", "8395", "8395P"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
					"dim:bas": "bas:m70",
				},
				codes: ["8365", "8375", "8395", "8395P"],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
					"dim:bas": "bas:m70",
				},
				codes: ["8365", "8375", "8395", "8395P"],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
					"dim:bas": "bas:m70",
				},
				codes: ["8365", "8375", "8395", "8395P"],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
					"dim:bas": "bas:m70",
				},
				codes: ["8385", "8386"],
				fallback: "0",
			},
			{
				name: "am22",
				metric: "am2",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
					"dim:mdp": "mdp:m7",
					"dim:bas": "bas:m70",
				},
				codes: ["8386"],
				fallback: "0",
			},
		],
		equation: "8395 = 8395P + 8365 - 8375 + 8385 + 8386",
		bindings: [
			{
				am1: "8395",
				am12: "8395P",
				am13: "8365",
				am14: "8375",
				am2: "8385",
				am22: "8386",
			},
		],
	},
	{
		id: "va_06.01.3_0002",
		kind: "legal",
		section: "06.01.3",
		test: "$am1 eq $am12 + $am13 + $am14 - $am15 + $am2",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
				},
				codes: [
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
				},
				codes: [
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
				},
				codes: [
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
				],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
				},
				codes: [
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
				],
				fallback: "0",
			},
			{
				name: "am15",
				metric: "am1",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
				},
				codes: [
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
				],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
				},
				codes: ["8385", "8386", "8445", "8515", "8545"],
				fallback: "0",
			},
		],
		equation: "8455 = 8455P + 8415 + 8425 - 8435 + 8445",
		bindings: [
			{
				am1: "8455",
				am12: "8455P",
				am13: "8415",
				am14: "8425",
				am15: "8435",
				am2: "8445",
			},
		],
	},
	{
		id: "va_06.01.3_0003",
		kind: "legal",
		section: "06.01.3",
		test: "$am1 eq $am12 + $am13 - $am14 + $am15 - $am16 + $am2",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
				},
				codes: [
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
				},
				codes: [
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
				},
				codes: [
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
				],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
				},
				codes: [
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
				],
				fallback: "0",
			},
			{
				name: "am15",
				metric: "am1",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
				},
				codes: [
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
				],
				fallback: "0",
			},
			{
				name: "am16",
				metric: "am1",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
				},
				codes: [
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
				],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
				},
				codes: ["8385", "8386", "8445", "8515", "8545"],
				fallback: "0",
			},
		],
		equation: "8525 = 8525P + 8475 - 8485 + 8495 - 8505 + 8515",
		bindings: [
			{
				am1: "8525",
				am12: "8525P",
				am13: "8475",
				am14: "8485",
				am15: "8495",
				am16: "8505",
				am2: "8515",
			},
		],
	},
	{
		id: "va_06.01.3_0004",
		kind: "legal",
		section: "06.01.3",
		test: "$am1 eq $am12 + $am2",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
					"dim:bkd": "bkd:m6",
				},
				codes: ["8555", "8555P"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
					"dim:bkd": "bkd:m6",
				},
				codes: ["8555", "8555P"],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:rls": "rls:m1",
					"dim:psn": "psn:m1",
					"dim:bkd": "bkd:m6",
				},
				codes: ["8545"],
				fallback: "0",
			},
		],
		equation: "8555 = 8555P + 8545",
		bindings: [
			{
				am1: "8555",
				am12: "8555P",
				am2: "8545",
			},
		],
	},
	{
		id: "ca_04.00.0_0001",
		kind: "nbb",
		section: "04.00.0",
		test: "$am1 eq $am12 + $am13",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "630 = 8079 + 8279",
		bindings: [
			{
				am1: "630",
				am12: "8079",
				am13: "8279",
			},
		],
	},
	{
		id: "ca_04.00.0_0002",
		kind: "nbb",
		section: "04.00.0",
		test: "$am2 ge $am1",
		variables: [
			{
				name: "am2",
				metric: "am2",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
		],
		equation: "66B ≥ 8475",
		bindings: [
			{
				am2: "66B",
				am1: "8475",
			},
		],
	},
	{
		id: "ca_06.02.0_0001",
		kind: "nbb",
		section: "06.02.0",
		test: "$am1 le $am12 + $am13",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:qlt": "qlt:m2",
					"dim:spu": "spu:m25",
					"dim:typ": "typ:m22",
					"dim:ntr": "ntr:m13",
					"dim:gtd": "gtd:m3",
					"dim:bas": "bas:m87",
					"dim:spec": "spec:m60",
				},
				codes: ["91611"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"8349",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"8349",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
		],
		equation: "91611 ≤ 22/27 + 30/36",
		bindings: [
			{
				am1: "91611",
				am12: "22/27",
				am13: "30/36",
			},
		],
	},
	{
		id: "ca_06.02.0_0003",
		kind: "nbb",
		section: "06.02.0",
		test: "$am1 le $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:spu": "spu:m25",
					"dim:typ": "typ:m24",
					"dim:ntr": "ntr:m13",
					"dim:gtd": "gtd:m3",
					"dim:bas": "bas:m87",
					"dim:spec": "spec:m63",
				},
				codes: ["91711"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"8349",
					"9149",
					"9150",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
		],
		equation: "91711 ≤ 20/58",
		bindings: [
			{
				am1: "91711",
				am12: "20/58",
			},
		],
	},
	{
		id: "ca_06.02.0_0004",
		kind: "nbb",
		section: "06.02.0",
		test: "$am1 le $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:spu": "spu:m25",
					"dim:typ": "typ:m25",
					"dim:ntr": "ntr:m13",
					"dim:gtd": "gtd:m3",
					"dim:bas": "bas:m87",
					"dim:spec": "spec:m64",
				},
				codes: ["91721"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"8349",
					"9149",
					"9150",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
		],
		equation: "91721 ≤ 20/58",
		bindings: [
			{
				am1: "91721",
				am12: "20/58",
			},
		],
	},
	{
		id: "ca_06.02.0_0007",
		kind: "nbb",
		section: "06.02.0",
		test: "$am1 le $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:qlt": "qlt:m2",
					"dim:spu": "spu:m25",
					"dim:typ": "typ:m26",
					"dim:ntr": "ntr:m13",
					"dim:gtd": "gtd:m3",
					"dim:bas": "bas:m87",
					"dim:spec": "spec:m60",
				},
				codes: ["91811"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"8349",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
		],
		equation: "91811 ≤ 20/58",
		bindings: [
			{
				am1: "91811",
				am12: "20/58",
			},
		],
	},
	{
		id: "ca_06.02.0_0009",
		kind: "nbb",
		section: "06.02.0",
		test: "$am1 le $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:spu": "spu:m25",
					"dim:typ": "typ:m26",
					"dim:ntr": "ntr:m13",
					"dim:gtd": "gtd:m3",
					"dim:bas": "bas:m87",
					"dim:spec": "spec:m65",
				},
				codes: ["91821"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"8349",
					"9149",
					"9150",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
		],
		equation: "91821 ≤ 20/58",
		bindings: [
			{
				am1: "91821",
				am12: "20/58",
			},
		],
	},
	{
		id: "ca_06.02.0_0011",
		kind: "nbb",
		section: "06.02.0",
		test: "$am1 le $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:spu": "spu:m25",
					"dim:typ": "typ:m27",
					"dim:ntr": "ntr:m13",
					"dim:gtd": "gtd:m3",
					"dim:bas": "bas:m87",
					"dim:spec": "spec:m66",
				},
				codes: ["91911"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"8349",
					"9149",
					"9150",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
		],
		equation: "91911 ≤ 20/58",
		bindings: [
			{
				am1: "91911",
				am12: "20/58",
			},
		],
	},
	{
		id: "ca_06.02.0_0013",
		kind: "nbb",
		section: "06.02.0",
		test: "$am1 le $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:spu": "spu:m25",
					"dim:typ": "typ:m27",
					"dim:ntr": "ntr:m13",
					"dim:gtd": "gtd:m3",
					"dim:bas": "bas:m87",
					"dim:spec": "spec:m65",
				},
				codes: ["91921"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"8349",
					"9149",
					"9150",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
		],
		equation: "91921 ≤ 20/58",
		bindings: [
			{
				am1: "91921",
				am12: "20/58",
			},
		],
	},
	{
		id: "ca_06.02.0_0015",
		kind: "nbb",
		section: "06.02.0",
		test: "$am1 le $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:qlt": "qlt:m2",
					"dim:spu": "spu:m25",
					"dim:typ": "typ:m28",
					"dim:ntr": "ntr:m13",
					"dim:gtd": "gtd:m3",
					"dim:bas": "bas:m87",
					"dim:spec": "spec:m67",
				},
				codes: ["92011"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"8349",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
		],
		equation: "92011 ≤ 20/58",
		bindings: [
			{
				am1: "92011",
				am12: "20/58",
			},
		],
	},
	{
		id: "ca_06.02.0_0016",
		kind: "nbb",
		section: "06.02.0",
		test: "$am1 le $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:spu": "spu:m25",
					"dim:typ": "typ:m28",
					"dim:ntr": "ntr:m13",
					"dim:gtd": "gtd:m3",
					"dim:bas": "bas:m87",
					"dim:spec": "spec:m68",
				},
				codes: ["92021"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"8349",
					"9149",
					"9150",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92021",
					"92022",
					"9220",
				],
				fallback: "0",
			},
		],
		equation: "92021 ≤ 20/58",
		bindings: [
			{
				am1: "92021",
				am12: "20/58",
			},
		],
	},
	{
		id: "ca_06.02.0_0019",
		kind: "nbb",
		section: "06.02.0",
		test: "$am1 != 0 and ($am12 != 0 or $am13 != 0)",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:qlt": "qlt:m2",
					"dim:spu": "spu:m25",
					"dim:typ": "typ:m22",
					"dim:ntr": "ntr:m13",
					"dim:gtd": "gtd:m3",
					"dim:bas": "bas:m87",
					"dim:spec": "spec:m60",
				},
				codes: ["91611"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:spu": "spu:m25",
					"dim:typ": "typ:m22",
					"dim:ntr": "ntr:m13",
					"dim:gtd": "gtd:m3",
					"dim:bas": "bas:m87",
					"dim:spec": "spec:m61",
				},
				codes: ["91621"],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {
					"dim:spu": "spu:m25",
					"dim:typ": "typ:m23",
					"dim:ntr": "ntr:m13",
					"dim:gtd": "gtd:m3",
					"dim:bas": "bas:m87",
					"dim:spec": "spec:m62",
				},
				codes: ["91631"],
				fallback: "0",
			},
		],
		equation:
			"If ( 91611 >< '0' ) or ( 91621 >< '0' ) or ( 91631 >< '0' ) then ( 91611 >< '0' ) and (( 91621 >< '0' ) or ( 91631 >< '0' ))",
		bindings: [
			{
				am1: "91611",
				am12: "91621",
				am13: "91631",
			},
		],
	},
	{
		id: "ca_06.02.0_0002",
		kind: "nbb",
		section: "06.02.0",
		test: "$am1 le $am12 + $am13",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:spu": "spu:m17",
				},
				codes: [
					"91612",
					"91622",
					"91632",
					"91712",
					"91722",
					"91812",
					"91822",
					"91912",
					"91922",
					"92012",
					"92022",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
				],
				fallback: "0",
			},
		],
		equation: "91612 ≤ 22/27 + 30/36",
		bindings: [
			{
				am1: "91612",
				am12: "22/27",
				am13: "30/36",
			},
		],
	},
	{
		id: "ca_06.02.0_0005",
		kind: "nbb",
		section: "06.02.0",
		test: "$am1 le $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:spu": "spu:m17",
				},
				codes: [
					"91612",
					"91622",
					"91632",
					"91712",
					"91722",
					"91812",
					"91822",
					"91912",
					"91922",
					"92012",
					"92022",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
				],
				fallback: "0",
			},
		],
		equation: "91712 ≤ 20/58",
		bindings: [
			{
				am1: "91712",
				am12: "20/58",
			},
		],
	},
	{
		id: "ca_06.02.0_0006",
		kind: "nbb",
		section: "06.02.0",
		test: "$am1 le $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:spu": "spu:m17",
				},
				codes: [
					"91612",
					"91622",
					"91632",
					"91712",
					"91722",
					"91812",
					"91822",
					"91912",
					"91922",
					"92012",
					"92022",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
				],
				fallback: "0",
			},
		],
		equation: "91722 ≤ 20/58",
		bindings: [
			{
				am1: "91722",
				am12: "20/58",
			},
		],
	},
	{
		id: "ca_06.02.0_0008",
		kind: "nbb",
		section: "06.02.0",
		test: "$am1 le $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:spu": "spu:m17",
				},
				codes: [
					"91612",
					"91622",
					"91632",
					"91712",
					"91722",
					"91812",
					"91822",
					"91912",
					"91922",
					"92012",
					"92022",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
				],
				fallback: "0",
			},
		],
		equation: "91812 ≤ 20/58",
		bindings: [
			{
				am1: "91812",
				am12: "20/58",
			},
		],
	},
	{
		id: "ca_06.02.0_0010",
		kind: "nbb",
		section: "06.02.0",
		test: "$am1 le $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:spu": "spu:m17",
				},
				codes: [
					"91612",
					"91622",
					"91632",
					"91712",
					"91722",
					"91812",
					"91822",
					"91912",
					"91922",
					"92012",
					"92022",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
				],
				fallback: "0",
			},
		],
		equation: "91822 ≤ 20/58",
		bindings: [
			{
				am1: "91822",
				am12: "20/58",
			},
		],
	},
	{
		id: "ca_06.02.0_0012",
		kind: "nbb",
		section: "06.02.0",
		test: "$am1 le $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:spu": "spu:m17",
				},
				codes: [
					"91612",
					"91622",
					"91632",
					"91712",
					"91722",
					"91812",
					"91822",
					"91912",
					"91922",
					"92012",
					"92022",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
				],
				fallback: "0",
			},
		],
		equation: "91912 ≤ 20/58",
		bindings: [
			{
				am1: "91912",
				am12: "20/58",
			},
		],
	},
	{
		id: "ca_06.02.0_0014",
		kind: "nbb",
		section: "06.02.0",
		test: "$am1 le $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:spu": "spu:m17",
				},
				codes: [
					"91612",
					"91622",
					"91632",
					"91712",
					"91722",
					"91812",
					"91822",
					"91912",
					"91922",
					"92012",
					"92022",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
				],
				fallback: "0",
			},
		],
		equation: "91922 ≤ 20/58",
		bindings: [
			{
				am1: "91922",
				am12: "20/58",
			},
		],
	},
	{
		id: "ca_06.02.0_0017",
		kind: "nbb",
		section: "06.02.0",
		test: "$am1 le $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:spu": "spu:m17",
				},
				codes: [
					"91612",
					"91622",
					"91632",
					"91712",
					"91722",
					"91812",
					"91822",
					"91912",
					"91922",
					"92012",
					"92022",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
				],
				fallback: "0",
			},
		],
		equation: "92012 ≤ 20/58",
		bindings: [
			{
				am1: "92012",
				am12: "20/58",
			},
		],
	},
	{
		id: "ca_06.02.0_0018",
		kind: "nbb",
		section: "06.02.0",
		test: "$am1 le $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:spu": "spu:m17",
				},
				codes: [
					"91612",
					"91622",
					"91632",
					"91712",
					"91722",
					"91812",
					"91822",
					"91912",
					"91922",
					"92012",
					"92022",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
				],
				fallback: "0",
			},
		],
		equation: "92022 ≤ 20/58",
		bindings: [
			{
				am1: "92022",
				am12: "20/58",
			},
		],
	},
	{
		id: "ca_06.02.0_0020",
		kind: "nbb",
		section: "06.02.0",
		test: "$am1 != 0 and ($am12 != 0 or $am13 != 0)",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:spu": "spu:m17",
				},
				codes: [
					"91612",
					"91622",
					"91632",
					"91712",
					"91722",
					"91812",
					"91822",
					"91912",
					"91922",
					"92012",
					"92022",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:spu": "spu:m17",
				},
				codes: [
					"91612",
					"91622",
					"91632",
					"91712",
					"91722",
					"91812",
					"91822",
					"91912",
					"91922",
					"92012",
					"92022",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {
					"dim:spu": "spu:m17",
				},
				codes: [
					"91612",
					"91622",
					"91632",
					"91712",
					"91722",
					"91812",
					"91822",
					"91912",
					"91922",
					"92012",
					"92022",
				],
				fallback: "0",
			},
		],
		equation:
			"If ( 91612 >< '0' ) or ( 91622 >< '0' ) or ( 91632 >< '0' ) then ( 91612 >< '0' ) and (( 91622 >< '0' ) or ( 91632 >< '0' ))",
		bindings: [
			{
				am1: "91612",
				am12: "91622",
				am13: "91632",
			},
		],
	},
	{
		id: "ca_06.03.0_0001",
		kind: "nbb",
		section: "06.03.0",
		test: "$am1 le $am12 + $am13",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:psn": "psn:m31",
					"dim:spec": "spec:m74",
				},
				codes: ["9500"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: ["9500"],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: ["9500"],
				fallback: "0",
			},
		],
		equation: "9500 ≤ 291 + 41",
		bindings: [
			{
				am1: "9500",
				am12: "291",
				am13: "41",
			},
		],
	},
	{
		id: "ca_06.03.0_0002",
		kind: "nbb",
		section: "06.03.0",
		test: "$am1 le $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:gtd": "gtd:m6",
					"dim:bas": "bas:m102",
					"dim:bnf": "bnf:m6",
				},
				codes: ["9501"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: ["9501"],
				fallback: "0",
			},
		],
		equation: "9501 ≤ 20/58",
		bindings: [
			{
				am1: "9501",
				am12: "20/58",
			},
		],
	},
	{
		id: "ca_06.03.0_0003",
		kind: "nbb",
		section: "06.03.0",
		test: "$am1 le $am12",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:sts": "sts:m23",
					"dim:gtd": "gtd:m6",
					"dim:bas": "bas:m89",
					"dim:bnf": "bnf:m6",
				},
				codes: ["9502"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: ["9502"],
				fallback: "0",
			},
		],
		equation: "9502 ≤ 20/58",
		bindings: [
			{
				am1: "9502",
				am12: "20/58",
			},
		],
	},
	{
		id: "ca_03.02.0_0001",
		kind: "nbb",
		section: "03.02.0",
		test: "$am1 eq $am12 + $am13 - $am14 + $am15 - $am16",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am15",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am16",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "12 = 12P + 8259 - 8259P + 8455 - 8455P",
		bindings: [
			{
				am1: "12",
				am12: "12",
				am13: "8259",
				am14: "8259P",
				am15: "8455",
				am16: "8455P",
			},
		],
		precedingColumn: ["am12"],
	},
	{
		id: "ca_03.01.0_0001",
		kind: "nbb",
		section: "03.01.0",
		test: "$am1 eq $am12 - $am13",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "21P = 8059P - 8129P",
		bindings: [
			{
				am1: "21",
				am12: "8059P",
				am13: "8129P",
			},
		],
		precedingColumn: ["am1"],
	},
	{
		id: "ca_03.01.0_0002",
		kind: "nbb",
		section: "03.01.0",
		test: "$am1 eq $am12 + $am13 - $am14",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "22/27P = 8199P + 8259P - 8329P",
		bindings: [
			{
				am1: "22/27",
				am12: "8199P",
				am13: "8259P",
				am14: "8329P",
			},
		],
		precedingColumn: ["am1"],
	},
	{
		id: "ca_03.01.0_0003",
		kind: "nbb",
		section: "03.01.0",
		test: "$am1 eq $am12 + $am13 - $am14 - $am15",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
			{
				name: "am15",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "28P = 8395P + 8455P - 8525P - 8555P",
		bindings: [
			{
				am1: "28",
				am12: "8395P",
				am13: "8455P",
				am14: "8525P",
				am15: "8555P",
			},
		],
		precedingColumn: ["am1"],
	},
	{
		id: "ca_03.01.0_0004",
		kind: "nbb",
		section: "03.01.0",
		test: "$am1 gt 0",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {},
				codes: [
					"(21)",
					"(22/27)",
					"(28)",
					"10/49",
					"12",
					"13",
					"15",
					"16",
					"160",
					"160/5",
					"161",
					"162",
					"163",
					"164/5",
					"167",
					"168",
					"17",
					"17/49",
					"170/4",
					"172/3",
					"174/0",
					"175",
					"176",
					"178/9",
					"20",
					"20/58",
					"21",
					"21/28",
					"22",
					"22/27",
					"23",
					"24",
					"25",
					"26",
					"27",
					"28",
					"29",
					"29/58",
					"290",
					"291",
					"3",
					"30/36",
					"37",
					"40",
					"40/41",
					"41",
					"42",
					"42/48",
					"43",
					"430/8",
					"439",
					"44",
					"440/4",
					"441",
					"45",
					"450/3",
					"454/9",
					"46",
					"48",
					"490/1",
					"492/3",
					"50/53",
					"54/58",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5823-5833",
					"5843-5853",
					"60/61",
					"630",
					"640/8",
					"680",
					"689",
					"691",
					"70",
					"75",
					"75/76B",
					"76A",
					"76B",
					"780",
					"789",
					"791",
					"8029",
					"8039",
					"8059",
					"8059P",
					"8079",
					"8089",
					"8099",
					"8109",
					"8129",
					"8129P",
					"8169",
					"8179",
					"8199",
					"8199P",
					"8219",
					"8229",
					"8239",
					"8259",
					"8259P",
					"8279",
					"8289",
					"8299",
					"8309",
					"8329",
					"8329P",
					"8349",
					"8365",
					"8375",
					"8395",
					"8395P",
					"8415",
					"8425",
					"8435",
					"8455",
					"8455P",
					"8475",
					"8485",
					"8495",
					"8505",
					"8525",
					"8525P",
					"8555",
					"8555P",
					"9149",
					"9150",
					"91611",
					"91612",
					"91621",
					"91622",
					"91631",
					"91632",
					"91711",
					"91712",
					"91721",
					"91722",
					"91811",
					"91812",
					"91821",
					"91822",
					"91911",
					"91912",
					"91921",
					"91922",
					"92011",
					"92012",
					"92021",
					"92022",
					"9220",
					"9500",
					"9501",
					"9502",
				],
				fallback: "0",
			},
		],
		equation: "20/58 > '0'",
		bindings: [
			{
				am1: "20/58",
			},
		],
	},
	{
		id: "cx_06.04.0_0001",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $am2",
		variables: [
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m3",
					"dim:part": "part:m8",
					"dim:typ": "typ:m16",
					"dim:bas": "bas:m73",
				},
				codes: ["1001-1002-1003-1003P"],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {},
				codes: ["635/9", "9904"],
				fallback: "0",
			},
		],
		equation: "If 1003 is completed, then 1003 < 62",
		bindings: [
			{
				dec1: "1001-1002-1003-1003P",
				am2: "635/9",
			},
			{
				dec1: "1001-1002-1003-1003P",
				am2: "9904",
			},
		],
	},
	{
		id: "cx_06.04.0_0002",
		kind: "social-balance",
		section: "06.04.0",
		test: "$am2 le $am22",
		variables: [
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m3",
					"dim:part": "part:m8",
				},
				codes: ["1021-1022-1023-1023P"],
				fallback: "0",
			},
			{
				name: "am22",
				metric: "am2",
				dimensions: {},
				codes: ["1021-1022-1023-1023P", "62"],
				fallback: "0",
			},
		],
		equation: "1023 ≤ 62",
		bindings: [
			{
				am2: "1021-1022-1023-1023P",
				am22: "62",
			},
			{
				am2: "1021-1022-1023-1023P",
				am22: "1021-1022-1023-1023P",
			},
		],
	},
	{
		id: "cs_06.04.0_0001",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 gt 0 and $int4 gt 0 and $am2 gt 0 and $dec1 lt $int4",
		variables: [
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m1",
					"dim:part": "part:m8",
					"dim:typ": "typ:m16",
					"dim:bas": "bas:m73",
				},
				codes: ["1001-1002-1003-1003P"],
				fallback: "0",
			},
			{
				name: "int4",
				metric: "int4",
				dimensions: {
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m1",
					"dim:part": "part:m8",
					"dim:hrs": "hrs:m1",
					"dim:bas": "bas:m73",
					"dim:wkr": "wkr:m1",
				},
				codes: ["1011-1012-1013-1013P"],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m1",
					"dim:part": "part:m8",
				},
				codes: ["1021-1022-1023-1023P"],
				fallback: "0",
			},
		],
		equation:
			"If 1001 > 0 or 1011 > 0 or 1021 > 0 then 1001 > 0 and 1011 > 0 and 1021 > 0 and 1001 < 1011",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0002",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec12 eq $dec13",
		variables: [
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m2",
					"dim:part": "part:m8",
					"dim:typ": "typ:m16",
					"dim:bas": "bas:m73",
				},
				codes: ["1001-1002-1003-1003P"],
				fallback: "()",
			},
			{
				name: "dec12",
				metric: "dec1",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m3",
					"dim:part": "part:m8",
					"dim:typ": "typ:m16",
					"dim:bas": "bas:m73",
				},
				codes: ["1001-1002-1003-1003P"],
				fallback: "0",
			},
			{
				name: "dec13",
				metric: "dec1",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m1",
					"dim:part": "part:m8",
					"dim:typ": "typ:m16",
					"dim:bas": "bas:m73",
				},
				codes: ["1001-1002-1003-1003P"],
				fallback: "0",
			},
		],
		equation: "If 1002 is not completed then 1003 = 1001",
		bindings: [
			{
				dec1: "1001-1002-1003-1003P",
				dec12: "1001-1002-1003-1003P",
				dec13: "1001-1002-1003-1003P",
			},
		],
	},
	{
		id: "cs_06.04.0_0003",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 gt 0 and $int4 gt 0 and $am2 gt 0 and $dec1 lt $int4",
		variables: [
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m2",
					"dim:part": "part:m8",
					"dim:typ": "typ:m16",
					"dim:bas": "bas:m73",
				},
				codes: ["1001-1002-1003-1003P"],
				fallback: "0",
			},
			{
				name: "int4",
				metric: "int4",
				dimensions: {
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m2",
					"dim:part": "part:m8",
					"dim:hrs": "hrs:m1",
					"dim:bas": "bas:m73",
					"dim:wkr": "wkr:m1",
				},
				codes: ["1011-1012-1013-1013P"],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m2",
					"dim:part": "part:m8",
				},
				codes: ["1021-1022-1023-1023P"],
				fallback: "0",
			},
		],
		equation:
			"If 1002 > 0 or 1012 > 0 or 1022 > 0 then 1002 > 0 and 1012 > 0 and 1022 > 0 and 1002 < 1012",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0004",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec12 gt $dec13 and $dec12 lt $dec13 + $dec1",
		variables: [
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m2",
					"dim:part": "part:m8",
					"dim:typ": "typ:m16",
					"dim:bas": "bas:m73",
				},
				codes: ["1001-1002-1003-1003P"],
				fallback: "0",
			},
			{
				name: "dec12",
				metric: "dec1",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m3",
					"dim:part": "part:m8",
					"dim:typ": "typ:m16",
					"dim:bas": "bas:m73",
				},
				codes: ["1001-1002-1003-1003P"],
				fallback: "0",
			},
			{
				name: "dec13",
				metric: "dec1",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m1",
					"dim:part": "part:m8",
					"dim:typ": "typ:m16",
					"dim:bas": "bas:m73",
				},
				codes: ["1001-1002-1003-1003P"],
				fallback: "0",
			},
		],
		equation: "If 1002 > 0 then 1003 < 1001 + 1002 and 1003 > 1001",
		bindings: [
			{
				dec1: "1001-1002-1003-1003P",
				dec12: "1001-1002-1003-1003P",
				dec13: "1001-1002-1003-1003P",
			},
		],
	},
	{
		id: "cs_06.04.0_0005",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $am2",
		variables: [
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m1",
					"dim:part": "part:m8",
					"dim:typ": "typ:m16",
					"dim:bas": "bas:m73",
				},
				codes: ["1001-1002-1003-1003P"],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m1",
					"dim:part": "part:m8",
				},
				codes: ["1021-1022-1023-1023P"],
				fallback: "0",
			},
		],
		equation: "If 1001 > 1 then 1001 < 1021",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0006",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $am2",
		variables: [
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m2",
					"dim:part": "part:m8",
					"dim:typ": "typ:m16",
					"dim:bas": "bas:m73",
				},
				codes: ["1001-1002-1003-1003P"],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m2",
					"dim:part": "part:m8",
				},
				codes: ["1021-1022-1023-1023P"],
				fallback: "0",
			},
		],
		equation: "If 1002 > 1 then 1002 < 1022",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0007",
		kind: "social-balance",
		section: "06.04.0",
		test: "$am2 eq $am22 + $am23",
		variables: [
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m3",
					"dim:part": "part:m8",
				},
				codes: ["1021-1022-1023-1023P"],
				fallback: "0",
			},
			{
				name: "am22",
				metric: "am2",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m1",
					"dim:part": "part:m8",
				},
				codes: ["1021-1022-1023-1023P"],
				fallback: "0",
			},
			{
				name: "am23",
				metric: "am2",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m2",
					"dim:part": "part:m8",
				},
				codes: ["1021-1022-1023-1023P"],
				fallback: "0",
			},
		],
		equation: "1023 = 1021 + 1022",
		bindings: [
			{
				am2: "1021-1022-1023-1023P",
				am22: "1021-1022-1023-1023P",
				am23: "1021-1022-1023-1023P",
			},
		],
	},
	{
		id: "cs_06.04.0_0008",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 gt 0 and $int4 gt 0 and $am2 gt 0 and $dec1 lt $int4",
		variables: [
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m3",
					"dim:part": "part:m8",
					"dim:typ": "typ:m16",
					"dim:bas": "bas:m73",
				},
				codes: ["1001-1002-1003-1003P"],
				fallback: "0",
			},
			{
				name: "int4",
				metric: "int4",
				dimensions: {
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m3",
					"dim:part": "part:m8",
					"dim:hrs": "hrs:m1",
					"dim:bas": "bas:m73",
					"dim:wkr": "wkr:m1",
				},
				codes: ["1011-1012-1013-1013P"],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m3",
					"dim:part": "part:m8",
				},
				codes: ["1021-1022-1023-1023P"],
				fallback: "0",
			},
		],
		equation:
			"If 1003 > 0 or 1013 > 0 or 1023 > 0 then 1003 > 0 and 1013 > 0 and 1023 > 0 and 1003 < 1013",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0009",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $am2",
		variables: [
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m3",
					"dim:part": "part:m8",
					"dim:typ": "typ:m16",
					"dim:bas": "bas:m73",
				},
				codes: ["1001-1002-1003-1003P"],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m3",
					"dim:part": "part:m8",
				},
				codes: ["1021-1022-1023-1023P"],
				fallback: "0",
			},
		],
		equation: "If 1003 > 1 then 1003 < 1023",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0010",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int4 eq $int42 + $int43",
		variables: [
			{
				name: "int4",
				metric: "int4",
				dimensions: {
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m3",
					"dim:part": "part:m8",
					"dim:hrs": "hrs:m1",
					"dim:bas": "bas:m73",
					"dim:wkr": "wkr:m1",
				},
				codes: ["1011-1012-1013-1013P"],
				fallback: "0",
			},
			{
				name: "int42",
				metric: "int4",
				dimensions: {
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m1",
					"dim:part": "part:m8",
					"dim:hrs": "hrs:m1",
					"dim:bas": "bas:m73",
					"dim:wkr": "wkr:m1",
				},
				codes: ["1011-1012-1013-1013P"],
				fallback: "0",
			},
			{
				name: "int43",
				metric: "int4",
				dimensions: {
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m2",
					"dim:part": "part:m8",
					"dim:hrs": "hrs:m1",
					"dim:bas": "bas:m73",
					"dim:wkr": "wkr:m1",
				},
				codes: ["1011-1012-1013-1013P"],
				fallback: "0",
			},
		],
		equation: "1013 = 1011 + 1012",
		bindings: [
			{
				int4: "1011-1012-1013-1013P",
				int42: "1011-1012-1013-1013P",
				int43: "1011-1012-1013-1013P",
			},
		],
	},
	{
		id: "cs_06.04.0_0011",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 gt 0 and $int4 gt 0 and $am2 gt 0 and $dec1 lt $int4",
		variables: [
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m3",
					"dim:part": "part:m8",
					"dim:typ": "typ:m16",
					"dim:bas": "bas:m73",
				},
				codes: ["1001-1002-1003-1003P"],
				fallback: "0",
			},
			{
				name: "int4",
				metric: "int4",
				dimensions: {
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m3",
					"dim:part": "part:m8",
					"dim:hrs": "hrs:m1",
					"dim:bas": "bas:m73",
					"dim:wkr": "wkr:m1",
				},
				codes: ["1011-1012-1013-1013P"],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m3",
					"dim:part": "part:m8",
				},
				codes: ["1021-1022-1023-1023P"],
				fallback: "0",
			},
		],
		equation:
			"If 1003P > 0 or 1013P > 0 or 1023P > 0 then 1003P > 0 and 1013P > 0 and 1023P > 0 and 1003P < 1013P",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0012",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $am2",
		variables: [
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m3",
					"dim:part": "part:m8",
					"dim:typ": "typ:m16",
					"dim:bas": "bas:m73",
				},
				codes: ["1001-1002-1003-1003P"],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {
					"dim:wkr": "wkr:m1",
					"dim:gdr": "gdr:m1",
					"dim:wrg": "wrg:m3",
					"dim:part": "part:m8",
				},
				codes: ["1021-1022-1023-1023P"],
				fallback: "0",
			},
		],
		equation: "If 1003P > 1 then 1003P < 1023P",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0013",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {},
				codes: ["5801-5811", "5821-5831", "5841-5851"],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
				},
				codes: ["5801-5811", "5821-5831", "5841-5851"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {},
				codes: ["5801-5811", "5821-5831", "5841-5851"],
				fallback: "0",
			},
		],
		equation: "If 1052 is not completed then 1053 = 1051",
		bindings: [
			{
				int3: "5801-5811",
				dec1: "5801-5811",
				int32: "5801-5811",
			},
			{
				int3: "5821-5831",
				dec1: "5821-5831",
				int32: "5821-5831",
			},
			{
				int3: "5841-5851",
				dec1: "5841-5851",
				int32: "5841-5851",
			},
		],
	},
	{
		id: "cs_06.04.0_0014",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {},
				codes: ["5801-5811", "5821-5831", "5841-5851"],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
				},
				codes: ["5801-5811", "5821-5831", "5841-5851"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {},
				codes: ["5801-5811", "5821-5831", "5841-5851"],
				fallback: "0",
			},
		],
		equation: "If 1052 > 0 then 1053 < 1051 + 1052 and 1053 > 1051",
		bindings: [
			{
				int3: "5801-5811",
				dec1: "5801-5811",
				int32: "5801-5811",
			},
			{
				int3: "5821-5831",
				dec1: "5821-5831",
				int32: "5821-5831",
			},
			{
				int3: "5841-5851",
				dec1: "5841-5851",
				int32: "5841-5851",
			},
		],
	},
	{
		id: "cs_06.04.0_0015",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m1",
				},
				codes: ["1101-1102-1103"],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:epc": "epc:m1",
				},
				codes: ["1101-1102-1103"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m1",
				},
				codes: ["1101-1102-1103"],
				fallback: "0",
			},
		],
		equation: "If 1102 is not completed then 1103 = 1101",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0016",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m1",
				},
				codes: ["1101-1102-1103"],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:epc": "epc:m1",
				},
				codes: ["1101-1102-1103"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m1",
				},
				codes: ["1101-1102-1103"],
				fallback: "0",
			},
		],
		equation: "If 1102 > 0 then 1103 < 1101 + 1102 and 1103 > 1101",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0017",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m2",
				},
				codes: ["1111-1112-1113"],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:epc": "epc:m2",
				},
				codes: ["1111-1112-1113"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m2",
				},
				codes: ["1111-1112-1113"],
				fallback: "0",
			},
		],
		equation: "If 1112 is not completed then 1113 = 1111",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0018",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m2",
				},
				codes: ["1111-1112-1113"],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:epc": "epc:m2",
				},
				codes: ["1111-1112-1113"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m2",
				},
				codes: ["1111-1112-1113"],
				fallback: "0",
			},
		],
		equation: "If 1112 > 0 then 1113 < 1111 + 1112 and 1113 > 1111",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0019",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m3",
				},
				codes: ["1121-1122-1123"],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:epc": "epc:m3",
				},
				codes: ["1121-1122-1123"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m3",
				},
				codes: ["1121-1122-1123"],
				fallback: "0",
			},
		],
		equation: "If 1122 is not completed then 1123 = 1121",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0020",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m3",
				},
				codes: ["1121-1122-1123"],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:epc": "epc:m3",
				},
				codes: ["1121-1122-1123"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m3",
				},
				codes: ["1121-1122-1123"],
				fallback: "0",
			},
		],
		equation: "If 1122 > 0 then 1123 < 1121 + 1122 and 1123 > 1121",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0021",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m4",
				},
				codes: ["1131-1132-1133"],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:epc": "epc:m4",
				},
				codes: ["1131-1132-1133"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m4",
				},
				codes: ["1131-1132-1133"],
				fallback: "0",
			},
		],
		equation: "If 1132 is not completed then 1133 = 1131",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0022",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m4",
				},
				codes: ["1131-1132-1133"],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:epc": "epc:m4",
				},
				codes: ["1131-1132-1133"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m4",
				},
				codes: ["1131-1132-1133"],
				fallback: "0",
			},
		],
		equation: "If 1132 > 0 then 1133 < 1131 + 1132 and 1133 > 1131",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0023",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:gdr": "gdr:m2",
				},
				codes: [
					"12001-12002-12003",
					"1201-1202-1203",
					"12011-12012-12013",
					"12021-12022-12023",
					"12031-12032-12033",
					"5801-5811",
					"5821-5831",
					"5841-5851",
				],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: [
					"12001-12002-12003",
					"1201-1202-1203",
					"12011-12012-12013",
					"12021-12022-12023",
					"12031-12032-12033",
				],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:gdr": "gdr:m2",
				},
				codes: [
					"12001-12002-12003",
					"1201-1202-1203",
					"12011-12012-12013",
					"12021-12022-12023",
					"12031-12032-12033",
					"5801-5811",
					"5821-5831",
					"5841-5851",
				],
				fallback: "0",
			},
		],
		equation: "If 1202 is not completed then 1203 = 1201",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0024",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m1",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12001-12002-12003"],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m1",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12001-12002-12003"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m1",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12001-12002-12003"],
				fallback: "0",
			},
		],
		equation: "If 12002 is not completed then 12003 = 12001",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0025",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m2",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12011-12012-12013"],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m2",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12011-12012-12013"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m2",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12011-12012-12013"],
				fallback: "0",
			},
		],
		equation: "If 12012 is not completed then 12013 = 12011",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0026",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12021-12022-12023"],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m3",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12021-12022-12023"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12021-12022-12023"],
				fallback: "0",
			},
		],
		equation: "If 12022 is not completed then 12023 = 12021",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0027",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m4",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12031-12032-12033"],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m4",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12031-12032-12033"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m4",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12031-12032-12033"],
				fallback: "0",
			},
		],
		equation: "If 12032 is not completed then 12033 = 12031",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0028",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:gdr": "gdr:m2",
				},
				codes: [
					"12001-12002-12003",
					"1201-1202-1203",
					"12011-12012-12013",
					"12021-12022-12023",
					"12031-12032-12033",
					"5801-5811",
					"5821-5831",
					"5841-5851",
				],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: [
					"12001-12002-12003",
					"1201-1202-1203",
					"12011-12012-12013",
					"12021-12022-12023",
					"12031-12032-12033",
				],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:gdr": "gdr:m2",
				},
				codes: [
					"12001-12002-12003",
					"1201-1202-1203",
					"12011-12012-12013",
					"12021-12022-12023",
					"12031-12032-12033",
					"5801-5811",
					"5821-5831",
					"5841-5851",
				],
				fallback: "0",
			},
		],
		equation: "If 1202 > 0 then 1203 < 1201 + 1202 and 1203 > 1201",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0029",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m1",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12001-12002-12003"],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m1",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12001-12002-12003"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m1",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12001-12002-12003"],
				fallback: "0",
			},
		],
		equation: "If 12002 > 0 then 12003 < 12001 + 12002 and 12003 > 12001",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0030",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m2",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12011-12012-12013"],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m2",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12011-12012-12013"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m2",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12011-12012-12013"],
				fallback: "0",
			},
		],
		equation: "If 12012 > 0 then 12013 < 12011 + 12012 and 12013 > 12011",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0031",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12021-12022-12023"],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m3",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12021-12022-12023"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12021-12022-12023"],
				fallback: "0",
			},
		],
		equation: "If 12022 > 0 then 12023 < 12021 + 12022 and 12023 > 12021",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0032",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m4",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12031-12032-12033"],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m4",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12031-12032-12033"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m4",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12031-12032-12033"],
				fallback: "0",
			},
		],
		equation: "If 12032 > 0 then 12033 < 12031 + 12032 and 12033 > 12031",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0033",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:gdr": "gdr:m3",
				},
				codes: [
					"12101-12102-12103",
					"1211-1212-1213",
					"12111-12112-12113",
					"12121-12122-12123",
					"12131-12132-12133",
					"5801-5811",
					"5821-5831",
					"5841-5851",
				],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: [
					"12101-12102-12103",
					"1211-1212-1213",
					"12111-12112-12113",
					"12121-12122-12123",
					"12131-12132-12133",
				],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:gdr": "gdr:m3",
				},
				codes: [
					"12101-12102-12103",
					"1211-1212-1213",
					"12111-12112-12113",
					"12121-12122-12123",
					"12131-12132-12133",
					"5801-5811",
					"5821-5831",
					"5841-5851",
				],
				fallback: "0",
			},
		],
		equation: "If 1212 is not completed then 1213 = 1211",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0034",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m1",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12101-12102-12103"],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m1",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12101-12102-12103"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m1",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12101-12102-12103"],
				fallback: "0",
			},
		],
		equation: "If 12102 is not completed then 12103 = 12101",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0035",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m2",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12111-12112-12113"],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m2",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12111-12112-12113"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m2",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12111-12112-12113"],
				fallback: "0",
			},
		],
		equation: "If 12112 is not completed then 12113 = 12111",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0036",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12121-12122-12123"],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m3",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12121-12122-12123"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12121-12122-12123"],
				fallback: "0",
			},
		],
		equation: "If 12122 is not completed then 12123 = 12121",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0037",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m4",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12131-12132-12133"],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m4",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12131-12132-12133"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m4",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12131-12132-12133"],
				fallback: "0",
			},
		],
		equation: "If 12132 is not completed then 12133 = 12131",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0038",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:gdr": "gdr:m3",
				},
				codes: [
					"12101-12102-12103",
					"1211-1212-1213",
					"12111-12112-12113",
					"12121-12122-12123",
					"12131-12132-12133",
					"5801-5811",
					"5821-5831",
					"5841-5851",
				],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: [
					"12101-12102-12103",
					"1211-1212-1213",
					"12111-12112-12113",
					"12121-12122-12123",
					"12131-12132-12133",
				],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:gdr": "gdr:m3",
				},
				codes: [
					"12101-12102-12103",
					"1211-1212-1213",
					"12111-12112-12113",
					"12121-12122-12123",
					"12131-12132-12133",
					"5801-5811",
					"5821-5831",
					"5841-5851",
				],
				fallback: "0",
			},
		],
		equation: "If 1212 > 0 then 1213 < 1211 + 1212 and 1213 > 1211",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0039",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m1",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12101-12102-12103"],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m1",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12101-12102-12103"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m1",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12101-12102-12103"],
				fallback: "0",
			},
		],
		equation: "If 12102 > 0 then 12103 < 12101 + 12102 and 12103 > 12101",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0040",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m2",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12111-12112-12113"],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m2",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12111-12112-12113"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m2",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12111-12112-12113"],
				fallback: "0",
			},
		],
		equation: "If 12112 > 0 then 12113 < 12111 + 12112 and 12113 > 12111",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0041",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12121-12122-12123"],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m3",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12121-12122-12123"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12121-12122-12123"],
				fallback: "0",
			},
		],
		equation: "If 12122 > 0 then 12123 < 12121 + 12122 and 12123 > 12121",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0042",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m4",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12131-12132-12133"],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m4",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12131-12132-12133"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m4",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12131-12132-12133"],
				fallback: "0",
			},
		],
		equation: "If 12132 > 0 then 12133 < 12131 + 12132 and 12133 > 12131",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0043",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m1",
				},
				codes: ["1301-1302-1303"],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:pfc": "pfc:m1",
				},
				codes: ["1301-1302-1303"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m1",
				},
				codes: ["1301-1302-1303"],
				fallback: "0",
			},
		],
		equation: "If 1302 is not completed then 1303 = 1301",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0044",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m1",
				},
				codes: ["1301-1302-1303"],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:pfc": "pfc:m1",
				},
				codes: ["1301-1302-1303"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m1",
				},
				codes: ["1301-1302-1303"],
				fallback: "0",
			},
		],
		equation: "If 1302 > 0 then 1303 < 1301 + 1302 and 1303 > 1301",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0045",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m2",
				},
				codes: ["1341-1342-1343"],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:pfc": "pfc:m2",
				},
				codes: ["1341-1342-1343"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m2",
				},
				codes: ["1341-1342-1343"],
				fallback: "0",
			},
		],
		equation: "If 1342 is not completed then 1343 = 1341",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0046",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m2",
				},
				codes: ["1341-1342-1343"],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:pfc": "pfc:m2",
				},
				codes: ["1341-1342-1343"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m2",
				},
				codes: ["1341-1342-1343"],
				fallback: "0",
			},
		],
		equation: "If 1342 > 0 then 1343 < 1341 + 1342 and 1343 > 1341",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0047",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m3",
				},
				codes: ["1321-1322-1323"],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:pfc": "pfc:m3",
				},
				codes: ["1321-1322-1323"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m3",
				},
				codes: ["1321-1322-1323"],
				fallback: "0",
			},
		],
		equation: "If 1322 is not completed then 1323 = 1321",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0048",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m3",
				},
				codes: ["1321-1322-1323"],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:pfc": "pfc:m3",
				},
				codes: ["1321-1322-1323"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m3",
				},
				codes: ["1321-1322-1323"],
				fallback: "0",
			},
		],
		equation: "If 1322 > 0 then 1323 < 1321 + 1322 and 1323 > 1321",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0049",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m4",
				},
				codes: ["1331-1332-1333"],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:pfc": "pfc:m4",
				},
				codes: ["1331-1332-1333"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m4",
				},
				codes: ["1331-1332-1333"],
				fallback: "0",
			},
		],
		equation: "If 1332 is not completed then 1333 = 1331",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0050",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m4",
				},
				codes: ["1331-1332-1333"],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:pfc": "pfc:m4",
				},
				codes: ["1331-1332-1333"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m4",
				},
				codes: ["1331-1332-1333"],
				fallback: "0",
			},
		],
		equation: "If 1332 > 0 then 1333 < 1331 + 1332 and 1333 > 1331",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0051",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 eq $int32 + $int33 + $int34 + $int35",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {},
				codes: [
					"1051-1052-1053",
					"1101-1102-1103",
					"1111-1112-1113",
					"1121-1122-1123",
					"1131-1132-1133",
				],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m1",
				},
				codes: ["1101-1102-1103"],
				fallback: "0",
			},
			{
				name: "int33",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m2",
				},
				codes: ["1111-1112-1113"],
				fallback: "0",
			},
			{
				name: "int34",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m3",
				},
				codes: ["1121-1122-1123"],
				fallback: "0",
			},
			{
				name: "int35",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m4",
				},
				codes: ["1131-1132-1133"],
				fallback: "0",
			},
		],
		equation: "1051 = 1101 + 1111 + 1121 + 1131",
		bindings: [
			{
				int3: "1051-1052-1053",
				int32: "1101-1102-1103",
				int33: "1111-1112-1113",
				int34: "1121-1122-1123",
				int35: "1131-1132-1133",
			},
			{
				int3: "1101-1102-1103",
				int32: "1101-1102-1103",
				int33: "1111-1112-1113",
				int34: "1121-1122-1123",
				int35: "1131-1132-1133",
			},
			{
				int3: "1111-1112-1113",
				int32: "1101-1102-1103",
				int33: "1111-1112-1113",
				int34: "1121-1122-1123",
				int35: "1131-1132-1133",
			},
			{
				int3: "1121-1122-1123",
				int32: "1101-1102-1103",
				int33: "1111-1112-1113",
				int34: "1121-1122-1123",
				int35: "1131-1132-1133",
			},
			{
				int3: "1131-1132-1133",
				int32: "1101-1102-1103",
				int33: "1111-1112-1113",
				int34: "1121-1122-1123",
				int35: "1131-1132-1133",
			},
		],
	},
	{
		id: "cs_06.04.0_0052",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 eq $int32 + $int33 + $int34 + $int35",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {},
				codes: [
					"1051-1052-1053",
					"1101-1102-1103",
					"1111-1112-1113",
					"1121-1122-1123",
					"1131-1132-1133",
				],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m1",
				},
				codes: ["1101-1102-1103"],
				fallback: "0",
			},
			{
				name: "int33",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m2",
				},
				codes: ["1111-1112-1113"],
				fallback: "0",
			},
			{
				name: "int34",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m3",
				},
				codes: ["1121-1122-1123"],
				fallback: "0",
			},
			{
				name: "int35",
				metric: "int3",
				dimensions: {
					"dim:epc": "epc:m4",
				},
				codes: ["1131-1132-1133"],
				fallback: "0",
			},
		],
		equation: "1052 = 1102 + 1112 + 1122 + 1132",
		bindings: [
			{
				int3: "1051-1052-1053",
				int32: "1101-1102-1103",
				int33: "1111-1112-1113",
				int34: "1121-1122-1123",
				int35: "1131-1132-1133",
			},
			{
				int3: "1101-1102-1103",
				int32: "1101-1102-1103",
				int33: "1111-1112-1113",
				int34: "1121-1122-1123",
				int35: "1131-1132-1133",
			},
			{
				int3: "1111-1112-1113",
				int32: "1101-1102-1103",
				int33: "1111-1112-1113",
				int34: "1121-1122-1123",
				int35: "1131-1132-1133",
			},
			{
				int3: "1121-1122-1123",
				int32: "1101-1102-1103",
				int33: "1111-1112-1113",
				int34: "1121-1122-1123",
				int35: "1131-1132-1133",
			},
			{
				int3: "1131-1132-1133",
				int32: "1101-1102-1103",
				int33: "1111-1112-1113",
				int34: "1121-1122-1123",
				int35: "1131-1132-1133",
			},
		],
	},
	{
		id: "cs_06.04.0_0053",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $dec12 + $dec13 + $dec14 + $dec15",
		variables: [
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
				},
				codes: [
					"1051-1052-1053",
					"1101-1102-1103",
					"1111-1112-1113",
					"1121-1122-1123",
					"1131-1132-1133",
				],
				fallback: "0",
			},
			{
				name: "dec12",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:epc": "epc:m1",
				},
				codes: ["1101-1102-1103"],
				fallback: "0",
			},
			{
				name: "dec13",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:epc": "epc:m2",
				},
				codes: ["1111-1112-1113"],
				fallback: "0",
			},
			{
				name: "dec14",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:epc": "epc:m3",
				},
				codes: ["1121-1122-1123"],
				fallback: "0",
			},
			{
				name: "dec15",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:epc": "epc:m4",
				},
				codes: ["1131-1132-1133"],
				fallback: "0",
			},
		],
		equation: "1053 = 1103 + 1113 + 1123 + 1133",
		bindings: [
			{
				dec1: "1051-1052-1053",
				dec12: "1101-1102-1103",
				dec13: "1111-1112-1113",
				dec14: "1121-1122-1123",
				dec15: "1131-1132-1133",
			},
			{
				dec1: "1101-1102-1103",
				dec12: "1101-1102-1103",
				dec13: "1111-1112-1113",
				dec14: "1121-1122-1123",
				dec15: "1131-1132-1133",
			},
			{
				dec1: "1111-1112-1113",
				dec12: "1101-1102-1103",
				dec13: "1111-1112-1113",
				dec14: "1121-1122-1123",
				dec15: "1131-1132-1133",
			},
			{
				dec1: "1121-1122-1123",
				dec12: "1101-1102-1103",
				dec13: "1111-1112-1113",
				dec14: "1121-1122-1123",
				dec15: "1131-1132-1133",
			},
			{
				dec1: "1131-1132-1133",
				dec12: "1101-1102-1103",
				dec13: "1111-1112-1113",
				dec14: "1121-1122-1123",
				dec15: "1131-1132-1133",
			},
		],
	},
	{
		id: "cs_06.04.0_0054",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 eq $int32 + $int33",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {},
				codes: [
					"1051-1052-1053",
					"12001-12002-12003",
					"1201-1202-1203",
					"12011-12012-12013",
					"12021-12022-12023",
					"12031-12032-12033",
					"12101-12102-12103",
					"1211-1212-1213",
					"12111-12112-12113",
					"12121-12122-12123",
					"12131-12132-12133",
					"5801-5811",
					"5821-5831",
					"5841-5851",
				],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:gdr": "gdr:m2",
				},
				codes: [
					"12001-12002-12003",
					"1201-1202-1203",
					"12011-12012-12013",
					"12021-12022-12023",
					"12031-12032-12033",
					"5801-5811",
					"5821-5831",
					"5841-5851",
				],
				fallback: "0",
			},
			{
				name: "int33",
				metric: "int3",
				dimensions: {
					"dim:gdr": "gdr:m3",
				},
				codes: [
					"12101-12102-12103",
					"1211-1212-1213",
					"12111-12112-12113",
					"12121-12122-12123",
					"12131-12132-12133",
					"5801-5811",
					"5821-5831",
					"5841-5851",
				],
				fallback: "0",
			},
		],
		equation: "1051 = 1201 + 1211",
		bindings: [
			{
				int3: "12001-12002-12003",
				int32: "12001-12002-12003",
				int33: "12101-12102-12103",
			},
			{
				int3: "12101-12102-12103",
				int32: "12001-12002-12003",
				int33: "12101-12102-12103",
			},
			{
				int3: "12011-12012-12013",
				int32: "12011-12012-12013",
				int33: "12111-12112-12113",
			},
			{
				int3: "12111-12112-12113",
				int32: "12011-12012-12013",
				int33: "12111-12112-12113",
			},
			{
				int3: "12021-12022-12023",
				int32: "12021-12022-12023",
				int33: "12121-12122-12123",
			},
			{
				int3: "12121-12122-12123",
				int32: "12021-12022-12023",
				int33: "12121-12122-12123",
			},
			{
				int3: "12031-12032-12033",
				int32: "12031-12032-12033",
				int33: "12131-12132-12133",
			},
			{
				int3: "12131-12132-12133",
				int32: "12031-12032-12033",
				int33: "12131-12132-12133",
			},
			{
				int3: "1051-1052-1053",
				int32: "1201-1202-1203",
				int33: "1211-1212-1213",
			},
			{
				int3: "1201-1202-1203",
				int32: "1201-1202-1203",
				int33: "1211-1212-1213",
			},
			{
				int3: "1211-1212-1213",
				int32: "1201-1202-1203",
				int33: "1211-1212-1213",
			},
			{
				int3: "5801-5811",
				int32: "5801-5811",
				int33: "5801-5811",
			},
			{
				int3: "5821-5831",
				int32: "5821-5831",
				int33: "5821-5831",
			},
			{
				int3: "5841-5851",
				int32: "5841-5851",
				int33: "5841-5851",
			},
		],
	},
	{
		id: "cs_06.04.0_0055",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 eq $int32 + $int33 + $int34 + $int35",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:gdr": "gdr:m2",
				},
				codes: [
					"12001-12002-12003",
					"1201-1202-1203",
					"12011-12012-12013",
					"12021-12022-12023",
					"12031-12032-12033",
				],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m1",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12001-12002-12003"],
				fallback: "0",
			},
			{
				name: "int33",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m2",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12011-12012-12013"],
				fallback: "0",
			},
			{
				name: "int34",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12021-12022-12023"],
				fallback: "0",
			},
			{
				name: "int35",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m4",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12031-12032-12033"],
				fallback: "0",
			},
		],
		equation: "1201 = 12001 + 12011 + 12021 + 12031",
		bindings: [
			{
				int3: "12001-12002-12003",
				int32: "12001-12002-12003",
				int33: "12011-12012-12013",
				int34: "12021-12022-12023",
				int35: "12031-12032-12033",
			},
			{
				int3: "12011-12012-12013",
				int32: "12001-12002-12003",
				int33: "12011-12012-12013",
				int34: "12021-12022-12023",
				int35: "12031-12032-12033",
			},
			{
				int3: "12021-12022-12023",
				int32: "12001-12002-12003",
				int33: "12011-12012-12013",
				int34: "12021-12022-12023",
				int35: "12031-12032-12033",
			},
			{
				int3: "12031-12032-12033",
				int32: "12001-12002-12003",
				int33: "12011-12012-12013",
				int34: "12021-12022-12023",
				int35: "12031-12032-12033",
			},
			{
				int3: "1201-1202-1203",
				int32: "12001-12002-12003",
				int33: "12011-12012-12013",
				int34: "12021-12022-12023",
				int35: "12031-12032-12033",
			},
		],
	},
	{
		id: "cs_06.04.0_0056",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 eq $int32 + $int33 + $int34 + $int35",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:gdr": "gdr:m3",
				},
				codes: [
					"12101-12102-12103",
					"1211-1212-1213",
					"12111-12112-12113",
					"12121-12122-12123",
					"12131-12132-12133",
				],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m1",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12101-12102-12103"],
				fallback: "0",
			},
			{
				name: "int33",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m2",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12111-12112-12113"],
				fallback: "0",
			},
			{
				name: "int34",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12121-12122-12123"],
				fallback: "0",
			},
			{
				name: "int35",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m4",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12131-12132-12133"],
				fallback: "0",
			},
		],
		equation: "1211 = 12101 + 12111 + 12121 + 12131",
		bindings: [
			{
				int3: "12101-12102-12103",
				int32: "12101-12102-12103",
				int33: "12111-12112-12113",
				int34: "12121-12122-12123",
				int35: "12131-12132-12133",
			},
			{
				int3: "12111-12112-12113",
				int32: "12101-12102-12103",
				int33: "12111-12112-12113",
				int34: "12121-12122-12123",
				int35: "12131-12132-12133",
			},
			{
				int3: "12121-12122-12123",
				int32: "12101-12102-12103",
				int33: "12111-12112-12113",
				int34: "12121-12122-12123",
				int35: "12131-12132-12133",
			},
			{
				int3: "12131-12132-12133",
				int32: "12101-12102-12103",
				int33: "12111-12112-12113",
				int34: "12121-12122-12123",
				int35: "12131-12132-12133",
			},
			{
				int3: "1211-1212-1213",
				int32: "12101-12102-12103",
				int33: "12111-12112-12113",
				int34: "12121-12122-12123",
				int35: "12131-12132-12133",
			},
		],
	},
	{
		id: "cs_06.04.0_0057",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 eq $int32 + $int33",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {},
				codes: [
					"1051-1052-1053",
					"12001-12002-12003",
					"1201-1202-1203",
					"12011-12012-12013",
					"12021-12022-12023",
					"12031-12032-12033",
					"12101-12102-12103",
					"1211-1212-1213",
					"12111-12112-12113",
					"12121-12122-12123",
					"12131-12132-12133",
					"5801-5811",
					"5821-5831",
					"5841-5851",
				],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:gdr": "gdr:m2",
				},
				codes: [
					"12001-12002-12003",
					"1201-1202-1203",
					"12011-12012-12013",
					"12021-12022-12023",
					"12031-12032-12033",
					"5801-5811",
					"5821-5831",
					"5841-5851",
				],
				fallback: "0",
			},
			{
				name: "int33",
				metric: "int3",
				dimensions: {
					"dim:gdr": "gdr:m3",
				},
				codes: [
					"12101-12102-12103",
					"1211-1212-1213",
					"12111-12112-12113",
					"12121-12122-12123",
					"12131-12132-12133",
					"5801-5811",
					"5821-5831",
					"5841-5851",
				],
				fallback: "0",
			},
		],
		equation: "1052 = 1202 + 1212",
		bindings: [
			{
				int3: "12001-12002-12003",
				int32: "12001-12002-12003",
				int33: "12101-12102-12103",
			},
			{
				int3: "12101-12102-12103",
				int32: "12001-12002-12003",
				int33: "12101-12102-12103",
			},
			{
				int3: "12011-12012-12013",
				int32: "12011-12012-12013",
				int33: "12111-12112-12113",
			},
			{
				int3: "12111-12112-12113",
				int32: "12011-12012-12013",
				int33: "12111-12112-12113",
			},
			{
				int3: "12021-12022-12023",
				int32: "12021-12022-12023",
				int33: "12121-12122-12123",
			},
			{
				int3: "12121-12122-12123",
				int32: "12021-12022-12023",
				int33: "12121-12122-12123",
			},
			{
				int3: "12031-12032-12033",
				int32: "12031-12032-12033",
				int33: "12131-12132-12133",
			},
			{
				int3: "12131-12132-12133",
				int32: "12031-12032-12033",
				int33: "12131-12132-12133",
			},
			{
				int3: "1051-1052-1053",
				int32: "1201-1202-1203",
				int33: "1211-1212-1213",
			},
			{
				int3: "1201-1202-1203",
				int32: "1201-1202-1203",
				int33: "1211-1212-1213",
			},
			{
				int3: "1211-1212-1213",
				int32: "1201-1202-1203",
				int33: "1211-1212-1213",
			},
			{
				int3: "5801-5811",
				int32: "5801-5811",
				int33: "5801-5811",
			},
			{
				int3: "5821-5831",
				int32: "5821-5831",
				int33: "5821-5831",
			},
			{
				int3: "5841-5851",
				int32: "5841-5851",
				int33: "5841-5851",
			},
		],
	},
	{
		id: "cs_06.04.0_0058",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 eq $int32 + $int33 + $int34 + $int35",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:gdr": "gdr:m2",
				},
				codes: [
					"12001-12002-12003",
					"1201-1202-1203",
					"12011-12012-12013",
					"12021-12022-12023",
					"12031-12032-12033",
				],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m1",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12001-12002-12003"],
				fallback: "0",
			},
			{
				name: "int33",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m2",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12011-12012-12013"],
				fallback: "0",
			},
			{
				name: "int34",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12021-12022-12023"],
				fallback: "0",
			},
			{
				name: "int35",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m4",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12031-12032-12033"],
				fallback: "0",
			},
		],
		equation: "1202 = 12002 + 12012 + 12022 + 12032",
		bindings: [
			{
				int3: "12001-12002-12003",
				int32: "12001-12002-12003",
				int33: "12011-12012-12013",
				int34: "12021-12022-12023",
				int35: "12031-12032-12033",
			},
			{
				int3: "12011-12012-12013",
				int32: "12001-12002-12003",
				int33: "12011-12012-12013",
				int34: "12021-12022-12023",
				int35: "12031-12032-12033",
			},
			{
				int3: "12021-12022-12023",
				int32: "12001-12002-12003",
				int33: "12011-12012-12013",
				int34: "12021-12022-12023",
				int35: "12031-12032-12033",
			},
			{
				int3: "12031-12032-12033",
				int32: "12001-12002-12003",
				int33: "12011-12012-12013",
				int34: "12021-12022-12023",
				int35: "12031-12032-12033",
			},
			{
				int3: "1201-1202-1203",
				int32: "12001-12002-12003",
				int33: "12011-12012-12013",
				int34: "12021-12022-12023",
				int35: "12031-12032-12033",
			},
		],
	},
	{
		id: "cs_06.04.0_0059",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 eq $int32 + $int33 + $int34 + $int35",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:gdr": "gdr:m3",
				},
				codes: [
					"12101-12102-12103",
					"1211-1212-1213",
					"12111-12112-12113",
					"12121-12122-12123",
					"12131-12132-12133",
				],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m1",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12101-12102-12103"],
				fallback: "0",
			},
			{
				name: "int33",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m2",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12111-12112-12113"],
				fallback: "0",
			},
			{
				name: "int34",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12121-12122-12123"],
				fallback: "0",
			},
			{
				name: "int35",
				metric: "int3",
				dimensions: {
					"dim:dgl": "dgl:m4",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12131-12132-12133"],
				fallback: "0",
			},
		],
		equation: "1212 = 12102 + 12112 + 12122 + 12132",
		bindings: [
			{
				int3: "12101-12102-12103",
				int32: "12101-12102-12103",
				int33: "12111-12112-12113",
				int34: "12121-12122-12123",
				int35: "12131-12132-12133",
			},
			{
				int3: "12111-12112-12113",
				int32: "12101-12102-12103",
				int33: "12111-12112-12113",
				int34: "12121-12122-12123",
				int35: "12131-12132-12133",
			},
			{
				int3: "12121-12122-12123",
				int32: "12101-12102-12103",
				int33: "12111-12112-12113",
				int34: "12121-12122-12123",
				int35: "12131-12132-12133",
			},
			{
				int3: "12131-12132-12133",
				int32: "12101-12102-12103",
				int33: "12111-12112-12113",
				int34: "12121-12122-12123",
				int35: "12131-12132-12133",
			},
			{
				int3: "1211-1212-1213",
				int32: "12101-12102-12103",
				int33: "12111-12112-12113",
				int34: "12121-12122-12123",
				int35: "12131-12132-12133",
			},
		],
	},
	{
		id: "cs_06.04.0_0060",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $dec12 + $dec13",
		variables: [
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
				},
				codes: [
					"1051-1052-1053",
					"12001-12002-12003",
					"1201-1202-1203",
					"12011-12012-12013",
					"12021-12022-12023",
					"12031-12032-12033",
					"12101-12102-12103",
					"1211-1212-1213",
					"12111-12112-12113",
					"12121-12122-12123",
					"12131-12132-12133",
				],
				fallback: "0",
			},
			{
				name: "dec12",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: [
					"12001-12002-12003",
					"1201-1202-1203",
					"12011-12012-12013",
					"12021-12022-12023",
					"12031-12032-12033",
				],
				fallback: "0",
			},
			{
				name: "dec13",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: [
					"12101-12102-12103",
					"1211-1212-1213",
					"12111-12112-12113",
					"12121-12122-12123",
					"12131-12132-12133",
				],
				fallback: "0",
			},
		],
		equation: "1053 = 1203 + 1213",
		bindings: [
			{
				dec1: "12001-12002-12003",
				dec12: "12001-12002-12003",
				dec13: "12101-12102-12103",
			},
			{
				dec1: "12101-12102-12103",
				dec12: "12001-12002-12003",
				dec13: "12101-12102-12103",
			},
			{
				dec1: "12011-12012-12013",
				dec12: "12011-12012-12013",
				dec13: "12111-12112-12113",
			},
			{
				dec1: "12111-12112-12113",
				dec12: "12011-12012-12013",
				dec13: "12111-12112-12113",
			},
			{
				dec1: "12021-12022-12023",
				dec12: "12021-12022-12023",
				dec13: "12121-12122-12123",
			},
			{
				dec1: "12121-12122-12123",
				dec12: "12021-12022-12023",
				dec13: "12121-12122-12123",
			},
			{
				dec1: "12031-12032-12033",
				dec12: "12031-12032-12033",
				dec13: "12131-12132-12133",
			},
			{
				dec1: "12131-12132-12133",
				dec12: "12031-12032-12033",
				dec13: "12131-12132-12133",
			},
			{
				dec1: "1051-1052-1053",
				dec12: "1201-1202-1203",
				dec13: "1211-1212-1213",
			},
			{
				dec1: "1201-1202-1203",
				dec12: "1201-1202-1203",
				dec13: "1211-1212-1213",
			},
			{
				dec1: "1211-1212-1213",
				dec12: "1201-1202-1203",
				dec13: "1211-1212-1213",
			},
		],
	},
	{
		id: "cs_06.04.0_0061",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $dec12 + $dec13 + $dec14 + $dec15",
		variables: [
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: [
					"12001-12002-12003",
					"1201-1202-1203",
					"12011-12012-12013",
					"12021-12022-12023",
					"12031-12032-12033",
				],
				fallback: "0",
			},
			{
				name: "dec12",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m1",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12001-12002-12003"],
				fallback: "0",
			},
			{
				name: "dec13",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m2",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12011-12012-12013"],
				fallback: "0",
			},
			{
				name: "dec14",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m3",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12021-12022-12023"],
				fallback: "0",
			},
			{
				name: "dec15",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m4",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m2",
				},
				codes: ["12031-12032-12033"],
				fallback: "0",
			},
		],
		equation: "1203 = 12003 + 12013 + 12023 + 12033",
		bindings: [
			{
				dec1: "12001-12002-12003",
				dec12: "12001-12002-12003",
				dec13: "12011-12012-12013",
				dec14: "12021-12022-12023",
				dec15: "12031-12032-12033",
			},
			{
				dec1: "12011-12012-12013",
				dec12: "12001-12002-12003",
				dec13: "12011-12012-12013",
				dec14: "12021-12022-12023",
				dec15: "12031-12032-12033",
			},
			{
				dec1: "12021-12022-12023",
				dec12: "12001-12002-12003",
				dec13: "12011-12012-12013",
				dec14: "12021-12022-12023",
				dec15: "12031-12032-12033",
			},
			{
				dec1: "12031-12032-12033",
				dec12: "12001-12002-12003",
				dec13: "12011-12012-12013",
				dec14: "12021-12022-12023",
				dec15: "12031-12032-12033",
			},
			{
				dec1: "1201-1202-1203",
				dec12: "12001-12002-12003",
				dec13: "12011-12012-12013",
				dec14: "12021-12022-12023",
				dec15: "12031-12032-12033",
			},
		],
	},
	{
		id: "cs_06.04.0_0062",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $dec12 + $dec13 + $dec14 + $dec15",
		variables: [
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: [
					"12101-12102-12103",
					"1211-1212-1213",
					"12111-12112-12113",
					"12121-12122-12123",
					"12131-12132-12133",
				],
				fallback: "0",
			},
			{
				name: "dec12",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m1",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12101-12102-12103"],
				fallback: "0",
			},
			{
				name: "dec13",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m2",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12111-12112-12113"],
				fallback: "0",
			},
			{
				name: "dec14",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m3",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12121-12122-12123"],
				fallback: "0",
			},
			{
				name: "dec15",
				metric: "dec1",
				dimensions: {
					"dim:dgl": "dgl:m4",
					"dim:qlt": "qlt:m3",
					"dim:gdr": "gdr:m3",
				},
				codes: ["12131-12132-12133"],
				fallback: "0",
			},
		],
		equation: "1213 = 12103 + 12113 + 12123 + 12133",
		bindings: [
			{
				dec1: "12101-12102-12103",
				dec12: "12101-12102-12103",
				dec13: "12111-12112-12113",
				dec14: "12121-12122-12123",
				dec15: "12131-12132-12133",
			},
			{
				dec1: "12111-12112-12113",
				dec12: "12101-12102-12103",
				dec13: "12111-12112-12113",
				dec14: "12121-12122-12123",
				dec15: "12131-12132-12133",
			},
			{
				dec1: "12121-12122-12123",
				dec12: "12101-12102-12103",
				dec13: "12111-12112-12113",
				dec14: "12121-12122-12123",
				dec15: "12131-12132-12133",
			},
			{
				dec1: "12131-12132-12133",
				dec12: "12101-12102-12103",
				dec13: "12111-12112-12113",
				dec14: "12121-12122-12123",
				dec15: "12131-12132-12133",
			},
			{
				dec1: "1211-1212-1213",
				dec12: "12101-12102-12103",
				dec13: "12111-12112-12113",
				dec14: "12121-12122-12123",
				dec15: "12131-12132-12133",
			},
		],
	},
	{
		id: "cs_06.04.0_0063",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 eq $int32 + $int33 + $int34 + $int35",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {},
				codes: [
					"1051-1052-1053",
					"1301-1302-1303",
					"1321-1322-1323",
					"1331-1332-1333",
					"1341-1342-1343",
				],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m1",
				},
				codes: ["1301-1302-1303"],
				fallback: "0",
			},
			{
				name: "int33",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m2",
				},
				codes: ["1341-1342-1343"],
				fallback: "0",
			},
			{
				name: "int34",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m3",
				},
				codes: ["1321-1322-1323"],
				fallback: "0",
			},
			{
				name: "int35",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m4",
				},
				codes: ["1331-1332-1333"],
				fallback: "0",
			},
		],
		equation: "1051 = 1301 + 1341 + 1321 + 1331",
		bindings: [
			{
				int3: "1051-1052-1053",
				int32: "1301-1302-1303",
				int33: "1341-1342-1343",
				int34: "1321-1322-1323",
				int35: "1331-1332-1333",
			},
			{
				int3: "1301-1302-1303",
				int32: "1301-1302-1303",
				int33: "1341-1342-1343",
				int34: "1321-1322-1323",
				int35: "1331-1332-1333",
			},
			{
				int3: "1341-1342-1343",
				int32: "1301-1302-1303",
				int33: "1341-1342-1343",
				int34: "1321-1322-1323",
				int35: "1331-1332-1333",
			},
			{
				int3: "1321-1322-1323",
				int32: "1301-1302-1303",
				int33: "1341-1342-1343",
				int34: "1321-1322-1323",
				int35: "1331-1332-1333",
			},
			{
				int3: "1331-1332-1333",
				int32: "1301-1302-1303",
				int33: "1341-1342-1343",
				int34: "1321-1322-1323",
				int35: "1331-1332-1333",
			},
		],
	},
	{
		id: "cs_06.04.0_0064",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 eq $int32 + $int33 + $int34 + $int35",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {},
				codes: [
					"1051-1052-1053",
					"1301-1302-1303",
					"1321-1322-1323",
					"1331-1332-1333",
					"1341-1342-1343",
				],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m1",
				},
				codes: ["1301-1302-1303"],
				fallback: "0",
			},
			{
				name: "int33",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m2",
				},
				codes: ["1341-1342-1343"],
				fallback: "0",
			},
			{
				name: "int34",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m3",
				},
				codes: ["1321-1322-1323"],
				fallback: "0",
			},
			{
				name: "int35",
				metric: "int3",
				dimensions: {
					"dim:pfc": "pfc:m4",
				},
				codes: ["1331-1332-1333"],
				fallback: "0",
			},
		],
		equation: "1052 = 1302 + 1342 + 1322 + 1332",
		bindings: [
			{
				int3: "1051-1052-1053",
				int32: "1301-1302-1303",
				int33: "1341-1342-1343",
				int34: "1321-1322-1323",
				int35: "1331-1332-1333",
			},
			{
				int3: "1301-1302-1303",
				int32: "1301-1302-1303",
				int33: "1341-1342-1343",
				int34: "1321-1322-1323",
				int35: "1331-1332-1333",
			},
			{
				int3: "1341-1342-1343",
				int32: "1301-1302-1303",
				int33: "1341-1342-1343",
				int34: "1321-1322-1323",
				int35: "1331-1332-1333",
			},
			{
				int3: "1321-1322-1323",
				int32: "1301-1302-1303",
				int33: "1341-1342-1343",
				int34: "1321-1322-1323",
				int35: "1331-1332-1333",
			},
			{
				int3: "1331-1332-1333",
				int32: "1301-1302-1303",
				int33: "1341-1342-1343",
				int34: "1321-1322-1323",
				int35: "1331-1332-1333",
			},
		],
	},
	{
		id: "cs_06.04.0_0065",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $dec12 + $dec13 + $dec14 + $dec15",
		variables: [
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
				},
				codes: [
					"1051-1052-1053",
					"1301-1302-1303",
					"1321-1322-1323",
					"1331-1332-1333",
					"1341-1342-1343",
				],
				fallback: "0",
			},
			{
				name: "dec12",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:pfc": "pfc:m1",
				},
				codes: ["1301-1302-1303"],
				fallback: "0",
			},
			{
				name: "dec13",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:pfc": "pfc:m2",
				},
				codes: ["1341-1342-1343"],
				fallback: "0",
			},
			{
				name: "dec14",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:pfc": "pfc:m3",
				},
				codes: ["1321-1322-1323"],
				fallback: "0",
			},
			{
				name: "dec15",
				metric: "dec1",
				dimensions: {
					"dim:qlt": "qlt:m3",
					"dim:pfc": "pfc:m4",
				},
				codes: ["1331-1332-1333"],
				fallback: "0",
			},
		],
		equation: "1053 = 1303 + 1343 + 1323 + 1333",
		bindings: [
			{
				dec1: "1051-1052-1053",
				dec12: "1301-1302-1303",
				dec13: "1341-1342-1343",
				dec14: "1321-1322-1323",
				dec15: "1331-1332-1333",
			},
			{
				dec1: "1301-1302-1303",
				dec12: "1301-1302-1303",
				dec13: "1341-1342-1343",
				dec14: "1321-1322-1323",
				dec15: "1331-1332-1333",
			},
			{
				dec1: "1341-1342-1343",
				dec12: "1301-1302-1303",
				dec13: "1341-1342-1343",
				dec14: "1321-1322-1323",
				dec15: "1331-1332-1333",
			},
			{
				dec1: "1321-1322-1323",
				dec12: "1301-1302-1303",
				dec13: "1341-1342-1343",
				dec14: "1321-1322-1323",
				dec15: "1331-1332-1333",
			},
			{
				dec1: "1331-1332-1333",
				dec12: "1301-1302-1303",
				dec13: "1341-1342-1343",
				dec14: "1321-1322-1323",
				dec15: "1331-1332-1333",
			},
		],
	},
	{
		id: "cs_06.04.0_0066",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:mdp": "mdp:m19",
				},
				codes: ["2051-2052-2053"],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:mdp": "mdp:m19",
				},
				codes: ["2051-2052-2053"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:mdp": "mdp:m19",
				},
				codes: ["2051-2052-2053"],
				fallback: "0",
			},
		],
		equation: "If 2052 is not completed then 2053 = 2051",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0067",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:mdp": "mdp:m19",
				},
				codes: ["2051-2052-2053"],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:mdp": "mdp:m19",
				},
				codes: ["2051-2052-2053"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:mdp": "mdp:m19",
				},
				codes: ["2051-2052-2053"],
				fallback: "0",
			},
		],
		equation: "If 2052 > 0 then 2053 < 2051 + 2052 and 2053 > 2051",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0068",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 eq $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:mdp": "mdp:m20",
				},
				codes: ["3051-3052-3053"],
				fallback: "()",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:mdp": "mdp:m20",
				},
				codes: ["3051-3052-3053"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:mdp": "mdp:m20",
				},
				codes: ["3051-3052-3053"],
				fallback: "0",
			},
		],
		equation: "If 3052 is not completed then 3053 = 3051",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0069",
		kind: "social-balance",
		section: "06.04.0",
		test: "$dec1 lt $int32 + $int3 and $dec1 gt $int32",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:mdp": "mdp:m20",
				},
				codes: ["3051-3052-3053"],
				fallback: "0",
			},
			{
				name: "dec1",
				metric: "dec1",
				dimensions: {
					"dim:mdp": "mdp:m20",
				},
				codes: ["3051-3052-3053"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:mdp": "mdp:m20",
				},
				codes: ["3051-3052-3053"],
				fallback: "0",
			},
		],
		equation: "If 3052 > 0 then 3053 < 3051 + 3052 and 3053 > 3051",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0070",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 gt 0 and $int4 gt 0 and $am1 gt 0 and $int3 lt $int4 and $int3 lt $am1",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:cvt": "cvt:m1",
				},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
				],
				fallback: "0",
			},
			{
				name: "int4",
				metric: "int4",
				dimensions: {
					"dim:hrs": "hrs:m2",
					"dim:cvt": "cvt:m1",
				},
				codes: ["5802-5812"],
				fallback: "0",
			},
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:ntr": "ntr:m9",
					"dim:cvt": "cvt:m1",
				},
				codes: ["5803-5813", "58032-58132", "58033-58133"],
				fallback: "0",
			},
		],
		equation:
			"If 5801 > 0 or 5802 > 0 or 5803 > 0 then 5801 > 0 and 5802 > 0 and 5803 > 0 and 5801 < 5802 and 5801 < 5803",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0071",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 gt 0 and $int4 gt 0 and $am1 gt 0 and $int3 lt $int4 and $int3 lt $am1",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:cvt": "cvt:m1",
				},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
				],
				fallback: "0",
			},
			{
				name: "int4",
				metric: "int4",
				dimensions: {
					"dim:hrs": "hrs:m2",
					"dim:cvt": "cvt:m1",
				},
				codes: ["5802-5812"],
				fallback: "0",
			},
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:ntr": "ntr:m9",
					"dim:cvt": "cvt:m1",
				},
				codes: ["5803-5813", "58032-58132", "58033-58133"],
				fallback: "0",
			},
		],
		equation:
			"If 5811 > 0 or 5812 > 0 or 5813 > 0 then 5811 > 0 and 5812 > 0 and 5813 > 0 and 5811 < 5812 and 5811 < 5813",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0072",
		kind: "social-balance",
		section: "06.04.0",
		test: "$am1 + $am12 lt $am2",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:ntr": "ntr:m9",
					"dim:cvt": "cvt:m1",
				},
				codes: ["5803-5813"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:ntr": "ntr:m9",
					"dim:cvt": "cvt:m1",
				},
				codes: ["5803-5813"],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {},
				codes: ["1021-1022-1023-1023P"],
				fallback: "0",
			},
		],
		equation: "If 5803 or 5813 are completed, then 5803 + 5813 < 1023",
		bindings: [
			{
				am1: "5803-5813",
				am12: "5803-5813",
				am2: "1021-1022-1023-1023P",
			},
		],
	},
	{
		id: "cs_06.04.0_0073",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 le $int32 + $int33",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:cvt": "cvt:m1",
				},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
				],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5821-5831",
					"5822-5832",
					"5823-5833",
					"5841-5851",
					"5842-5852",
					"5843-5853",
				],
				fallback: "0",
			},
			{
				name: "int33",
				metric: "int3",
				dimensions: {},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5821-5831",
					"5822-5832",
					"5823-5833",
					"5841-5851",
					"5842-5852",
					"5843-5853",
				],
				fallback: "0",
			},
		],
		equation: "5801 ≤ 1201 + 1202",
		bindings: [
			{
				int3: "5801-5811",
				int32: "5801-5811",
				int33: "5801-5811",
			},
			{
				int3: "5801-5811",
				int32: "5821-5831",
				int33: "5821-5831",
			},
			{
				int3: "5801-5811",
				int32: "5841-5851",
				int33: "5841-5851",
			},
			{
				int3: "5802-5812",
				int32: "5802-5812",
				int33: "5802-5812",
			},
			{
				int3: "5802-5812",
				int32: "5822-5832",
				int33: "5822-5832",
			},
			{
				int3: "5802-5812",
				int32: "5842-5852",
				int33: "5842-5852",
			},
			{
				int3: "58031-58131",
				int32: "58031-58131",
				int33: "58031-58131",
			},
			{
				int3: "58032-58132",
				int32: "58032-58132",
				int33: "58032-58132",
			},
			{
				int3: "58033-58133",
				int32: "58033-58133",
				int33: "58033-58133",
			},
			{
				int3: "5803-5813",
				int32: "5803-5813",
				int33: "5803-5813",
			},
			{
				int3: "5803-5813",
				int32: "5823-5833",
				int33: "5823-5833",
			},
			{
				int3: "5803-5813",
				int32: "5843-5853",
				int33: "5843-5853",
			},
		],
	},
	{
		id: "cs_06.04.0_0074",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 le $int32 + $int33",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:cvt": "cvt:m1",
				},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
				],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5821-5831",
					"5822-5832",
					"5823-5833",
					"5841-5851",
					"5842-5852",
					"5843-5853",
				],
				fallback: "0",
			},
			{
				name: "int33",
				metric: "int3",
				dimensions: {},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5821-5831",
					"5822-5832",
					"5823-5833",
					"5841-5851",
					"5842-5852",
					"5843-5853",
				],
				fallback: "0",
			},
		],
		equation: "5811 ≤ 1211 + 1212",
		bindings: [
			{
				int3: "5801-5811",
				int32: "5801-5811",
				int33: "5801-5811",
			},
			{
				int3: "5801-5811",
				int32: "5821-5831",
				int33: "5821-5831",
			},
			{
				int3: "5801-5811",
				int32: "5841-5851",
				int33: "5841-5851",
			},
			{
				int3: "5802-5812",
				int32: "5802-5812",
				int33: "5802-5812",
			},
			{
				int3: "5802-5812",
				int32: "5822-5832",
				int33: "5822-5832",
			},
			{
				int3: "5802-5812",
				int32: "5842-5852",
				int33: "5842-5852",
			},
			{
				int3: "58031-58131",
				int32: "58031-58131",
				int33: "58031-58131",
			},
			{
				int3: "58032-58132",
				int32: "58032-58132",
				int33: "58032-58132",
			},
			{
				int3: "58033-58133",
				int32: "58033-58133",
				int33: "58033-58133",
			},
			{
				int3: "5803-5813",
				int32: "5803-5813",
				int33: "5803-5813",
			},
			{
				int3: "5803-5813",
				int32: "5823-5833",
				int33: "5823-5833",
			},
			{
				int3: "5803-5813",
				int32: "5843-5853",
				int33: "5843-5853",
			},
		],
	},
	{
		id: "cs_06.04.0_0075",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 + $int32 le $int33 + $int34 + $int35 + $int36",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:cvt": "cvt:m1",
				},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
				],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:cvt": "cvt:m1",
				},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
				],
				fallback: "0",
			},
			{
				name: "int33",
				metric: "int3",
				dimensions: {},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5821-5831",
					"5822-5832",
					"5823-5833",
					"5841-5851",
					"5842-5852",
					"5843-5853",
				],
				fallback: "0",
			},
			{
				name: "int34",
				metric: "int3",
				dimensions: {},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5821-5831",
					"5822-5832",
					"5823-5833",
					"5841-5851",
					"5842-5852",
					"5843-5853",
				],
				fallback: "0",
			},
			{
				name: "int35",
				metric: "int3",
				dimensions: {},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5821-5831",
					"5822-5832",
					"5823-5833",
					"5841-5851",
					"5842-5852",
					"5843-5853",
				],
				fallback: "0",
			},
			{
				name: "int36",
				metric: "int3",
				dimensions: {},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"58031-58131",
					"58032-58132",
					"58033-58133",
					"5821-5831",
					"5822-5832",
					"5823-5833",
					"5841-5851",
					"5842-5852",
					"5843-5853",
				],
				fallback: "0",
			},
		],
		equation: "5801 + 5811 ≤ 1051 + 1052 + 3051 + 3052",
		bindings: [
			{
				int3: "5801-5811",
				int32: "5801-5811",
				int33: "5801-5811",
				int34: "5801-5811",
				int35: "5801-5811",
				int36: "5801-5811",
			},
			{
				int3: "5801-5811",
				int32: "5801-5811",
				int33: "5821-5831",
				int34: "5821-5831",
				int35: "5821-5831",
				int36: "5821-5831",
			},
			{
				int3: "5801-5811",
				int32: "5801-5811",
				int33: "5841-5851",
				int34: "5841-5851",
				int35: "5841-5851",
				int36: "5841-5851",
			},
			{
				int3: "5802-5812",
				int32: "5802-5812",
				int33: "5802-5812",
				int34: "5802-5812",
				int35: "5802-5812",
				int36: "5802-5812",
			},
			{
				int3: "5802-5812",
				int32: "5802-5812",
				int33: "5822-5832",
				int34: "5822-5832",
				int35: "5822-5832",
				int36: "5822-5832",
			},
			{
				int3: "5802-5812",
				int32: "5802-5812",
				int33: "5842-5852",
				int34: "5842-5852",
				int35: "5842-5852",
				int36: "5842-5852",
			},
			{
				int3: "58031-58131",
				int32: "58031-58131",
				int33: "58031-58131",
				int34: "58031-58131",
				int35: "58031-58131",
				int36: "58031-58131",
			},
			{
				int3: "58032-58132",
				int32: "58032-58132",
				int33: "58032-58132",
				int34: "58032-58132",
				int35: "58032-58132",
				int36: "58032-58132",
			},
			{
				int3: "58033-58133",
				int32: "58033-58133",
				int33: "58033-58133",
				int34: "58033-58133",
				int35: "58033-58133",
				int36: "58033-58133",
			},
			{
				int3: "5803-5813",
				int32: "5803-5813",
				int33: "5803-5813",
				int34: "5803-5813",
				int35: "5803-5813",
				int36: "5803-5813",
			},
			{
				int3: "5803-5813",
				int32: "5803-5813",
				int33: "5823-5833",
				int34: "5823-5833",
				int35: "5823-5833",
				int36: "5823-5833",
			},
			{
				int3: "5803-5813",
				int32: "5803-5813",
				int33: "5843-5853",
				int34: "5843-5853",
				int35: "5843-5853",
				int36: "5843-5853",
			},
		],
	},
	{
		id: "cs_06.04.0_0076",
		kind: "social-balance",
		section: "06.04.0",
		test: "$am1 eq $am12 + $am13 - $am14",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:ntr": "ntr:m9",
					"dim:cvt": "cvt:m1",
				},
				codes: ["5803-5813"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:spec": "spec:m84",
					"dim:cvt": "cvt:m1",
				},
				codes: ["58031-58131"],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {
					"dim:typ": "typ:m31",
					"dim:ntr": "ntr:m9",
					"dim:cvt": "cvt:m1",
					"dim:bnf": "bnf:m5",
				},
				codes: ["58032-58132"],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {
					"dim:ntr": "ntr:m9",
					"dim:bas": "bas:m112",
					"dim:mdp": "mdp:m21",
					"dim:cvt": "cvt:m1",
				},
				codes: ["58033-58133"],
				fallback: "0",
			},
		],
		equation: "5803 = 58031 + 58032 - 58033",
		bindings: [
			{
				am1: "5803-5813",
				am12: "58031-58131",
				am13: "58032-58132",
				am14: "58033-58133",
			},
		],
	},
	{
		id: "cs_06.04.0_0077",
		kind: "social-balance",
		section: "06.04.0",
		test: "$am1 eq $am12 + $am13 - $am14",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:ntr": "ntr:m9",
					"dim:cvt": "cvt:m1",
				},
				codes: ["5803-5813"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:spec": "spec:m84",
					"dim:cvt": "cvt:m1",
				},
				codes: ["58031-58131"],
				fallback: "0",
			},
			{
				name: "am13",
				metric: "am1",
				dimensions: {
					"dim:typ": "typ:m31",
					"dim:ntr": "ntr:m9",
					"dim:cvt": "cvt:m1",
					"dim:bnf": "bnf:m5",
				},
				codes: ["58032-58132"],
				fallback: "0",
			},
			{
				name: "am14",
				metric: "am1",
				dimensions: {
					"dim:ntr": "ntr:m9",
					"dim:bas": "bas:m112",
					"dim:mdp": "mdp:m21",
					"dim:cvt": "cvt:m1",
				},
				codes: ["58033-58133"],
				fallback: "0",
			},
		],
		equation: "5813 = 58131 + 58132 - 58133",
		bindings: [
			{
				am1: "5803-5813",
				am12: "58031-58131",
				am13: "58032-58132",
				am14: "58033-58133",
			},
		],
	},
	{
		id: "cs_06.04.0_0078",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 gt 0 and $int4 gt 0 and $am1 gt 0 and $int3 lt $int4 and $int3 lt $am1",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:cvt": "cvt:m2",
				},
				codes: ["5821-5831", "5822-5832", "5823-5833"],
				fallback: "0",
			},
			{
				name: "int4",
				metric: "int4",
				dimensions: {
					"dim:hrs": "hrs:m2",
					"dim:cvt": "cvt:m2",
				},
				codes: ["5822-5832"],
				fallback: "0",
			},
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:ntr": "ntr:m9",
					"dim:cvt": "cvt:m2",
				},
				codes: ["5823-5833"],
				fallback: "0",
			},
		],
		equation:
			"If 5821 > 0 or 5822 > 0 or 5823 > 0 then 5821 > 0 and 5822 > 0 and 5823 > 0 and 5821 < 5822 and 5821 < 5823",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0079",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 gt 0 and $int4 gt 0 and $am1 gt 0 and $int3 lt $int4 and $int3 lt $am1",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:cvt": "cvt:m2",
				},
				codes: ["5821-5831", "5822-5832", "5823-5833"],
				fallback: "0",
			},
			{
				name: "int4",
				metric: "int4",
				dimensions: {
					"dim:hrs": "hrs:m2",
					"dim:cvt": "cvt:m2",
				},
				codes: ["5822-5832"],
				fallback: "0",
			},
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:ntr": "ntr:m9",
					"dim:cvt": "cvt:m2",
				},
				codes: ["5823-5833"],
				fallback: "0",
			},
		],
		equation:
			"If 5831 > 0 or 5832 > 0 or 5833 > 0 then 5831 > 0 and 5832 > 0 and 5833 > 0 and 5831 < 5832 and 5831 < 5833",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0080",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 gt 0 and $int4 gt 0 and $am1 gt 0 and $int3 lt $int4 and $int3 lt $am1",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:cvt": "cvt:m3",
				},
				codes: ["5841-5851", "5842-5852", "5843-5853"],
				fallback: "0",
			},
			{
				name: "int4",
				metric: "int4",
				dimensions: {
					"dim:hrs": "hrs:m2",
					"dim:cvt": "cvt:m3",
				},
				codes: ["5842-5852"],
				fallback: "0",
			},
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:ntr": "ntr:m9",
					"dim:cvt": "cvt:m3",
				},
				codes: ["5843-5853"],
				fallback: "0",
			},
		],
		equation:
			"If 5841 > 0 or 5842 > 0 or 5843 > 0 then 5841 > 0 and 5842 > 0 and 5843 > 0 and 5841 < 5842 and 5841 < 5843",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0081",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 gt 0 and $int4 gt 0 and $am1 gt 0 and $int3 lt $int4 and $int3 lt $am1",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:cvt": "cvt:m3",
				},
				codes: ["5841-5851", "5842-5852", "5843-5853"],
				fallback: "0",
			},
			{
				name: "int4",
				metric: "int4",
				dimensions: {
					"dim:hrs": "hrs:m2",
					"dim:cvt": "cvt:m3",
				},
				codes: ["5842-5852"],
				fallback: "0",
			},
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:ntr": "ntr:m9",
					"dim:cvt": "cvt:m3",
				},
				codes: ["5843-5853"],
				fallback: "0",
			},
		],
		equation:
			"If 5851 > 0 or 5852 > 0 or 5853 > 0 then 5851 > 0 and 5852 > 0 and 5853 > 0 and 5851 < 5852 and 5851 < 5853",
		notApplicable: true,
	},
	{
		id: "cs_06.04.0_0082",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int4 + $int42 + $int43 + $int44 + $int45 + $int46 lt $int47",
		variables: [
			{
				name: "int4",
				metric: "int4",
				dimensions: {
					"dim:hrs": "hrs:m2",
					"dim:cvt": "cvt:m1",
				},
				codes: ["5802-5812"],
				fallback: "0",
			},
			{
				name: "int42",
				metric: "int4",
				dimensions: {
					"dim:hrs": "hrs:m2",
					"dim:cvt": "cvt:m1",
				},
				codes: ["5802-5812"],
				fallback: "0",
			},
			{
				name: "int43",
				metric: "int4",
				dimensions: {
					"dim:hrs": "hrs:m2",
					"dim:cvt": "cvt:m2",
				},
				codes: ["5822-5832"],
				fallback: "0",
			},
			{
				name: "int44",
				metric: "int4",
				dimensions: {
					"dim:hrs": "hrs:m2",
					"dim:cvt": "cvt:m2",
				},
				codes: ["5822-5832"],
				fallback: "0",
			},
			{
				name: "int45",
				metric: "int4",
				dimensions: {
					"dim:hrs": "hrs:m2",
					"dim:cvt": "cvt:m3",
				},
				codes: ["5842-5852"],
				fallback: "0",
			},
			{
				name: "int46",
				metric: "int4",
				dimensions: {
					"dim:hrs": "hrs:m2",
					"dim:cvt": "cvt:m3",
				},
				codes: ["5842-5852"],
				fallback: "0",
			},
			{
				name: "int47",
				metric: "int4",
				dimensions: {},
				codes: ["1011-1012-1013-1013P", "5802-5812", "5822-5832", "5842-5852"],
				fallback: "0",
			},
		],
		equation:
			"If 5802 or 5812 or 5822 or 5832 or 5842 or 5852 are completed, then 5802 + 5812 + 5822 + 5832 + 5842 + 5852 < 1013",
		bindings: [
			{
				int4: "5802-5812",
				int42: "5802-5812",
				int43: "5822-5832",
				int44: "5822-5832",
				int45: "5842-5852",
				int46: "5842-5852",
				int47: "5802-5812",
			},
			{
				int4: "5802-5812",
				int42: "5802-5812",
				int43: "5822-5832",
				int44: "5822-5832",
				int45: "5842-5852",
				int46: "5842-5852",
				int47: "5822-5832",
			},
			{
				int4: "5802-5812",
				int42: "5802-5812",
				int43: "5822-5832",
				int44: "5822-5832",
				int45: "5842-5852",
				int46: "5842-5852",
				int47: "5842-5852",
			},
			{
				int4: "5802-5812",
				int42: "5802-5812",
				int43: "5822-5832",
				int44: "5822-5832",
				int45: "5842-5852",
				int46: "5842-5852",
				int47: "1011-1012-1013-1013P",
			},
		],
	},
	{
		id: "cs_06.04.0_0083",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 + $int32 le $int33 + $int34 + $int35 + $int36",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:cvt": "cvt:m2",
				},
				codes: ["5821-5831", "5822-5832", "5823-5833"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:cvt": "cvt:m2",
				},
				codes: ["5821-5831", "5822-5832", "5823-5833"],
				fallback: "0",
			},
			{
				name: "int33",
				metric: "int3",
				dimensions: {},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"5821-5831",
					"5822-5832",
					"5823-5833",
					"5841-5851",
					"5842-5852",
					"5843-5853",
				],
				fallback: "0",
			},
			{
				name: "int34",
				metric: "int3",
				dimensions: {},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"5821-5831",
					"5822-5832",
					"5823-5833",
					"5841-5851",
					"5842-5852",
					"5843-5853",
				],
				fallback: "0",
			},
			{
				name: "int35",
				metric: "int3",
				dimensions: {},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"5821-5831",
					"5822-5832",
					"5823-5833",
					"5841-5851",
					"5842-5852",
					"5843-5853",
				],
				fallback: "0",
			},
			{
				name: "int36",
				metric: "int3",
				dimensions: {},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"5821-5831",
					"5822-5832",
					"5823-5833",
					"5841-5851",
					"5842-5852",
					"5843-5853",
				],
				fallback: "0",
			},
		],
		equation: "5821 + 5831 ≤ 1051 + 1052 + 3051 + 3052",
		bindings: [
			{
				int3: "5821-5831",
				int32: "5821-5831",
				int33: "5801-5811",
				int34: "5801-5811",
				int35: "5801-5811",
				int36: "5801-5811",
			},
			{
				int3: "5821-5831",
				int32: "5821-5831",
				int33: "5821-5831",
				int34: "5821-5831",
				int35: "5821-5831",
				int36: "5821-5831",
			},
			{
				int3: "5821-5831",
				int32: "5821-5831",
				int33: "5841-5851",
				int34: "5841-5851",
				int35: "5841-5851",
				int36: "5841-5851",
			},
			{
				int3: "5822-5832",
				int32: "5822-5832",
				int33: "5802-5812",
				int34: "5802-5812",
				int35: "5802-5812",
				int36: "5802-5812",
			},
			{
				int3: "5822-5832",
				int32: "5822-5832",
				int33: "5822-5832",
				int34: "5822-5832",
				int35: "5822-5832",
				int36: "5822-5832",
			},
			{
				int3: "5822-5832",
				int32: "5822-5832",
				int33: "5842-5852",
				int34: "5842-5852",
				int35: "5842-5852",
				int36: "5842-5852",
			},
			{
				int3: "5823-5833",
				int32: "5823-5833",
				int33: "5803-5813",
				int34: "5803-5813",
				int35: "5803-5813",
				int36: "5803-5813",
			},
			{
				int3: "5823-5833",
				int32: "5823-5833",
				int33: "5823-5833",
				int34: "5823-5833",
				int35: "5823-5833",
				int36: "5823-5833",
			},
			{
				int3: "5823-5833",
				int32: "5823-5833",
				int33: "5843-5853",
				int34: "5843-5853",
				int35: "5843-5853",
				int36: "5843-5853",
			},
		],
	},
	{
		id: "cs_06.04.0_0084",
		kind: "social-balance",
		section: "06.04.0",
		test: "$int3 + $int32 le $int33 + $int34 + $int35 + $int36",
		variables: [
			{
				name: "int3",
				metric: "int3",
				dimensions: {
					"dim:cvt": "cvt:m3",
				},
				codes: ["5841-5851", "5842-5852", "5843-5853"],
				fallback: "0",
			},
			{
				name: "int32",
				metric: "int3",
				dimensions: {
					"dim:cvt": "cvt:m3",
				},
				codes: ["5841-5851", "5842-5852", "5843-5853"],
				fallback: "0",
			},
			{
				name: "int33",
				metric: "int3",
				dimensions: {},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"5821-5831",
					"5822-5832",
					"5823-5833",
					"5841-5851",
					"5842-5852",
					"5843-5853",
				],
				fallback: "0",
			},
			{
				name: "int34",
				metric: "int3",
				dimensions: {},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"5821-5831",
					"5822-5832",
					"5823-5833",
					"5841-5851",
					"5842-5852",
					"5843-5853",
				],
				fallback: "0",
			},
			{
				name: "int35",
				metric: "int3",
				dimensions: {},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"5821-5831",
					"5822-5832",
					"5823-5833",
					"5841-5851",
					"5842-5852",
					"5843-5853",
				],
				fallback: "0",
			},
			{
				name: "int36",
				metric: "int3",
				dimensions: {},
				codes: [
					"5801-5811",
					"5802-5812",
					"5803-5813",
					"5821-5831",
					"5822-5832",
					"5823-5833",
					"5841-5851",
					"5842-5852",
					"5843-5853",
				],
				fallback: "0",
			},
		],
		equation: "5841 + 5851 ≤ 1051 + 1052 + 3051 + 3052",
		bindings: [
			{
				int3: "5841-5851",
				int32: "5841-5851",
				int33: "5801-5811",
				int34: "5801-5811",
				int35: "5801-5811",
				int36: "5801-5811",
			},
			{
				int3: "5841-5851",
				int32: "5841-5851",
				int33: "5821-5831",
				int34: "5821-5831",
				int35: "5821-5831",
				int36: "5821-5831",
			},
			{
				int3: "5841-5851",
				int32: "5841-5851",
				int33: "5841-5851",
				int34: "5841-5851",
				int35: "5841-5851",
				int36: "5841-5851",
			},
			{
				int3: "5842-5852",
				int32: "5842-5852",
				int33: "5802-5812",
				int34: "5802-5812",
				int35: "5802-5812",
				int36: "5802-5812",
			},
			{
				int3: "5842-5852",
				int32: "5842-5852",
				int33: "5822-5832",
				int34: "5822-5832",
				int35: "5822-5832",
				int36: "5822-5832",
			},
			{
				int3: "5842-5852",
				int32: "5842-5852",
				int33: "5842-5852",
				int34: "5842-5852",
				int35: "5842-5852",
				int36: "5842-5852",
			},
			{
				int3: "5843-5853",
				int32: "5843-5853",
				int33: "5803-5813",
				int34: "5803-5813",
				int35: "5803-5813",
				int36: "5803-5813",
			},
			{
				int3: "5843-5853",
				int32: "5843-5853",
				int33: "5823-5833",
				int34: "5823-5833",
				int35: "5823-5833",
				int36: "5823-5833",
			},
			{
				int3: "5843-5853",
				int32: "5843-5853",
				int33: "5843-5853",
				int34: "5843-5853",
				int35: "5843-5853",
				int36: "5843-5853",
			},
		],
	},
	{
		id: "cs_06.04.0_0085",
		kind: "social-balance",
		section: "06.04.0",
		test: "$am1 + $am12 lt $am2",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:ntr": "ntr:m9",
					"dim:cvt": "cvt:m2",
				},
				codes: ["5823-5833"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:ntr": "ntr:m9",
					"dim:cvt": "cvt:m2",
				},
				codes: ["5823-5833"],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {},
				codes: ["1021-1022-1023-1023P"],
				fallback: "0",
			},
		],
		equation: "If 5823 or 5833 are completed, then 5823 + 5833 < 1023",
		bindings: [
			{
				am1: "5823-5833",
				am12: "5823-5833",
				am2: "1021-1022-1023-1023P",
			},
		],
	},
	{
		id: "cs_06.04.0_0086",
		kind: "social-balance",
		section: "06.04.0",
		test: "$am1 + $am12 lt $am2",
		variables: [
			{
				name: "am1",
				metric: "am1",
				dimensions: {
					"dim:ntr": "ntr:m9",
					"dim:cvt": "cvt:m3",
				},
				codes: ["5843-5853"],
				fallback: "0",
			},
			{
				name: "am12",
				metric: "am1",
				dimensions: {
					"dim:ntr": "ntr:m9",
					"dim:cvt": "cvt:m3",
				},
				codes: ["5843-5853"],
				fallback: "0",
			},
			{
				name: "am2",
				metric: "am2",
				dimensions: {},
				codes: ["1021-1022-1023-1023P"],
				fallback: "0",
			},
		],
		equation: "If 5843 or 5853 are completed, then 5843 + 5853 < 1023",
		bindings: [
			{
				am1: "5843-5853",
				am12: "5843-5853",
				am2: "1021-1022-1023-1023P",
			},
		],
	},
];

const module: TaxonomyModule = {
	version: "26.0.15",
	model: "m08",
	part: "f",
	schemaRef: "http://www.nbb.be/be/fr/cbso/fws/26.0/mod/m08/m08-f.xsd",
	metricNamespace: "http://www.nbb.be/be/fr/cbso/dict/met",
	namespaces: {
		xsd: "http://www.w3.org/2001/XMLSchema",
		"s.01.00.1": "http://www.nbb.be/be/fr/cbso/fws/26.0/sect/s.01.00.1",
		ssf: "http://xbrl.org/2008/filter/segment-scenario",
		qlt: "http://www.nbb.be/be/fr/cbso/dict/dom/qlt",
		xsi: "http://www.w3.org/2001/XMLSchema-instance",
		table: "http://xbrl.org/2014/table",
		reference: "http://xbrl.org/2008/reference",
		bf: "http://xbrl.org/2008/filter/boolean",
		evt: "http://www.nbb.be/be/fr/cbso/dict/dom/evt",
		mmt: "http://www.nbb.be/be/fr/cbso/dict/dom/mmt",
		xlink: "http://www.w3.org/1999/xlink",
		ctc: "http://www.nbb.be/be/fr/cbso/dict/dom/ctc",
		gen: "http://xbrl.org/2008/generic",
		formula: "http://xbrl.org/2008/formula",
		variable: "http://xbrl.org/2008/variable",
		label: "http://xbrl.org/2008/label",
		prd: "http://www.nbb.be/be/fr/cbso/dict/dom/prd",
		bas: "http://www.nbb.be/be/fr/cbso/dict/dom/bas",
		lge: "http://www.nbb.be/be/fr/cbso/dict/dom/lge",
		uf: "http://xbrl.org/2008/filter/unit",
		"lge-enum": "http://www.nbb.be/be/fr/cbso/26.0/enum/lge-enum",
		nonnum: "http://www.xbrl.org/dtr/type/non-numeric",
		link: "http://www.xbrl.org/2003/linkbase",
		va: "http://xbrl.org/2008/assertion/value",
		enum: "http://xbrl.org/2014/extensible-enumerations",
		vf: "http://xbrl.org/2008/filter/value",
		ext: "http://www.nbb.be/be/fr/cbso/dict/ext",
		dcl: "http://www.nbb.be/be/fr/cbso/dict/dom/dcl",
		dim: "http://www.nbb.be/be/fr/cbso/dict/dim",
		part: "http://www.nbb.be/be/fr/cbso/dict/dom/part",
		xbrli: "http://www.xbrl.org/2003/instance",
		ef: "http://xbrl.org/2008/filter/entity",
		xbrldt: "http://xbrl.org/2005/xbrldt",
		tf: "http://xbrl.org/2008/filter/tuple",
		cf: "http://xbrl.org/2008/filter/concept",
		ea: "http://xbrl.org/2008/assertion/existence",
		met: "http://www.nbb.be/be/fr/cbso/dict/met",
		validation: "http://xbrl.org/2008/validation",
		df: "http://xbrl.org/2008/filter/dimension",
		ntl: "http://www.nbb.be/be/fr/cbso/dict/dom/ntl",
		model: "http://www.eurofiling.info/xbrl/ext/model",
		rf: "http://xbrl.org/2008/filter/relative",
		"pfs-dt": "http://www.nbb.be/be/fr/cbso/dict/dt",
		xl: "http://www.xbrl.org/2003/XLink",
		ca: "http://xbrl.org/2008/assertion/consistency",
		pf: "http://xbrl.org/2008/filter/period",
		gf: "http://xbrl.org/2008/filter/general",
		mf: "http://xbrl.org/2008/filter/match",
		psn: "http://www.nbb.be/be/fr/cbso/dict/dom/psn",
		rls: "http://www.nbb.be/be/fr/cbso/dict/dom/rls",
		"s.01.00.2": "http://www.nbb.be/be/fr/cbso/fws/26.0/sect/s.01.00.2",
		cty: "http://www.nbb.be/be/fr/cbso/dict/dom/cty",
		pcd: "http://www.nbb.be/be/fr/cbso/dict/dom/pcd",
		"lgf-enum": "http://www.nbb.be/be/fr/cbso/26.0/enum/lgf-enum",
		"cty-enum": "http://www.nbb.be/be/fr/cbso/26.0/enum/cty-enum",
		lgf: "http://www.nbb.be/be/fr/cbso/dict/dom/lgf",
		"pcd-enum": "http://www.nbb.be/be/fr/cbso/26.0/enum/pcd-enum",
		cct: "http://www.nbb.be/be/fr/cbso/dict/dom/cct",
		"cct-enum": "http://www.nbb.be/be/fr/cbso/26.0/enum/cct-enum",
		"s.02.01.0.ghi": "http://www.nbb.be/be/fr/cbso/fws/26.0/sect/s.02.01.0.ghi",
		nmt: "http://www.nbb.be/be/fr/cbso/dict/dom/nmt",
		"nmt-enum": "http://www.nbb.be/be/fr/cbso/26.0/enum/nmt-enum",
		fct: "http://www.nbb.be/be/fr/cbso/dict/dom/fct",
		"atc-enum": "http://www.nbb.be/be/fr/cbso/26.0/enum/atc-enum",
		atc: "http://www.nbb.be/be/fr/cbso/dict/dom/atc",
		"atc-npi": "http://www.nbb.be/be/fr/cbso/dict/dom/atc-npi",
		spec: "http://www.nbb.be/be/fr/cbso/dict/dom/spec",
		"fct-enum": "http://www.nbb.be/be/fr/cbso/26.0/enum/fct-enum",
		"atc-npi-enum": "http://www.nbb.be/be/fr/cbso/26.0/enum/atc-npi-enum",
		open: "http://www.nbb.be/be/fr/cbso/dict/dom/open",
		"s.03.01.0.cdefhi":
			"http://www.nbb.be/be/fr/cbso/fws/26.0/sect/s.03.01.0.cdefhi",
		sts: "http://www.nbb.be/be/fr/cbso/dict/dom/sts",
		spu: "http://www.nbb.be/be/fr/cbso/dict/dom/spu",
		typ: "http://www.nbb.be/be/fr/cbso/dict/dom/typ",
		rst: "http://www.nbb.be/be/fr/cbso/dict/dom/rst",
		ntr: "http://www.nbb.be/be/fr/cbso/dict/dom/ntr",
		"s.03.02.0.i": "http://www.nbb.be/be/fr/cbso/fws/26.0/sect/s.03.02.0.i",
		rsn: "http://www.nbb.be/be/fr/cbso/dict/dom/rsn",
		"s.04.00.0.i": "http://www.nbb.be/be/fr/cbso/fws/26.0/sect/s.04.00.0.i",
		mdp: "http://www.nbb.be/be/fr/cbso/dict/dom/mdp",
		bkd: "http://www.nbb.be/be/fr/cbso/dict/dom/bkd",
		"s.05.00.0.ghi": "http://www.nbb.be/be/fr/cbso/fws/26.0/sect/s.05.00.0.ghi",
		bnf: "http://www.nbb.be/be/fr/cbso/dict/dom/bnf",
		"s.06.01.1.cdefhi":
			"http://www.nbb.be/be/fr/cbso/fws/26.0/sect/s.06.01.1.cdefhi",
		aln: "http://www.nbb.be/be/fr/cbso/dict/dom/aln",
		rut: "http://www.nbb.be/be/fr/cbso/dict/dom/rut",
		dom: "http://www.nbb.be/be/fr/cbso/dict/dom/",
		"s.06.01.2.hi": "http://www.nbb.be/be/fr/cbso/fws/26.0/sect/s.06.01.2.hi",
		"s.06.01.3.cdefhi":
			"http://www.nbb.be/be/fr/cbso/fws/26.0/sect/s.06.01.3.cdefhi",
		"s.06.02.0.i": "http://www.nbb.be/be/fr/cbso/fws/26.0/sect/s.06.02.0.i",
		gtd: "http://www.nbb.be/be/fr/cbso/dict/dom/gtd",
		"s.06.03.0.i": "http://www.nbb.be/be/fr/cbso/fws/26.0/sect/s.06.03.0.i",
		"s.06.04.0.i": "http://www.nbb.be/be/fr/cbso/fws/26.0/sect/s.06.04.0.i",
		jcc: "http://www.nbb.be/be/fr/cbso/dict/dom/jcc",
		hrs: "http://www.nbb.be/be/fr/cbso/dict/dom/hrs",
		dgl: "http://www.nbb.be/be/fr/cbso/dict/dom/dgl",
		pfc: "http://www.nbb.be/be/fr/cbso/dict/dom/pfc",
		epc: "http://www.nbb.be/be/fr/cbso/dict/dom/epc",
		cvt: "http://www.nbb.be/be/fr/cbso/dict/dom/cvt",
		wrg: "http://www.nbb.be/be/fr/cbso/dict/dom/wrg",
		"jcc-enum": "http://www.nbb.be/be/fr/cbso/26.0/enum/jcc-enum",
		wkr: "http://www.nbb.be/be/fr/cbso/dict/dom/wkr",
		gdr: "http://www.nbb.be/be/fr/cbso/dict/dom/gdr",
		"s.06.05.0.i": "http://www.nbb.be/be/fr/cbso/fws/26.0/sect/s.06.05.0.i",
		"s.07.00.0.ghi": "http://www.nbb.be/be/fr/cbso/fws/26.0/sect/s.07.00.0.ghi",
		"s.08.00.0.abghi": "http://www.nbb.be/be/fr/cbso/fws/26.0/sect/s.08.00.0.abghi",
		opn: "http://www.nbb.be/be/fr/cbso/dict/dom/opn",
		"opn-enum": "http://www.nbb.be/be/fr/cbso/26.0/enum/opn-enum",
	},
	datapoints: [...datapoints0, ...datapoints1],
	checks: [...checks0],
};

export default module;
