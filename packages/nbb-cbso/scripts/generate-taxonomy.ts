 ...
 *
 * Each model becomes its own file under `src/taxonomies/`, published as the
 * subpath `@financica/nbb-cbso/taxonomies/<model>-<part>`. There is no index
 * of them: a caller imports the module it files against and passes it in,
 * which is what keeps the models a bundler cannot see out of the bundle. The
 * enumerations go to `src/generated/`, which the main entry does import.
 */
...
/**
 * Rows per emitted constant.
 *
 * `tsc` infers the type of an array literal from its elements before checking
 * it against the annotation, and on a table of two thousand distinct object
 * literals that inference blows past its complexity limit (TS2590). Spreading
 * a few annotated constants into the table keeps every element checked
 * against `Datapoint` and `Check` while never asking for the literal's own
 * type.
 */
const ROWS_PER_CHUNK = 500;

function emit(module: TaxonomyModule): string {
	const { datapoints, checks, ...rest } = module;
	const chunk = <T>(name: string, type: string, rows: readonly T[]) => {
		const constants: string[] = [];
		const names: string[] = [];
		for (let i = 0; i < rows.length; i += ROWS_PER_CHUNK) {
			const id = `${name}${constants.length}`;
			names.push(id);
			constants.push(
				`const ${id}: readonly ${type}[] = ${JSON.stringify(rows.slice(i, i + ROWS_PER_CHUNK), null, "\t")}\n`,
			);
		}
		return { constants, spread: `[${names.map((n) => `...${n}`).join(", ")}]` };
	};
	const dp = chunk("datapoints", "Datapoint", datapoints);
	const ck = chunk("checks", "Check", checks);
	const body = JSON.stringify(rest, null, "\t").replace(
		/\n}$/,
		`,\n\t"datapoints": ${dp.spread},\n\t"checks": ${ck.spread}\n}`,
	);
	return `// Generated by scripts/generate-taxonomy.ts from NBB-CBSO ${module.version}.
...
${dp.constants.join("\n")}
${ck.constants.join("\n")}
const module: TaxonomyModule = ${body}
...
function main(): void {
	...
	const srcDir = join(dirname(import.meta.dirname), "src");
	const outDir = join(srcDir, "taxonomies");
	...
	const enumerations = join(generatedDir, "enumerations.ts");
	writeFileSync(enumerations, emitEnumerations(options));

	// Emitted as JSON, formatted like the rest of the source, so that a re-run
	// against the same release is a no-op in git rather than a whitespace diff.
	const files = [...written.map((t) => join(outDir, `${t}.ts`)), enumerations];
	const formatted = spawnSync("bunx", ["oxfmt", ...files], { stdio: "inherit" });
