...
- **The serializer writes the whole of EN 16931.** Every business term the model holds now goes out at its place in the UBL 2.1 sequence: the tax point date (BT-7) and its code (BT-8), the VAT accounting currency (BT-6) with the second `cac:TaxTotal` carrying BT-111, the buyer accounting reference (BT-19), the reference set (order BT-13/BT-14, contract BT-12, project BT-11, receiving advice BT-15, despatch BT-16, tender BT-17 and invoiced object BT-18, the last two as typed `cac:AdditionalDocumentReference`s, and BT-26 on the billing reference), party identifications, legal form and contacts, the payee (BG-10), the tax representative (BG-11), delivery with its party, location id and address (BG-13), payment means with credit transfer, direct debit and card details (BG-16; on a credit note BT-9 rides in the first one as `cbc:PaymentDueDate`), document allowances and charges with their VAT category (BG-20/BG-21) and BT-107/BT-108 in the monetary total, line allowances and charges (BG-27/BG-28), the price discount (BT-147/BT-148), the exemption reason code (BT-121), attachment descriptions and text-only document references, and per line the note (BT-127), object identifier (BT-128), order line reference (BT-132), accounting reference (BT-133), buyer, seller and standard item identifiers, commodity classifications, origin country and attributes. Line-level tax totals stay unwritten: BIS Billing 3.0 forbids them.
- **The serializer checks the arithmetic.** Beyond the mandatory fields it now throws `UblBuildError`, naming the rule and the model path, for a document whose figures break BR-CO-3, BR-CO-10 to BR-CO-13, BR-CO-15, BR-CO-16, BR-53, the BR-S-08 family, or Peppol's R040 (allowance percentage) and R120 (line net against quantity, price and line allowances), and for a payment means without a code, a credit transfer without an account or a direct debit without a mandate. These all failed at the access point before, after the document had left.
- **`buildTaxTotals` takes `allowanceCharges`.** Each document-level item joins the VAT subtotal of its own category and feeds BT-107/BT-108, so BT-109 is BT-106 − BT-107 + BT-108. **`allocateAcrossTaxCategories(amount, lines, template)`** splits one amount across the categories the lines use, pro rata by line net with the cent remainders on the largest shares, for a shipping charge or an invoice-wide discount on a mixed-rate invoice.
- **The parser reads the same set**, so a document survives parse → serialize → parse unchanged: payee, tax representative, delivery party and location id, tax currency and BT-111, accounting references, despatch, receipt, tender and object references, line note, order line reference, standard item id, classifications, origin country, mandate debited account and card details, and a credit note's due date from its payment means. The model gains the matching fields (`UblSchemedId`, `UblPriceAllowance`, `UblCommodityClassification`; `payee`, `taxRepresentative`, `taxCurrency`, `accountingCost`, `receivingAdviceReference`, `despatchReference`, `tenderReference`, `invoicedObjectId`; `UblDelivery.locationId`/`partyName`; `UblPaymentMeans.debitedAccount`/`cardNumber`/`cardHolder`; `UblTaxTotal.taxAmountInTaxCurrency`; on a line `note`, `objectIdentifier`, `orderLineReference`, `accountingCost`, `standardItemId`, `commodityClassifications`, `originCountryCode`, `priceAllowance`).

### Changed

- **BREAKING: a price discount is `UblLine.priceAllowance`, not an entry of `allowanceCharges`.** `cac:Price/cac:AllowanceCharge` (BT-147/BT-148) used to be merged into the line's allowance list, which lost where it came from and could not be written back. `allowanceCharges` now holds only BG-27/BG-28; `discountAmount` still counts both, and `normalizeUblResponse` still lists the price discount in `extra.allowance_charges`, so the DTO is unchanged.

## 0.17.0

### Changed

- **BREAKING: one UBL model for the parser, the serializer and the build helpers.** `UblInvoice` (root export, re-exported from `@financica/ubl/build`) replaces the build-side `UblDocument`, which remains as a deprecated alias for one release. On the parsed model: `UblParty.endpointId`/`endpointSchemeId` → `endpoint: { value, scheme? }`; `companyId: string` + `companyIdSchemeId` → `companyId: { value, scheme? }`; `UblLine.taxPercent`/`taxCategoryId`/`taxSchemeId` → `taxCategory: { id?, percent?, schemeId?, exemptionReason?, exemptionReasonCode? }` (same object on `UblAllowanceCharge`); `UblTaxSubtotal.taxPercent`/`taxCategoryId`/`taxSchemeId`/`taxExemptionReason` → `category` (same shape); the top-level `taxSubtotals` → `taxTotal: { taxAmount?, subtotals }`, where `taxAmount` is BT-110 as the document states it (never summed from the subtotals); `UblLine.baseQuantity` (BT-149) is now read. On the build side: `supplier`/`customer` → `seller`/`buyer`, `vatNumber` → `vatId`, `legalName` → `registrationName`, `priceAmount` → `unitPrice` (the per-unit price; the serializer writes `PriceAmount = unitPrice × baseQuantity`, and `deriveUnitPrice` returns `{ unitPrice, baseQuantity? }`), line `name` → `description` (written as `cbc:Description`, and as `cbc:Name` unless `itemName` is set), attachment `base64` → `base64Content`, `precedingInvoiceId` → `billingReference.invoiceId`, `UblPeriod` → `UblInvoicePeriod`, `documentType: "invoice" | "creditNote"` → `"Invoice" | "CreditNote"`, and every absent output field is an absent key instead of `null` (`buildCompanyId`, `parsePeppolEndpoint`, `resolveVatEndpoint`, `resolveCompanyIdScheme` return `undefined`; `buildSupplierParty`/`buildCustomerParty`/`normalizeAddress` omit absent keys). Caller-input types (`UblSupplier`, `UblCustomer`, `AddressInput`, `CustomerTaxIdInput`) still accept `null`. `buildTaxTotals` throws `UblBuildError` for a line without a `taxCategory`. `normalizeUblResponse` produces the same `InvoiceExtractionDTO`; only the `extra.tax_subtotals` passthrough changes shape (each entry is now `{ taxableAmount?, taxAmount?, category? }`).
- **BREAKING: `serializeUblDocument` validates and throws `UblBuildError`.** Also exported as `serializeUblInvoice` (the preferred name). Because the model is permissive, the serializer enforces what EN 16931 / Peppol BIS Billing 3.0 make mandatory and throws the new `UblBuildError` (exported from the root and `/build`, `name` set, `readonly cause?`) naming the BT and model path: document `id`, `issueDate`, `currency`, `seller.name`, `buyer.name`, an `endpoint.scheme` on any endpoint, at least one line, per line `quantity`, `unitCode`, `unitPrice`, `lineExtensionAmount`, `taxCategory.id` (and `taxCategory.percent` unless the category is `O`), `taxTotal.taxAmount`, `monetaryTotal.lineExtensionAmount`/`taxExclusiveAmount`/`taxInclusiveAmount`/`payableAmount`, and per VAT subtotal its `taxableAmount`, `taxAmount` and `category`. An attachment needs `filename`, `mimeCode` and `base64Content`; an attachment with only `externalUri` is written as `cac:ExternalReference`. `customizationId`, `profileId` and `invoiceTypeCode` are written when set (defaulting to the BIS values), so a parsed document re-serializes faithfully.
